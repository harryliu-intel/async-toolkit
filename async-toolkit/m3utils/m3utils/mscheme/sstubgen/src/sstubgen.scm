;;
;; $Id: sstubgen.scm,v 1.11 2009/04/17 09:45:56 mika Exp $
;;
...
(define (format-parenthesized-signature sig env)
	;; format a procedure (or method) signature
	(string-append 
	 "("
	 (infixize (map (lambda(x)(format-formal x env)) 
...
(define (convert-subrange-type type env)
	(let ((names (extract-field 'elts type))
				(m3tn (string-type-alias type "." env))
				(u3tn (string-type-alias type "_" env)))
		(string-append
		 "PROCEDURE ConvertTo_" u3tn "(obj : SchemeObject.T) : " m3tn dnl
		 "  RAISES { Scheme.E } = " dnl
		 "  BEGIN" dnl
		 "    IF ISTYPE(obj, SchemeLongReal.T) THEN" dnl
		 "      WITH o = ROUND(SchemeLongReal.FromO(obj)) DO" dnl
		 "        IF o < ORD(FIRST("m3tn")) OR o > ORD(LAST("m3tn")) THEN" dnl
		 "           RAISE Scheme.E(\"converting to "m3tn": \" & Fmt.Int(o) &" dnl
		 "              \" out of range: \" & Fmt.Int(ORD(FIRST("m3tn"))) &" dnl
		 "             \"..\" & Fmt.Int(ORD(LAST("m3tn"))))" dnl
		 "        ELSE" dnl
		 "          RETURN VAL(o, "m3tn")" dnl
		 "        END" dnl
		 "      END" dnl
		 "    ELSIF ISTYPE(obj, SchemeSymbol.T) THEN" dnl
		 "      FOR i := FIRST("u3tn"_Names) TO LAST("u3tn"_Names) DO" dnl
		 "        IF "u3tn"_Names[i] = obj THEN RETURN i END" dnl
		 "      END" dnl
		 "    END;" dnl
		 "    RAISE Scheme.E(\"don't know how to convert to "m3tn":\" & " dnl
		 "             SchemeUtils.Stringify(obj))" dnl
		 "  END ConvertTo_"u3tn";" dnl
		 dnl
		 "VAR "u3tn"_Names := ARRAY [FIRST("m3tn")..LAST("m3tn")] OF Atom.T" dnl
		 "    { " (all-subrange-values xs) " };" dnl
		 dnl
		 )))

(define (all-subrange-values xs)
	(define (atomize string)
		(string-append "Atom.FromText(\"" string "\")"))
	(infixize (map atomize (map symbol->string (extract-field 'elts xs))) ", ")
	)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (convert-to-bridge type env)
	(if (not (memq (car type) '(Object Opaque))) 
			(error "cant bridge " (car type)))

	(let* ((m3tn        (string-type-alias type "." env))
				 (u3tn        (string-type-alias type "_" env))
				 (bridge-name (string-append u3tn "_Bridge"))
				 (methods (extract-field 'methods type)))

		(define (meth-proc-name meth-symbol)
			(string-append u3tn "_" (symbol->string meth-symbol) "_method"))

		(define (do-override sym sig)
			(string-append
			 "    " (symbol->string sym) " := " (meth-proc-name sym) ";" dnl)
			)

		(define (do-procedure sym sig)
			(let ((self-name (string-append "_self_" u3tn))
						(result (cddr (assoc 'result sig)))
						(formals (formals-from-sig sig))
						)
				...
				(let ((param-list 
							 (infixize 
								(cons 
								 (string-append self-name ": " bridge-name)
								 (map format-formal-decl formals))
								"; "
								))
							(call-list (infixize
													(cons self-name (map format-formal-param formals))
													", "))
							)

					(string-append
					 "PROCEDURE " (meth-proc-name sym) "(" param-list ") "(return-decl)"=" dnl
					 "  BEGIN" dnl
					 "    " (return-kw) m3tn "." (symbol->string sym) "(" call-list ")" dnl
					 "  END " (meth-proc-name sym) ";" dnl
					 dnl
					 ))))
		

		(define (iterate-methods what) 
			(apply string-append
						 (map 
							(lambda (x) 
		...
		(string-append 
		 "TYPE" dnl
		 "  " bridge-name " = " m3tn " BRANDED \"" bridge-name "\" OBJECT" dnl
     "  OVERRIDES" dnl
		 (iterate-methods do-override)
		 "  END;" dnl
		 dnl
		 (iterate-methods do-procedure)
		 )
...
(define (sxt) 
	(set! xtt (find-type-alias '(Example . TT) the-types))
	(set! xo (find-type-alias '(Example . O) the-types))
	(set! xoab (find-type-alias '(Example . OAB) the-types))
	(set! xt (find-type-alias '(Example . T) the-types)))
...
(define (reload) (load "/home/mika/t/mscheme/sstubgen/src/sstubgen.scm"))
...
(define env-type 
	(make-struct-type
	 ...
	 `((exports       ,(lambda()(make-symbol-set 100))) 
		 ...
		 (procedure-call-stubs ,(lambda()(make-symbol-hash-table 100))) 
		...
		(define (make-impl)
			(string-append
				 ...
				 ((UserDefined) 
					(let ((elts (extract-field 'elts type)))
						...
						(string-append 
						 ...
						 "        END" dnl
						 "      END;" dnl
						 "      RAISE Scheme.E(\"Not a value of "m3tn" : \" & Stringify(x))" dnl
						 "    END"
						 )
						)
					)
				 ...
				 ((Subrange)
					(let* ((base (extract-field 'base type))
						...
						(string-append
						 ...
						 "        RAISE Scheme.E(\"Value out of range for "m3tn" :\" & Stringify(x))" dnl
						 "      END" dnl
						 "    END"
						 )
						)
             ...
             "        WITH desc = SchemePair.Pair(p.first) DO" dnl
						 ...
						 "            RAISE Scheme.E(\"NIL pair in \" & Stringify(x))" dnl
						 "          END;" dnl
             ...
             "      END" dnl
				 ...
				 ((Procedure Object Opaque) 
					(string-append
					 ...
					 "      RAISE Scheme.E(\"Not of type "m3tn" : \" & Stringify(x))" dnl
					 "    END;" dnl
					 "    RETURN x" 
					 )
					)
...
(define (make-procedure-call-stub proc env)
	(let* ((qid (car proc))
				...
				(define (unpack-var f)
					...
					(string-append
					 (if (eq? (extract-field 'mode f) 'Mode.Var)
							 (string-append
								"        SchemeUtils.SetFirst(p," (to-scheme-proc-name arg-type env)"(" arg-name deref-caret"));" dnl
								)
							 ""
							 )
					 "        EVAL Next();" 
		...
		(let ((stubs (env 'get 'procedure-call-stubs))
					(qualified-name (string->symbol
													 (stringify-qid (cleanup-qid qid) 
																					"." 
																					(env-type 'new))))
					)
     ...
     "  PROCEDURE Next() : SchemeObject.T RAISES { Scheme.E } = " dnl
		 "    BEGIN" dnl
		 "      TRY RETURN SchemeUtils.First(p) FINALLY p := SchemePair.Pair(p.rest) END" dnl
		 "    END Next;" dnl	
		 " " dnl

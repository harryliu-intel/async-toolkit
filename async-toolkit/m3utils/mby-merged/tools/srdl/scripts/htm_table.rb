...
#
# Usage:
# % rdl.rb htm_table <rdl-in-file> <html-out-file>
#

class HtmTable < RDL::ViewGen
  ...
  def initialize(top_rdl_file, view_file)
    @security_info = SecurityInfo.new(ENV['MST_ROOT']+"/src/srdl/security.pm")
    super(top_rdl_file)                            # init @top_map
    File.delete view_file if File.exist? view_file # force re-generate
    source(view_file, "htm_table") # init @view_code from html.template if new
...
module Html
  ...
  class Table < Array
    ...
    # Construct HTML table w/ coroutine in caller
    def initialize()
      super()                   # is-an Array
  ...
  def self.desc(desc)           # NOTE: implements non-std markup
    term = ""
    html = Array.new

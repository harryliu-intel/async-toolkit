...
#define MEMORY_DEBUG_CALLER   TRUE
...
#define IN_SHARED_MEMORY(x)                             \
    ( (void *) (x) >= (void *) FM_SHARED_MEMORY_ADDR && \
     (void *) (x) < (void *) (FM_SHARED_MEMORY_ADDR + FM_SHARED_MEMORY_SIZE) )
...
static void *fmAlloc_(fm_uint size, int lock); /* prototype local */
...
static fm_memoryBucket *GetBucket(fm_sharedHeader *hdr, fm_uint size)
{
    ...
    {
      /* mutex is already locked here -- why wasnt this captured in
         the code before ? Is it safe/OK to lock the mutex multiple times? */
        bucket = fmAlloc_( sizeof(fm_memoryBucket), 0 );
 ...
 *
 *****************************************************************************/
static void *fmAlloc_(fm_uint size, int lock)
{
    fm_memoryBucket *bucket;
    fm_memoryBucket *it;
    ...
    if (bucket != NULL)
    {
        ...
        if (newObject == NULL)
        {
            ...
            if ( (void *) (ptr + size) <=
                 (void *) (FM_SHARED_MEMORY_ADDR + FM_SHARED_MEMORY_SIZE) )
            {
 ...
 *
 *****************************************************************************/
static void fmFree_(void *obj, int lock)
{
    fm_memoryBucket *bucket;
    fm_objectHeader *objHdr;
    fm_sharedHeader *hdr = (fm_sharedHeader *) FM_SHARED_MEMORY_ADDR;
    ...
    if ( !IN_SHARED_MEMORY(obj) )
    {
        ...
        FM_LOG_FATAL( FM_LOG_CAT_ALOS,
                     ...
                     (void *) (FM_SHARED_MEMORY_ADDR + FM_SHARED_MEMORY_SIZE - 1) );
...
#define MAX_TRACKED_BUCKETS 1000
    ...
    while (bucket != NULL)
    {
        ...
        *buf    = 0;
        ...
        for (i = 7 ; i >= 0 ; i--)
        {
            ...
            {
                retVal = snprintf(buf, bufSize, "%s%u",
    ...
    *buf    = 0;
    ...
    {
        retVal = snprintf(buf, bufSize, "%s%s",
...
void fmGetAllocatedMemorySize(fm_uint32 *allocMemory)
{
    ...
    *allocMemory = managed - freed - overhead;
...
{
    *ptr = (void *) (FM_SHARED_MEMORY_ADDR + FM_SHARED_MEMORY_SIZE);
...
fm_status fmMemInitialize(void)
{
    ...
    else
    {
        ...
        else if (addr != (void *) FM_SHARED_MEMORY_ADDR)
        {
            FM_LOG_FATAL(FM_LOG_CAT_ALOS,
                         "Can't map 0x%x-byte region at %p (mapped address: %p)\n",
                         FM_SHARED_MEMORY_SIZE,
                         (void *) FM_SHARED_MEMORY_ADDR,
                         addr);
            FM_LOG_EXIT(FM_LOG_CAT_ALOS, FM_FAIL);
    ...
    if (!hdr->initialized)
    {
        ...
        hdr->self                = addr;
        hdr->end                 = (void *) (FM_SHARED_MEMORY_ADDR
                                    + FM_SHARED_MEMORY_SIZE);
...
okay2:
        ...
        if ( hdr->end != (void *) (FM_SHARED_MEMORY_ADDR +
                                   FM_SHARED_MEMORY_SIZE) )
        {
            FM_LOG_FATAL(FM_LOG_CAT_ALOS,
...
/* #define ALLOC_DO_DEBUG 1 */
/* #undef ALLOC_DO_DEBUG */

#define ALLOC_REDZONESZ 64
/* ALLOC_REDZONESZ must be a multiple of largest alignment unit on arch */

/* layout of allocated structure 

fmAlloc returns -->   hlp_dalloc_dbg_t   
                      REDZONE
return to user  -->   user area
                      REDZONE
*/

#define HEADER_MAGIC 0xc001babe

typedef struct {
  fm_uint     header_magic;
  const char *filea; /* where allocated */
  int         linea;
  const char *filef; /* where freed */
  int         linef;
  void       *ptr;
  fm_uint     sz;
  fm_uint     active; /* 1 if allocated */
  char        padding[ALLOC_REDZONESZ]; /* force max alignment */
} hlp_dalloc_dbg_t;

#define HLP_DEBUG_REDZONE_HASH(i) (((i)+3)%17)

#define UNNIL(str) ((str) ? (str) : "**NULL**")

static void dfill_redzone(char *p, int base)
{
  int i;
  for (i=0; i<ALLOC_REDZONESZ; ++i)
    p[i] = (char)HLP_DEBUG_REDZONE_HASH(base+i);
}

static void dcheck_redzone(const char *p,
                           const hlp_dalloc_dbg_t *dbg,
                           const char *file,
                           const int line)
{
  int i;
  for (i=0; i<ALLOC_REDZONESZ; ++i)
    if(p[i] != (char)HLP_DEBUG_REDZONE_HASH(i)) {
      /* the reason to break the following up into multiple LOG messages
         is that if we fail, memory is corrupted, and we might get a seg
         fault or bus error on dereferencing dbg */
      
      FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                   "Redzone corrupted at 0x%lx, offset %d free at %s:%d\n",
                   (unsigned long)(p+i), i, file, line);
      FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                   "Redzone corrupted at 0x%lx, alloc at %s:%d\n",
                   (unsigned long)(p+i),
                   UNNIL(dbg->filea), dbg->linea);
      FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                   "Redzone corrupted at 0x%lx, ptr 0x%lx sz %d(0x%x) lim 0x%lx\n",
                   (unsigned long)(p+i),
                   (unsigned long)dbg->ptr,
                   dbg->sz,
                   dbg->sz,
                   (unsigned long)((char *)dbg->ptr+dbg->sz));
      FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                   "Redzone corrupted at 0x%lx, redzone at 0x%lx sz %d(0x%x) lim 0x%lx\n",
                   (unsigned long)(p+i),
                   (unsigned long) p,
                   ALLOC_REDZONESZ,
                   ALLOC_REDZONESZ,
                   (unsigned long)(p + ALLOC_REDZONESZ));
      break;
    }
}
...
typedef struct {
  ...
  long        nout;
  long        bout;
} fm_alos_allocacctg_t;

static unsigned hash_acctg(const void *x)
{
  const fm_alos_allocacctg_t *q = x;
  return q->linea + hashtable_hash_string(q->filea);
}

static unsigned comp_acctg(const void *x, const void *y)
{
  const fm_alos_allocacctg_t *q = x, *r = y;
  return q->linea == r->linea && !strcmp(q->filea, r->filea);
}

static int compar_acctg(const void *x, const void *y)
{
  /* compare bytes */
  const fm_alos_allocacctg_t *q = *(const fm_alos_allocacctg_t **)x,
                             *r = *(const fm_alos_allocacctg_t **)y;

  if      (q->bout > r->bout)
    return -1;
  else if (q->bout < r->bout)
    return 1;

  return 0;
}

static hashtable_t *acctgTbl = NULL; /* protected by LockMutex */

/* versions to use when already holding the lock */
static void *fmAllocL(size_t size) { return fmAlloc_(size, 0); }
static void fmFreeL(void *obj) { fmFree_(obj, 0); }

static       long int used      = 0;
#define DEBUG_MEM_STEP            16L*1024L*1024L
static       long int debuglim  = DEBUG_MEM_STEP;


static int fmDebugAccountingUnderLock(int          upDown,
                                      size_t       bytes,
                                      const char  *filea,
                                      int          linea)
/* returns whether it is time for a diagnostic dump */
{
  fm_alos_allocacctg_t *acct = fmAllocL(sizeof(fm_alos_allocacctg_t));
  void *vp;
  
  if (acctgTbl == NULL)
    acctgTbl = hashtable_init(hash_acctg, comp_acctg, fmAllocL, fmFreeL);

  memset(acct, 0, sizeof(fm_alos_allocacctg_t));
  acct->filea = filea;
  acct->linea = linea;

  if (hashtable_get(acctgTbl, acct, &vp)) {
    fmFreeL(acct);
    acct = vp;
  }
  else
    hashtable_put(acctgTbl, acct);
...
void fmDebugAccountingDump(void)
{
  ...
  n = hashtable_size(acctgTbl);
  arr = fmAllocL(n * sizeof(fm_alos_allocacctg_t *));

  iter = hashtable_iterate(acctgTbl);

  i = 0;
  while(hashtable_iterator_next(iter, &x))
    arr[i++] = x;

  qsort(arr, n, sizeof(fm_alos_allocacctg_t *), compar_acctg);

  fmLogMessage(0, FM_LOG_LEVEL_PRINT,
               __FILE__, __func__, __LINE__,
               ">>>>>>>>>>>>>>>>>>>>  fmAlloc debug accounting  >>>>>>>>>>>>>>>>>>>>\n"); 
  for (i=0; i<n; ++i) {
    fm_alos_allocacctg_t *r = arr[i];
    fmLogMessage(0, FM_LOG_LEVEL_PRINT,
                 __FILE__, __func__, __LINE__,
                 "@ %-60s l. %4d alloc %6ld / %10ld : free %6ld / %10ld  out %6ld / %10ld\n",
                 r->filea, r->linea,
                 r->nalloc, r->balloc,
                 r->nfree, r->bfree,
                 r->nout, r->bout);
...
static void fmDebugAccounting(int          upDown,
                              size_t       bytes,
                              const char  *filea,
                              int          linea)
{
  ...
  LockMutex(hdr);
  doDump = fmDebugAccountingUnderLock(upDown, bytes, filea, linea);
  UnlockMutex(hdr);

  if(doDump) fmDebugAccountingDump();
}

/**********************************************************************/

#define ACTIVE_MAGIC   0xc0edbabe
#define INACTIVE_MAGIC 0xdeadcafe

#define DEBUG(...) do { if (0) FM_LOG_ENTRY(FM_LOG_CAT_ALOS, __VA_ARGS__); } while(0)

void *fmAllocWrap(fm_uint sz, const char *file, const int line)
{
  void *res;
  assert(file); /* this is a code consistency check */
#ifdef ALLOC_DO_DEBUG
  int asz = sz + sizeof(hlp_dalloc_dbg_t) + 2*ALLOC_REDZONESZ;
  char *base;
  hlp_dalloc_dbg_t *dbg;

  base = fmAlloc_(asz,1);
  if (!base) goto FAIL;
  res = base + sizeof(hlp_dalloc_dbg_t) + ALLOC_REDZONESZ;
  
  dbg = (hlp_dalloc_dbg_t *)base;
  dbg->header_magic = HEADER_MAGIC;
  dbg->linea = line;
  dbg->filea = file;
  dbg->ptr   = res;
  dbg->sz    = sz;
  dbg->active=  ACTIVE_MAGIC;

  fmDebugAccounting(+1,  dbg->sz, dbg->filea, dbg->linea);
  DEBUG("fmAlloc usage %lu\n", used);
  dfill_redzone((char *)base + sizeof(hlp_dalloc_dbg_t), 0);
  dfill_redzone((char *)res  + sz                      , 0);
  
#else /*!ALLOC_DO_DEBUG*/
  res = fmAlloc_(sz,1);
  if (!res) goto FAIL;
#endif /*ALLOC_DO_DEBUG*/

  DEBUG("%s(%d, %p=%s:%d) -> %p\n", __func__, sz, file, file, line, res);
  return res;
...
void fmFreeWrap(void *obj, const char *file, const int line)
{
  ...
  void *ptr;
  hlp_dalloc_dbg_t *dbg;
  ptr = (char *)obj - sizeof(hlp_dalloc_dbg_t) - ALLOC_REDZONESZ;
  dbg = (hlp_dalloc_dbg_t *)ptr;

  DEBUG("%s(%p, %s:%d) alloc @ %s:%d\n", __func__, obj, file, line, dbg->filea, dbg->linea);

  if (dbg->header_magic != HEADER_MAGIC) {
    FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                 "fmFree: header magic overwritten: 0x%x\n", dbg->header_magic);
  }

  if (dbg->active == ACTIVE_MAGIC) {
    fmDebugAccounting(-1,  dbg->sz, dbg->filea, dbg->linea);
    dbg->filef = file;
    dbg->linef = line;
    dbg->active = INACTIVE_MAGIC;
  } else if (dbg->active == INACTIVE_MAGIC) {
    FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                 "fmFree of inactive area at %s:%d, allocated at %s:%d previous free at %s:%d\n",
                 file, line, UNNIL(dbg->filea), dbg->linea, UNNIL(dbg->filef), dbg->linef);
  } else {
    FM_LOG_FATAL(FM_LOG_CAT_PLATFORM,
                 "fmFree of non-active area at %s:%d, corrupted active=0x%x\n", file, line, dbg->active);
  }
  
  dcheck_redzone((char *)ptr + sizeof(hlp_dalloc_dbg_t), dbg, file, line);
  dcheck_redzone((char *)obj + dbg->sz, dbg, file, line);
  DEBUG("fmFree %p sz %d lim 0x%lx usage %lu\n", obj, dbg->sz, (unsigned long)obj + dbg->sz, used);

  dfill_redzone((char *)ptr + sizeof(hlp_dalloc_dbg_t), 1); /* clobber it */
  dfill_redzone((char *)obj + dbg->sz                 , 1);

  /* for good measure, lets blow away the data too */
...
void *fmAllocP(fm_uint size) { return fmAllocWrap(size, "**FUNCTION-POINTER**", -1); }
void fmFreeP(void *obj) { fmFreeWrap(obj, "**FUNCTION-POINTER**", -1); }

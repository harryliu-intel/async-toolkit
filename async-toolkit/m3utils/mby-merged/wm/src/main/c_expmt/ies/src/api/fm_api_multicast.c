 ...
 *
 * \param[in]       port is the listener port number.
 *
 * \param[in]       vlan is the listener vlan number.
 *
                                  ...
                                  fm_intMulticastListener **newListener)
{
    ...
    FM_LOG_ENTRY( FM_LOG_CAT_MULTICAST,
                 "group=%p(%d), port=%d, vlan=%u, newListener=%p\n",
                 (void *) group,
                 group->handle,
                 port,
                 vlan,
    ...
    memset(intListener, 0, nbytes);

    intListener->vlan  = vlan;
    intListener->port  = port;
                                ...
                                fm_int                   port)
{
    ...
    {
        subListener->remoteFlag = FALSE;
    }
    else
    {
        subListener->remoteFlag = listener->remoteFlag;
    }
...
static fm_status AddRemoteListener(fm_int                   sw,
                                   ...
                                   fm_intMulticastListener *intListener)
{
    fm_int                           internalPort = -1;
    fm_intMulticastInternalListener *internalPortListener = NULL;
    fm_multicastListener             tempListener;
    fm_int                           nbytes;
    fm_status                        err;

    FM_LOG_ENTRY(FM_LOG_CAT_MULTICAST,
                 "sw=%d, group=%d, vlan=%u, port=%d\n",
                 sw,
                 group->handle,
                 intListener->vlan,
                 intListener->port);
    ...
    if (internalPortListener == NULL)
    {
        ...
        nbytes = sizeof(fm_intMulticastInternalListener);

        internalPortListener = fmAlloc(nbytes);
        ...
        memset(internalPortListener, 0, nbytes);
    ...
    {
        memset( &tempListener, 0, sizeof(tempListener) );
                                ...
                                fm_intMulticastListener *intListener)
{
    ...
    err = fmLogicalPortToLagIndex(sw, intListener->port, &lagIndex);
    if (err == FM_OK)
    {
        for (cpi = 1 ; cpi < switchPtr->numCardinalPorts ; cpi++)
        {
            lagPort = GET_LOGICAL_PORT(sw, cpi);
            if ( fmPortIsInLAG(sw, lagPort, lagIndex) )
            {
                                       ...
                                       fm_intMulticastListener *intListener)
{
    ...
    FM_LOG_ENTRY(FM_LOG_CAT_MULTICAST,
                 "sw = %d, group = %p(%d), intListener = %p (vlan %u, port %d)\n",
                 sw,
                 (void *) group,
                 group->handle,
                 (void *) intListener,
    ...
    switchPtr = GET_SWITCH_PTR(sw);
    portPtr   = GET_PORT_PTR(sw, intListener->port);
    ...
    switch (portPtr->portType)
    {
        ...
        case FM_PORT_TYPE_REMOTE:
            ...
            if (internalPortListener->listenerCount <= 0)
            {
                memset( &tempListener, 0, sizeof(tempListener) );
                tempListener.vlan = FM_INVALID_INTERFACE_VLAN;
                tempListener.port = internalPortListener->port;

                err = fmDeleteMcastGroupListener(sw,
                                                 group->handle,
                                                 &tempListener);
                                         ...
                                         fm_intMulticastListener *intListener)
{
    ...
    err = fmTreeRemove(&portPtr->mcastGroupList,
                       GET_MG_KEY(group->handle, intListener->vlan),
                       NULL);
    ...
    err = fmTreeRemove(&group->listenerTree,
                       GET_LISTENER_KEY(intListener->port, intListener->vlan),
                       NULL);
 ...
 *
 *****************************************************************************/
static fm_int CompareMulticastAddresses(const void *key1, const void *key2)
{
                                ...
                                fm_intMulticastListener ** newListener)
{
    ...
    listenerKey = GET_LISTENER_KEY(listener->port, listener->vlan);
    ...
    if ((switchInfo.switchFamily == FM_SWITCH_FAMILY_FM4000) || \
        (switchInfo.switchFamily == FM_SWITCH_FAMILY_REMOTE_FM4000))
    {
        intListener->remoteFlag = FALSE;
    }
    else
    {
        intListener->remoteFlag = listener->remoteFlag;
    }
    ...
    err = fmTreeInsert(&portPtr->mcastGroupList,
                       GET_MG_KEY(groupPtr->handle, listener->vlan),
                       (void *) intListener);
...
fm_status fmCreateMcastGroupInt(fm_int sw, fm_int *mcastGroup, fm_bool stacking)
{
    ...
    group->singleAddressMode  = fmGetBoolApiAttribute(FM_AAK_API_1_ADDR_PER_MCAST_GROUP,
    ...
    fmCustomTreeInit(&group->addressTree, CompareMulticastAddresses);
...
fm_status fmMcastGroupInit(fm_int sw)
{
    ...
    fmCustomTreeInit(&switchPtr->mcastAddressTree, CompareMulticastAddresses);
...
fm_status fmCreateMcastGroup(fm_int sw, fm_int *mcastGroup)
{
    ...
    err = fmCreateMcastGroupInt(sw, mcastGroup, FALSE);
...
fm_status fmDeleteMcastGroup(fm_int sw, fm_int mcastGroup)
{
    ...
    while (1)
    {
        ...
        portPtr = switchPtr->portTable[listener->port];

        err = fmTreeRemove(&portPtr->mcastGroupList,
                           GET_MG_KEY(group->handle, listener->vlan),
                           NULL);
        FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_MULTICAST, err);
                                  ...
                                  fm_multicastListener *listener)
{
    ...
    fm_bool                          lockTaken = FALSE;
    fm_intMulticastGroup *           group = NULL;
    fm_intMulticastListener *        intListener = NULL;
    ...
    err = ValidateListenerPort(sw, listener->port);
    FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_MULTICAST, err);
 ...
 *
 * \param[in]       max is the size of listenerList, being the maximum number
 *                  of mmulticast group listeners that listenerList can hold.
 *
 * \return          FM_OK if successful.
 * \return          FM_ERR_INVALID_SWITCH if sw is invalid.
 * \return          FM_ERR_INVALID_MULTICAST_GROUP if mcastGroup is out of
                                      ...
                                      fm_int                max)
{
    ...
    *numListeners = 0;
    ...
    while (1)
    {
        ...
        listenerList[listOffset].vlan = intListener->vlan;
        listenerList[listOffset].port = intListener->port;
        listOffset++;
        (*numListeners)++;
    }
                                       ...
                                       fm_multicastListener *firstListener)
{
    ...
    if (err == FM_OK)
    {
        firstListener->vlan = intListener->vlan;
        firstListener->port = intListener->port;
        firstListener->remoteFlag = intListener->remoteFlag;
     }
                                      ...
                                      fm_multicastListener *nextListener)
{
    ...
    if (err == FM_OK)
    {
        ...
        if (err == FM_OK)
        {
            ...
            if (err == FM_OK)
            {
                nextListener->vlan = intListener->vlan;
                nextListener->port = intListener->port;
                nextListener->remoteFlag = intListener->remoteFlag;
            }
        }
    }
 ...
 * \return          FM_FAIL if an internal logic error occurs.
 *
 *****************************************************************************/
                               ...
                               fm_int mcastGroup)
{
    ...
    fm_intMulticastGroup *    group = NULL;
    fm_intMulticastListener * intListener;
    fm_intMulticastListener **listenerList = NULL;
    fm_int                    listenerCount = 0;
    fm_int                    nbytes;
    fm_int                    i;
    fm_treeIterator           iter;
    fm_uint64                 key;
    ...
    fm_bool                   logicalPortAllocated = FALSE;
    fm_bool                   portGroupInserted    = FALSE;
    fm_bool                   listenersAdded       = FALSE;
    fm_bool                   activatedGroup       = FALSE;
    fm_customTreeIterator     addrIter;
    fm_mcastAddrKey *         addrKey;
    fm_mcastAddrKey *         addrValue;
    fm_bool                   needEcmpGroup = TRUE;
...
fm_status fmMcastDeleteLagNotify(fm_int sw, fm_int lagIndex)
{
    ...
    fm_uint64                key;
    fm_intMulticastListener *listener;
    fm_multicastListener     lagPortListener;
    ...
    while (1)
    {
        ...
        lagPortListener.vlan = listener->vlan;
        lagPortListener.port = lagLogicalPort;
...
fm_status fmMcastDeleteVlanNotify(fm_int sw, fm_int vlan)
{
    ...
    while (fmTreeIterNext( &iterGrp, &key, (void **) &group) == FM_OK)
    {
        ...
        if (nbAddresses > 0)
        {
            ...
            {        
                listenerList = fmAlloc(sizeof(fm_multicastListener) * 
                ...
                {
                    if (listener->vlan == vlan)
                    {
                        listenerList[listenerListIndex].port = listener->port;
                        listenerList[listenerListIndex].vlan = listener->vlan;
                        listenerListIndex++;
                    }
                }
                ...
                {
                    FM_LOG_DEBUG(FM_LOG_CAT_MULTICAST,
                                 "Deleting mcastGrp=%d listener: port=%d, vlan=%d\n",
                                 group->handle, 
                                 listenerList[i].port, 
                                 listenerList[i].vlan);
                    err = fmDeleteMcastGroupListener(sw,

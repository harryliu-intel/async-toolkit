...
/* The precompiled headers process requires a fake variable to placed in
...
/* We extern the 2k, 4k and 6k structure and identify functions so that
...
extern const fm_switch           HLPSwitchDefaultTable;

extern fm_status hlpIdentifySwitch(fm_int            sw,
                                      fm_switchFamily * family,
                                      fm_switchModel *  model,
                                      fm_switchVersion *version);

extern fm_status hlpGetSwitchPartNumber(fm_int sw, fm_switchPartNum *pn);

#endif

#if FM_SUPPORT_CPK

extern const fm_switch           CPKSwitchDefaultTable;

extern fm_status cpkIdentifySwitch(fm_int            sw,
                                      fm_switchFamily * family,
                                      fm_switchModel *  model,
                                      fm_switchVersion *version);

extern fm_status cpkGetSwitchPartNumber(fm_int sw, fm_switchPartNum *pn);
...
static const fm_switchModelEntry fmSwitchModelList[] =
{
...
#if FM_SUPPORT_SWAG
    {
        ...
        FM_SWITCH_MODEL_SWAG,               /* switch model */
...
#if FM_SUPPORT_HLP
    {
        FM_SWITCH_FAMILY_HLP,
        FM_SWITCH_MODEL_FM10440,
        &HLPSwitchDefaultTable,
    },
#endif

#if FM_SUPPORT_CPK
    {
        FM_SWITCH_FAMILY_CPK,
        FM_SWITCH_MODEL_FM10440,
        &CPKSwitchDefaultTable,
    },
#endif
...
/*****************************************************************************/
/** fmAllocateGlobalLocks
 * \ingroup intSwitch
 *
 * \desc            create API global locks
 *
 * \return          FM_OK if successful.
 *
 *****************************************************************************/
static fm_status fmAllocateGlobalLocks(void)
{
    fm_status err = FM_OK;

    FM_LOG_ENTRY_NOARGS(FM_LOG_CAT_SWITCH);

    err = fmCreateLockV2("Table Update Stats", 
                         FM_LOCK_SWITCH_NONE,
                         FM_LOCK_PREC_TABLE_UPDATE_STATS,
                         &fmRootApi->tableUpdateStats.lck);
...
fm_status fmSetSwitchModelSpecificInfo(fm_int sw)
{
    ...
    switch(switchPtr->switchFamily)
    {
        ...
        case FM_SWITCH_FAMILY_HLP:
            status = hlpGetSwitchPartNumber(sw, &swPartNum);
            break;
#endif

#if FM_SUPPORT_CPK
        case FM_SWITCH_FAMILY_CPK:
            status = cpkGetSwitchPartNumber(sw, &swPartNum);
            break;
#endif
...
static fm_status fmAllocateSwitchDataStructures(fm_int sw)
{
    ...
    swstate->paritySweeperCfg.enabled = 
                fmGetBoolApiAttribute(FM_AAK_API_PARITY_SWEEPER_ENABLE, 
                                      FM_AAD_API_PARITY_SWEEPER_ENABLE);

    swstate->paritySweeperCfg.readBurstSize = 
                fmGetIntApiAttribute(FM_AAK_API_PARITY_SWEEPER_READ_BURST_SIZE,
                                     FM_AAD_API_PARITY_SWEEPER_READ_BURST_SIZE);

    swstate->paritySweeperCfg.sleepPeriod = 
                fmGetIntApiAttribute(FM_AAK_API_PARITY_SWEEPER_SLEEP_PERIOD,
    ...
    if ( fmRootApi->isSwitchFibmSlave[sw] &&
         !fmGetBoolApiAttribute(FM_AAK_API_PARITY_SWEEPER_FIBM_ENABLE, 
...
static fm_status fmFreeSwitchDataStructures(fm_int sw)
{
     ...
     **************************************************/
    if ( ( err = fmFreeLAGTable(sw) ) != FM_OK )
    {
        FM_LOG_ERROR(FM_LOG_CAT_SWITCH, "Fail to free LAG table: %s\n",
                     fmErrorMsg(err));
        retErr = err;
        /* Don't return, just continue on */
    }
...
//    err = fmVNFree(sw);
    ...
    if (err != FM_OK)
    {
        FM_LOG_ERROR(FM_LOG_CAT_SWITCH, "Fail to free router structure: %s\n",
                     fmErrorMsg(err));
        retErr = err;
        /* Don't return, just continue on */
    }
...
static fm_status fmInitializeSwitchDataStructure(fm_int sw)
{
    ...
    err = fmInitCounters(&swstate->counterInfo);
    FM_LOG_EXIT_ON_ERR(FM_LOG_CAT_SWITCH, err);
    ...
    /* Initialize load balancing group */
    err = fmInitLBG(swstate);
    FM_LOG_EXIT_ON_ERR(FM_LOG_CAT_SWITCH, err);
    ...
    /* the vlan learning mode is set to shared vlan mode */
    swstate->vlanLearningMode   = FM_VLAN_LEARNING_MODE_SHARED;
...
static fm_status fmApiRootInit(void)
{
    ...
    /* FIXME: move more locks into this function */
    err = fmAllocateGlobalLocks();
    FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_SWITCH, err);
...
err = 0;
    ...
    if ( !fmGetBoolApiAttribute(FM_AAK_DEBUG_BOOT_INTERRUPT_HANDLER,
    ...
    if (fmGetBoolApiAttribute(FM_AAK_API_MA_TABLE_MAINTENENANCE_ENABLE,
     ...
     */
    if ( fmGetBoolApiAttribute(FM_AAK_API_FAST_MAINTENANCE_ENABLE,
    ...
    if ( fmGetBoolApiAttribute(FM_AAK_API_PACKET_RECEIVE_ENABLE,
    ...
    if ( fmGetBoolApiAttribute(FM_AAK_API_PARITY_SWEEPER_ENABLE, 
                               FM_AAD_API_PARITY_SWEEPER_ENABLE) )
    {
 ...
 *                  clock frequency.
 *
 * \return          The frame handler clock fequency in MHz.
 *
 *****************************************************************************/
fm_float fmComputeFHClockFreq(fm_int sw)
{
    ...
    fhMhz = fmGetIntApiAttribute(FM_AAK_API_FM4000_BOOT_FHFREQ,
                                 FM_AAD_API_FM4000_BOOT_FHFREQ);

    FM_API_CALL_FAMILY(err, switchPtr->ComputeFHClockFreq, sw, &pllP, &pllM, &pllN);

    if (err == FM_OK)
    {
        fhRefClock = fmGetFloatApiAttribute(FM_AAK_API_SYSTEM_FH_REF_CLOCK,
                                        FM_AAD_API_SYSTEM_FH_REF_CLOCK);

        fhMhz = ( (fm_float) fhRefClock )
                * (fm_float) pllM
                / ( (fm_float) pllN )
                / ( (fm_float) (1 << pllP) );
    }
...
err = 0;
    ...
    if ( ( (err == FM_OK) || !state )
        ...
        && fmGetBoolApiAttribute(FM_AAK_API_SWAG_AUTO_SUB_SWITCHES, 
...
ABORT:
    ...
    if ((err != FM_OK) && fmRootApi->fmSwitchStateTable[sw])
    {
...
fm_status fmLockSwitch(fm_int sw)
{
    ...
    if (fmRootApi->fmSwitchLockTable[sw] == NULL)
    {
        ...
        snprintf(lockName, sizeof(lockName), "SwitchRwLock%d", sw);
...
fm_status fmHandleSwitchInserted(fm_int sw, fm_eventSwitchInserted *insertEvent)
{
    ...
    switch (insertEvent->model)
    {
        ...
        case FM_SWITCH_MODEL_SWAG:
            ...
            swptr->switchModel   = FM_SWITCH_MODEL_SWAG;
    ...
    if ( !fmGetBoolApiAttribute(FM_AAK_DEBUG_BOOT_IDENTIFYSWITCH,
    ...
    fmDelay( 0, fmGetIntApiAttribute(FM_AAK_API_BOOT_RESET_TIME,
...
ALREADY_IDENTIFIED:
    ...
    {
#endif
        if ( fmGetBoolApiAttribute(FM_AAK_DEBUG_BOOT_RESET,

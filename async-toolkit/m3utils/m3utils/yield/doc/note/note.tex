...
yield that do not depend on the assumption of independence by three
...
The work we describe here was in most parts anticipated by Koren and
Koren~25~years ago, but some of the derivations we present are
different, and we present a working implementation that makes good on
the promise in Koren and Koren that ``the extension to a larger number
of module types is straightforward'' while demonstrating (we believe)
that it is not quite as ``cumbersome'' as Koren and Koren
may have believed.\cite{koren}  In any case, the Koren and Koren paper
should be read as a companion to the current report.
...
First, we introduce the standard Euler differential operator $D_x$.\cite{differentials}  We write
\begin{equation}
  \begin{aligned}
...
the currently used yield models at Intel.  Israel Koren graciously
worked through his results with the author and provided up-to-date errata.
Whatever errors remain are
...
\begin{equation}
  x = Y(1)
\end{equation}
and a generalized extended polynomial is an expression of the form
\begin{equation}
  \sum_i a_i x^{p_i}
\end{equation}
where $a_i$ and $p_i$ are real (in particular, $p_i$ is not limited to
integers as in a regular polynomial---this is what makes our
polynomials ``generalized'') and $x \ge 0$.  Since $x$ is a yield and we
...
The full system also includes convenience functions for describing the
yield of subsystems and showing how they contribute to the total yield.
We do not cover the details in this report, but sample output is shown in figure~\ref{fig:Hal9000Screenshot.png}.  The output shows the effect of yield
...
\end{enumerate}
The fact that the polynomials grow exponentially is unfortunate.
...
For completeness, the following is the (obvious) implementation of
\verb+YieldModel.Stapper+, in the Modula-3 programming language.\cite{spwm3}
\begin{verbatim}
PROCEDURE Stapper(A, D0, n, alpha : LONGREAL) : LONGREAL =
  CONST 
    SqMmPerSqInch = 25.4d0 * 25.4d0;
  VAR
    Ai := A / SqMmPerSqInch;
  BEGIN
...
\bibitem{koren}{I.~Koren and Z.~Koren.  Defect tolerance in VLSI circuits: techniques and yield analysis.  {\it Proc.\ IEEE.} 9({\bf 86}), 1998.}

\bibitem{differentials}{L.~Euler.  {\it Institutiones Calculi Differentialis.} St.~Petersburg: Academiae Imperialis Scientiarum Petropolitanae, 1755.}

                                      ...
                                      fm_int *size)
{
    ...
    fm_policerInfo *      info    = &fmRootApi->fmSwitchStateTable[sw]->policerInfo;
    fm_status             treeErr = fmTreeFind(&info->policers, policer, &value);
                          ...
                          const fm_policerConfig *config)
{
    ...
    if (config == NULL)
    {
        ...
        entry->attributes.colorSource = FM_POLICER_COLOR_SRC_DSCP;
                          ...
                          fm_int policer)
{
    ...
    err = fmTreeRemove(&info->policers, policer, fmFreeP);
...
fm_status fmGetPolicerFirst(fm_int sw, fm_int *firstPolicer)
{
    ...
    info = &fmRootApi->fmSwitchStateTable[sw]->policerInfo;
...
fm_status fmDestroyPolicers(fm_policerInfo *info)
{
    ...
    fmTreeDestroy(&info->policers, fmFreeP);

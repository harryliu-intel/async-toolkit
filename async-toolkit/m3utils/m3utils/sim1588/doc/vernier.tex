...
\begin{abstract}
  ...
  clock domains is achieved, and far better would be possible in
  other applications.
\end{abstract}
...
multi-synchronous system.  A multi-synchronous system is a system where
...
clock domain and an event $E_1$ in another clock domain.  The required
...
the system as a whole or on the data path from $E_0$ to $E_1$, in case
they are causally related (but we also do not wish to require that they be
so related).
...
the usual way\footnote{It bears mentioning that this way of measuring time is only usual for intervals.  It is decidedly unusual to measure calendar time with a uniform time scale.  See the following.}, as a uniform and steady physical time scale.  Following
...
scale, or TAI ({\em Temps Atomique International\/}\cite{nbs}), which is a
steady count of SI seconds that agreed with the more commonly used
Universal Time on January~1, 1958.\footnote{TAI and UTC diverge
  because of leap seconds that are inserted into the latter scale and make it non-uniform.  As of the time of writing, TAI is
  precisely 37~s ahead of UTC.}
...
time into the past; and (2)~as the time at the receiver of the event
(not the sender).

The delay time {\tt dt} is to be measured in some time unit $\tau$, which
varies depending on the application.  (For example, the base unit
could be picoseconds, $\tau=1$~ps.  The representation of a current event would
have ${\tt dt}=0$, while one that is 2.3~ns old would have ${\tt
  dt}=2300 \tau$.)
...
$t_{0A}$ as zero or any other desired epoch.
...
\begin{equation}
  ...
  will vary (in some
pattern---{\em it does not matter which\/}) between 0 and $p_B$.}
...
\subsubsection{Non-ideal clocks}
...
remains to determine $\Delta$, and we want it to be between 0 and $p_B$ as
before.  It is easy to see that it does not matter which of the two
clocks is inaccurate.  What matters is that their ratio is a bit off
from $p_A/p_B$.  There are two cases:
...
negative after $\mathcal{C}^*$ is set.  However, we need do nothing special for this
case: simply keep monitoring $n_A p_A - n_B p_B$ and set $\mathcal{C}^*$ to the
smallest value as before.  This will always maintain a $\Delta^*$ that is
positive and between 0 and $p_B$.  If the ratio is off by a lot, $p_B$ may
never be reached.  (But to put things in perspective, a ``large'' error
...
monitor $n_A p_A - n_B p_B$ and set $\mathcal{C}^*$ to the largest value observed, less $p_B$.
...
{\bf if} $s > -\mathcal{C}^* + p_B$, &{\bf then} perform the update $\mathcal{C}^* := -(s - p_B)$\cr
}
...
{\bf if} $s > K + p_B$, &{\bf then} perform the update $K := s - p_B$\cr
}
...
\begin{equation}
  { \kappa \over p_A } = {p_B \over GCD(p_A,p_B)} \quad .
\end{equation}
...
\end{equation}
where $\pi_A \perp \pi_B$, by the Chinese Remainder Theorem\cite{hardy},
\begin{equation}
...
\[ GCD(p_E,p_E) = p_E \]
the precision of the measurement is one full clock cycle.
...
to each other (which is the normal situation, even in the case of so-called ``Synchronous Ethernet'').
...
Nothing in this document has so far mentioned wiring delays.  Systems built
using the modules described here are not assumed to have zero wiring
delays, nor are wiring delays considered unimportant.  In fact, the
main application for which the work was done is IEEE1588 Precision
Time Protocol (PTP).  In the PTP protocol, digital hardware calculates
and corrects for queuing delays, leaving wiring delays as a residual to
be measured by capturing packet round-trip delays.  The fact that wiring
delays are not considered in this document is thus deliberate: they are
intended to be measured and corrected for by higher-level protocols.
...
\item Ensure that paths whose delays are significant in the final
  result are as close to always through the same transistors.
...
\subsection{B Clock Domain Preparation Logic}
...
into the $A$ domain or it can be divided by an even divisor.  Odd divisors
are not supported because they would require negedge logic.
...
\subsection{A Clock Domain Calculation Logic}
...
increase or decrease without bound ({\em diverge\/}) if the clocks are drifting with respect
to each other.
...
Observe that the only value of interest to us is the difference
...
$s$ and $K$ do not diverge.  We introduce logic that monitors $s$, and
if $s$ gets too positive or too negative, it subtracts or adds an
arbitrary number from/to $s$ and subtracts or adds the same number
from/to $K$.  This algorithm maintains $s$ near zero and since it keeps the
phase $\Delta^*$ unchanged, that remains accurate.
...
Most constants of interest in the design are exported to the integrator as
...
serial symbol times is 2/825~ns~($=2.4\overline{24}$~ps).  See
...
it leads to rather wide logic (say, 48~bits) in the vernier logic.  If
...

{
  \begin{table}
    ...
    \begin{tabular}{l|l|>{\raggedleft\arraybackslash}p{1in}>{$=\,$}l|>{\raggedleft\arraybackslash}p{1in}}
      \bf Name & \bf Baud rate $[/s]$ & \bf Symbol time $[/(2/825~ns)]$ & \bf Factors & \bf Symbol time\par $[/\tau_E]$\\
      \hline
      50G & $50 \cdot 66/64$ G & 8     &$2^3$    & 136\\
...
of $200/672=297.6$~MHz or more conservatively, assuming 64-byte frames
...
programming language.  This simulator allows for much higher bandwidth
...
not originate with us.  We were inspired by the work of David Chu at
Hewlett-Packard in the 1970s, which resulted in the HP~5370 (see
figure~\ref{fig:hp5370a}) Universal Time Interval Counter, capable of
resolving 20-ps intervals using only a 200-MHz (5-ns) clock.\cite{vernier,hppatent}
...
picoseconds, entirely eliminating the error due to uncertainty from
...
\bibitem{nbs}{B.~E.~Blair, ed.  {\it Time and Frequency: Theory and
  ...
  \bibitem{mallan}{D.~W.~Allan and J.~A.~Barnes.  A modified ``Allan variance'' with increased oscillator characterization ability.  {\it Thirty Fifth Annual Frequency Control Symposium.}  IEEE, 1981.}

\bibitem{newton}{I.~Newton.  {\it Philosophi\ae{} Naturalis Principia Mathematica,} Book 1.  London:\ S.~Pepys, Royal Society Press, 1686.}

\bibitem{bresenham}{J.~E.~Bresenham.  Algorithm for computer control of a digital plotter. {\it IBM Systems Journal,} vol.\ 4, issue~1, 1965.}

\bibitem{hardy}{G.~H.~Hardy and E.~M.~Wright.  {\it An Introduction to the Theory of Numbers.}  Oxford:\ Clarendon Press, 1938.}

\bibitem{ethernet}{IEEE Std 802.3-2015.  {\it IEEE Standard for Ethernet.}  Institute for Electrical and Electronics Engineers, 2015.}

\bibitem{ieee1588}{IEEE Std 1588-2008.  {\it IEEE Standard for a Precision Clock Synchronization Protocol for Networked Measurement and Control Systems.}  Institute for Electrical and Electronics Engineers, 2008.}

  \bibitem{hppatent}{D.~C.~Chu.  Double vernier time interval measurement using triggered phase-locked oscillators.  U.S.~Patent~4,164,648.  Published August 14, 1979.}

 ...
 * \ingroup intSwitch
 *
 * \desc            maps logical port number to physical port number. If the
 *                  logical port is a remote port, returns the internal
 *                  port associated with the remote port.
 *
 * \param[in]       sstate points to the switch state entry
 *
 * \param[in]       logPort is the logical port number
 *
 * \param[out]      physPort contains the physical port number
 *
                                     ...
                                     fm_int *   physPort)
{
    ...
    if (!fmIsCardinalPort(sstate->switchNumber, logPort))
    {
        ...
        /* This is a remote logical port, so use the internal port. */
        err = fmGetInternalPortFromRemotePort(sstate->switchNumber,
                                              logPort,
                                              &internalPort);
        if (err != FM_OK)
        {
        ...
        if (fmIsCardinalPort(sstate->switchNumber, internalPort))
        {
            logPort = internalPort;
        }
        else
        {
            /* Internal port is a LAG, need to convert that LAG into one
             * physical port. */
            err = fmGetLAGMemberPorts(sstate->switchNumber,
                                      sstate->portTable[internalPort]->lagIndex,
                                      &numPorts,
                                      portList,
            ...
            {
                /* Select one of the LAG ports using pseudo-random election. */
                logPort = portList[logPort % numPorts];
            }
        }
    }

    return fmPlatformMapLogicalPortToPhysical(sstate->switchNumber,
                                              logPort,
                                              &switchNum,
                                              physPort);

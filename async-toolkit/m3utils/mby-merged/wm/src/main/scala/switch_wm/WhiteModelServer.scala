...
//    + array.toList.map(f => f"$f%x"))
  ...
  def checkoutCsr : Unit = {
    ...
    val a = theCsr.mpp(0).mpt(0).rx_ppe(0).policers(0).POL_CFG(0).POL_CFG(1).CREDIT_FRAME_ERR
    /// rx_ppe is not an array, so it can be referenced through transparently
    val b = theCsr.mpp(0).mpt(0).rx_ppe.policers(0).POL_CFG(0).POL_CFG(1).CREDIT_FRAME_ERR
    ...
    val c : Long = theCsr.mpp.mpt(0).rx_ppe.policers.POL_CFG(0)(1).CREDIT_FRAME_ERR

    val test = theCsr.mpp(0).mpt(0).rx_ppe.policers
    println("theCsr.mpp(0).mpt(0).rx_ppe.policers path is " + test.path)
    theCsr.mpp(0).mpt(0).rx_ppe.cm_apply(0).CM_APPLY_SOFTDROP_STATE(0).USAGE_OVER_LIMIT() = 1
    theCsr.mpp(0).mpt(0).rx_ppe.cm_apply(0).CM_APPLY_SOFTDROP_STATE(0).OVER_LIMIT() = 1
    ...
    val ipv4_h = theCsr.mpp(0).mpt(0).tx_ppe.modify(0).ipv4_hdr
    // or
    val group2 = theCsr.mpp(0).mpt(0).tx_ppe.modify(0).MOD_PROFILE_GROUP(0).get_group(1).assign(4)
    //
    val outputshift = theCsr.mpp(0).mpt(0).tx_ppe.modify(0).MOD_MAP_CFG(1).OUTPUT_SHIFT.reset()

                         ...
                         fm_lock *lck)
{
    ...
    if ( pthread_mutex_init( (pthread_mutex_t *) lck->handle, &attr ) != 0 )
    {
        pterr = pthread_mutexattr_destroy(&attr);
        if (pterr != 0)
        {
    ...
    /* FIXME: what attributes should we use? */
    ...
    FM_LOG_ASSERT(FM_LOG_CAT_ALOS_LOCK,
                  i < FM_ALOS_INTERNAL_MAX_LOCKS,

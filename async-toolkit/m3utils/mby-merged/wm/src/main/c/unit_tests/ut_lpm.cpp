...
#define MEM_BANK_SIZE 512
// This macro intentionally does NOT use parenthesis (..)
#define SPLIT_MEM_IDX(idx) idx / MEM_BANK_SIZE, idx % MEM_BANK_SIZE
...
TEST_F(LpmTests, SubtrieNoHit) {
    ...
    {
        EXPECT_FUNCTION_CALL(mock_getSubtrieStore,
                             (NULL, SPLIT_MEM_IDX(root_ptr), _))
            .Times(1)
            .WillOnce(SetArgPointee<3>(empty_store));
...
TEST_F(LpmTests, SubtrieNoHitRecursive) {
    ...
    for (fm_byte key_len=0; key_len <= 24; ++key_len)
    {
        EXPECT_FUNCTION_CALL(mock_getSubtrieStore,
                             (NULL, SPLIT_MEM_IDX(root_ptr), _))
            .Times(1)
            .WillOnce(SetArgPointee<3>(parent_store));

        if (key_len >= 8)
        {
            // Fetch the child subtrie and storage
            EXPECT_FUNCTION_CALL(mock_getSubtrie,
                                 (NULL, SPLIT_MEM_IDX(child_ptr), _))
                .Times(1)
                .WillOnce(SetArgPointee<3>(child_subtrie));
...
TEST_F(LpmTests, SubtrieMultiHit) {
    ...
    EXPECT_FUNCTION_CALL(mock_getSubtrieStore,
                         (NULL, SPLIT_MEM_IDX(root_ptr), _))
        .Times(1)
        .WillOnce(SetArgPointee<3>(match_store));

// -*- mode:c -*-

// Copyright (C) 2018 Intel Corporation

#ifndef MBY_HASH_H
#define MBY_HASH_H

// Includes:

#include <sys/param.h>   // MIN/MAX C macros
#include <mby_top_map.h> // header file auto-generated from RDL

#include "fm_types.h"
#include "mby_cgrp_types.h"
#include "mby_ma_table.h"
#include "mby_trig_results.h"
#include "mby_cla2hsh.h"
#include "mby_hsh2nxt.h"

// Defines:

#define MBY_FWD_HASHING_CFG_b_USE_METADATA  13
#define MBY_FWD_HASHING_CFG_b_ECMP_ROTATION 12
#define MBY_FWD_HASHING_CFG_l_ROTATION_B    10
#define MBY_FWD_HASHING_CFG_h_ROTATION_B    11
#define MBY_FWD_HASHING_CFG_l_ROTATION_A    8
#define MBY_FWD_HASHING_CFG_h_ROTATION_A    9
#define MBY_FWD_HASHING_CFG_b_USE_VID       7
#define MBY_FWD_HASHING_CFG_b_USE_VPRI      6
#define MBY_FWD_HASHING_CFG_b_USE_TYPE      5
#define MBY_FWD_HASHING_CFG_b_USE_SMAC      4
#define MBY_FWD_HASHING_CFG_b_USE_DMAC      3
#define MBY_FWD_HASHING_CFG_b_SYMMETRIC     2
#define MBY_FWD_HASHING_CFG_b_USE_L34       1
#define MBY_FWD_HASHING_CFG_b_USE_L2_IF_IP  0

#define MBY_MOD_META_BYTES                  entropy_meta_cfg_r_BYTE_DEFAULTS__nd / 2
#define BYTES_IN_DW                         8
#define BITS_IN_BYTE                        8

#define MBY_ENTROPY_HASH_KEY_MASK_DW_NUM    8
#define MBY_ENTROPY_HASH_KEY_MASK_BYTES     MBY_ENTROPY_HASH_KEY_MASK_DW_NUM * BYTES_IN_DW
#define MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR   2
#define MBY_ENTROPY_HASH_SYM_KEY8_WIDTH     5
#define MBY_ENTROPY_HASH_SYM_KEY16_WIDTH    5
#define MBY_ENTROPY_HASH_SYM_KEY32_WIDTH    4
#define MBY_ENTROPY_HASH_SYM_KEY8_NUM       (entropy_hash_sym8_r_KEY_PAIRS__nd  / MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR) / MBY_ENTROPY_HASH_SYM_KEY8_WIDTH
#define MBY_ENTROPY_HASH_SYM_KEY16_NUM      (entropy_hash_sym16_r_KEY_PAIRS__nd / MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR) / MBY_ENTROPY_HASH_SYM_KEY16_WIDTH
#define MBY_ENTROPY_HASH_SYM_KEY32_NUM      (entropy_hash_sym32_r_KEY_PAIRS__nd / MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR) / MBY_ENTROPY_HASH_SYM_KEY32_WIDTH
#define MBY_ENTROPY_HASH_SYM_KEY_NUM_MAX    MAX(MAX(MBY_ENTROPY_HASH_SYM_KEY8_NUM, MBY_ENTROPY_HASH_SYM_KEY16_NUM), MBY_ENTROPY_HASH_SYM_KEY32_NUM)

#define MBY_HASH_ACTION_l_PREC             29
#define MBY_CGRP_ACTION_h_PREC             31
// #define MBY_MOD_META_BITS  MBY_MOD_META_BYTES * 8

// Enums:

// Structs:

typedef struct mbyFwdHashingCfgStruct
{
    fm_bool   ECMP_ROTATION;
    fm_byte   ROTATION_B;
    fm_byte   ROTATION_A;

} mbyFwdHashingCfg;

typedef struct mbyEntropyMetaCfg
{
    fm_byte   BYTE_DEFAULTS[MBY_MOD_META_BYTES]; // 6 x 2b field
    fm_byte   HASH_START;                        // 6b field
    fm_byte   HASH_SIZE;                         // 6b field

} mbyEntropyMetaCfg;

typedef struct mbyEntropyHashSymStruct
{
    fm_byte   KEY8 [MBY_ENTROPY_HASH_SYM_KEY8_NUM] [MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR]; // 6 x 2 x 5b field
    fm_byte   KEY16[MBY_ENTROPY_HASH_SYM_KEY16_NUM][MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR]; // 6 x 2 x 5b field
    fm_byte   KEY32[MBY_ENTROPY_HASH_SYM_KEY32_NUM][MBY_ENTROPY_HASH_SYM_KEYS_IN_PAIR]; // 8 x 2 x 4b field

} mbyEntropyHashSym;

typedef struct mbyEntropyHashKeyMaskStruct
{
    fm_uint64 MASK[MBY_ENTROPY_HASH_KEY_MASK_DW_NUM]; // 64B == 8 x 8B field

} mbyEntropyHashKeyMask;

// Consts:
static const fm_byte ptable[] =
{
    11,  7,  4,  6,  0, 10,  3,  5,
    13, 14,  1,  2, 15, 12,  9,  8,
     3,  6,  7,  5, 11,  2, 10,  0,
    12, 15,  1,  4, 14, 13,  9,  8,
    12,  0,  8, 11, 15,  7, 13,  1,
     9,  4, 14,  5,  6,  2, 10,  3,
     8,  3,  9, 15, 13,  7, 11, 14,
    12,  2,  6,  0,  5, 10,  1,  4,
     5,  1, 13,  7,  9, 10,  0,  6,
     3,  8,  4, 11,  2, 12, 15, 14,
     0, 14, 15,  9,  5, 13, 11,  3,
     4,  6,  2,  1,  7, 12,  8, 10,
     8,  0, 10,  1,  3,  7,  4, 11,
     5, 12,  6, 14, 15, 13,  9,  2,
     6,  2,  5,  7, 12,  8, 15,  1,
    14, 11, 10, 13,  9,  0,  4,  3,
    15,  4,  3,  1, 14, 11, 13,  9,
    12, 10,  0,  5,  6,  8,  7,  2,
    11, 15,  2, 13,  7,  1,  5,  4,
     9,  8,  6,  0, 12,  3, 14, 10,
     1, 13,  4, 14,  8,  3,  5,  9,
     0, 11,  7, 10, 15,  6,  2, 12,
    14,  3,  2, 11,  9, 13,  8,  7,
     0,  1,  5, 12,  4,  6, 10, 15,
     4,  7, 14,  9, 11,  8, 13,  2,
    12, 15, 10,  1,  0,  6,  5,  3,
     9,  1, 13,  7, 15, 12,  0,  2,
     5, 14, 11,  4,  3, 10,  6,  8,
     5,  6, 10, 12, 14,  7,  3, 13,
     8,  4,  1, 11,  9, 15,  2,  0,
     8,  5, 12,  7, 15, 11,  6,  9,
     2,  4, 13,  3,  0, 10,  1, 14,
     7, 14,  2,  6,  8,  0,  5, 15,
    13,  3, 10,  9, 12, 11,  1,  4,
     6, 15, 13,  9,  8,  1, 12,  0,
     3, 11,  2, 10,  7,  4,  5, 14,
     5,  4,  2,  8,  7,  6,  9,  0,
    12,  3, 14, 10, 13, 11,  1, 15,
     3, 11, 15,  5,  0,  1, 14,  9,
     2,  8, 13, 10, 12,  7,  4,  6,
     9,  6, 13, 11,  2,  4,  7,  8,
     5,  1, 10, 15,  3, 14,  0, 12,
     2,  4,  5,  9, 14, 12,  7, 10,
    11,  6,  8,  3,  1, 13, 15,  0,
     7,  3,  6, 10, 15, 13, 12,  1,
     5,  4, 11,  8, 14,  2,  9,  0,
     7,  1, 15,  4,  9, 14,  6,  3,
    10, 12, 13,  8,  2,  0, 11,  5,
     5, 15,  1,  0,  9, 14,  4, 10,
    12,  6,  8,  2, 11,  7, 13,  3,
     6,  5,  0, 11,  8,  1,  7, 10,
     4,  3,  2, 13, 12,  9, 15, 14,
    15,  2,  9,  3,  5,  8,  7,  1,
     4, 12,  6, 14,  0, 11, 13, 10,
    11,  2, 15,  6,  1,  5,  4, 14,
    10,  8,  9, 12, 13,  3,  0,  7,
     6,  2,  5, 12,  9, 14, 13, 10,
     7,  0,  1, 11, 15,  4,  3,  8,
     0,  5,  1,  9,  2,  7, 10, 11,
    13, 12,  4,  8, 15,  3,  6, 14,
     5, 15,  3,  9, 10,  2,  0,  6,
    14, 11,  8,  1, 13,  7,  4, 12,
     1, 10,  9, 14, 13,  2,  8,  5,
    15,  7,  4, 12,  6, 11,  3,  0,
     6,  7, 13, 14,  2,  9, 15,  3,
    10,  1,  0, 12,  5, 11,  4,  8,
    15,  0,  4, 11, 12,  1,  8,  5,
    14,  7, 13,  3,  6, 10,  2,  9,
    14,  5, 13,  9,  1,  7, 11, 15,
     3,  0, 12,  8, 10,  4,  6,  2,
    13,  1, 15, 11,  6,  2,  5,  4,
     7,  3,  9,  0, 12, 10, 14,  8,
    10, 15,  8,  0,  1, 11,  6, 12,
     5, 14,  7,  4,  2,  3, 13,  9,
    11,  8, 10,  2,  3,  0, 14, 13,
     7,  1, 15,  5,  4,  6, 12,  9,
    10,  7,  0,  4,  1, 12,  3,  5,
    15, 14, 13,  9,  2,  6, 11,  8,
    12,  5,  6,  1,  7,  8, 13, 11,
     3, 15,  2, 10,  9,  0,  4, 14,
     1, 13,  6,  2,  4, 14,  9,  3,
     0, 12,  5, 11, 15,  8,  7, 10,
     6,  1, 15,  0, 11,  3,  8, 12,
    13,  5,  4, 14,  9,  7, 10,  2,
    14,  8,  0,  6,  9,  7, 15, 10,
    13,  2,  5,  4,  1, 12,  3, 11,
     0,  8, 11, 13, 14,  4,  6,  3,
     9, 10,  5,  2, 12,  1,  7, 15,
     0, 10, 11,  8, 12, 14,  3, 15,
     6,  7, 13,  1,  5,  2,  9,  4,
     0, 12, 13, 15,  5,  4, 10,  2,
     1,  8,  9,  6, 11, 14,  7,  3,
    15,  5, 13, 12, 11,  6,  4,  8,
     0, 14,  2,  1,  7, 10,  9,  3,
     2,  1,  4, 12,  9, 10,  5,  6,
    11,  3, 15,  8,  0,  7, 13, 14,
    10,  2,  5, 11,  1,  4,  3,  6,
    12, 15, 14,  0,  7, 13,  8,  9,
     9, 13,  5, 12,  0, 11, 14,  6,
     7,  8,  1, 10,  3, 15,  4,  2,
    14,  8,  7,  0,  9,  4, 12,  1,
    13,  3, 11,  2,  6, 10,  5, 15,
    13, 10,  7,  5,  6,  1,  3,  0,
     8, 14,  9,  2, 11,  4, 12, 15,
     5,  4, 11, 10,  3, 13,  9,  1,
    15,  2,  7,  6, 14, 12,  8,  0,
     7, 14,  4,  1, 10,  0, 12, 13,
     6,  2,  8,  5,  9,  3, 15, 11,
     2,  6, 14,  0, 10,  4, 15,  8,
     9,  7,  1, 12,  3,  5, 11, 13,
     1, 12,  7,  2, 13, 10,  0, 15,
     4,  6,  3,  5,  9,  8, 11, 14,
    10, 12, 13,  6,  9, 11, 14,  0,
     7,  1,  4, 15,  2,  8,  5,  3,
     0, 12,  2,  5, 13,  9, 10,  4,
    11,  8,  6,  7, 15,  1, 14,  3,
     1, 14, 15, 10,  7,  0,  9,  3,
    11,  5,  4, 12,  6, 13,  2,  8,
     7,  2, 13,  8, 10,  5, 11,  6,
     1,  4, 14, 12,  0, 15,  9,  3,
     6, 12,  9,  0,  5,  2, 15,  4,
     3,  8, 10,  1, 13, 11, 14,  7,
    13,  9,  3, 15, 14,  7,  6, 11,
     8,  0,  2, 10,  1,  4, 12,  5,
     2,  5, 11,  8,  9,  0, 10, 12,
    15,  1,  7,  3,  4,  6, 14, 13,
    14,  1,  9, 13,  7, 11,  3, 15,
     5, 12,  0,  4,  2,  6, 10,  8,
     1,  3,  5, 10,  4,  9,  6,  7,
    13,  8, 15,  2, 12,  0, 14, 11,
     4,  9,  2,  3,  8,  1,  7,  5,
    14, 13,  6, 10, 11,  0, 15, 12,
     5, 14,  3, 10, 11,  6,  1, 12,
     9,  7,  8,  2, 13,  0, 15,  4,
    11, 14,  9,  8, 12,  5,  6, 13,
    15,  7,  1, 10,  0,  2,  3,  4,
    12,  6,  0,  1, 15,  7, 11, 13,
    14,  9,  2,  3,  5,  8,  4, 10,
     4, 12, 13, 10, 11,  2,  3,  7,
    15,  9, 14,  1,  6,  0,  5,  8,
    15,  6, 13,  4,  3,  5,  9,  8,
    12, 14,  0,  7,  2, 11, 10,  1,
     4, 15,  2,  5, 13,  3,  6, 14,
     9,  1, 11, 10,  7,  8, 12,  0,
     0,  5,  8,  1,  4, 13,  7,  9,
    11, 10, 12,  6,  2,  3, 15, 14,
     4,  6, 10, 15,  7,  2,  1,  3,
     0,  8,  9,  5, 12, 11, 13, 14,
    11, 12, 10,  0,  1,  6, 13, 14,
     3,  7,  4,  8, 15,  5,  2,  9,
     4, 13,  3,  2,  9, 10,  6,  0,
     7, 14,  5,  1, 15,  8, 12, 11,
    14, 13,  0,  9,  3,  1,  7,  5,
     6,  2,  8, 10, 11,  4, 12, 15,
     8,  3,  4,  0, 11,  2, 15,  5,
     9, 14,  6, 12,  7, 13, 10,  1,
     6,  1,  7,  2,  5, 15,  4, 10,
     0, 14, 12, 11,  3, 13,  8,  9,
    14,  1,  6, 10, 13,  5, 11,  0,
    15,  8,  4, 12,  3,  2,  9,  7,
    10, 13,  1, 14, 11, 15,  3,  5,
     7,  8,  4, 12,  0,  9,  2,  6,
     7,  6,  8, 13,  3,  1, 12,  9,
     0,  2,  5,  4, 14, 10, 11, 15,
     1,  4,  6,  2,  5,  0,  7,  9,
     3, 11, 10,  8, 12, 15, 13, 14,
     9,  4,  8,  3, 12, 13,  1,  5,
     2,  7,  0, 10, 11, 15,  6, 14,
     2,  9,  4,  3,  6, 14,  0, 10,
     5,  8, 15, 12,  7, 11,  1, 13,
     6,  5, 15, 11,  8, 10,  3,  9,
     1, 12,  0,  2, 14,  4,  7, 13,
     2,  9,  4, 12, 14,  3,  1, 10,
     6, 11,  0, 15,  7,  5,  8, 13,
     9, 11,  3,  1,  0,  7, 10, 14,
    12, 13,  2,  8,  4, 15,  5,  6,
    12,  9,  7,  1,  6, 14, 10,  2,
    11, 13,  4,  0,  8,  5,  3, 15,
     7,  2,  5,  4,  9, 10,  1, 12,
    11,  8, 14, 15,  0,  6,  3, 13,
    12,  9, 14, 15,  3,  7,  4, 10,
     5,  1,  6, 13,  0,  2, 11,  8,
    15,  6,  2, 11,  7, 14, 12,  3,
     8,  9,  5, 13,  1, 10,  0,  4,
    10,  7,  0,  8, 11,  9, 15, 13,
    14,  5, 12,  1,  6,  3,  4,  2,
     0,  7, 14,  3, 11,  9, 10, 13,
     8,  2, 15,  6,  5,  4, 12,  1,
     9,  8,  2, 15, 11,  7,  3, 13,
     5,  4,  0, 10,  1, 12, 14,  6,
     5, 13,  0,  8,  7,  9,  4,  1,
     6,  3, 12, 14,  2, 15, 11, 10,
     1,  3,  5,  6, 12,  8,  9,  4,
     2, 10,  0, 14, 13, 11, 15,  7,
     0,  5, 15,  8,  3, 12, 11, 14,
     7,  4,  2,  9, 13, 10,  6,  1,
     6, 11, 10,  0, 15,  2,  1,  9,
     7,  4, 12, 13,  8,  3, 14,  5,
    11,  5, 12,  1,  4, 15,  6, 10,
    14,  3, 13,  7,  2,  9,  8,  0,
    11,  2,  1, 10,  7,  3, 12,  5,
     4,  0,  8, 13, 14,  6,  9, 15,
     9,  1, 11, 15, 12,  0, 10,  6,
     4,  3,  2,  8, 13,  7,  5, 14,
    11, 10,  2,  0,  3, 12,  8,  7,
     4,  9,  6,  1, 13, 14, 15,  5,
    12,  4, 10, 13,  2,  6,  7, 14,
    11,  3, 15,  1,  0,  8,  5,  9,
     0, 13,  5,  6,  1, 15,  3,  9,
    12,  4, 14,  2, 11, 10,  8,  7,
    13,  1,  6,  5,  9, 12, 10,  3,
    11, 15,  7,  4,  0,  8, 14,  2,
    11, 12,  3,  9, 14, 13,  4, 15,
     8,  7,  1, 10,  2,  5,  6,  0,
     9, 15,  4, 12, 14,  1,  6, 11,
     7,  5,  2, 10, 13,  3,  8,  0,
    10, 13,  6,  1,  5,  2, 12,  0,
     9,  4,  8, 11, 14, 15,  7,  3,
    12, 14,  7,  1,  6,  8, 11,  4,
     9, 13,  5, 10,  3,  2,  0, 15,
    14, 11,  6,  0, 13,  5,  8,  2,
     7,  9, 12, 15,  4,  3, 10,  1,
     4, 14,  6,  3,  1,  2,  5, 13,
    10,  9, 11, 15,  0,  7, 12,  8,
     3,  7,  0, 15,  2,  8,  9, 10,
    11, 14,  6,  1,  4,  5, 13, 12,
     4,  0, 11,  8,  2,  5,  1, 13,
    14,  7,  3, 10,  9, 15, 12,  6,
     4, 12, 14,  1,  2, 11,  0,  5,
    10,  7,  8, 15,  9, 13,  3,  6,
     2,  9,  4,  5, 10,  8, 14,  3,
     7, 13,  1,  6, 12,  0, 11, 15,
    15,  2, 10,  6,  9,  4, 14,  7,
     5, 13,  0, 11,  8,  1,  3, 12,
    10,  3,  4, 11, 13,  2, 15,  6,
     7,  0,  8, 14, 12,  5,  1,  9,
     9,  4, 15, 13, 11, 12,  6,  7,
    14,  1,  5,  2,  0, 10,  3,  8,
    12, 15,  2,  1, 13,  8,  9,  6,
     3,  5,  4, 14,  0, 10, 11,  7,
     6,  9,  3,  0,  8,  7, 11,  5,
     2, 13, 15, 10, 14, 12,  4,  1,
    13, 10,  3, 11,  8,  7,  0,  2,
     4, 15, 14,  6,  9,  5, 12,  1,
    15, 11,  4,  1, 13,  6,  5,  3,
     7,  2, 12,  0,  8,  9, 10, 14,
     4,  8, 13,  9, 10, 14, 11,  3,
     7,  1,  2,  0, 15,  6, 12,  5,
     1,  0,  5,  4, 10, 15,  8, 13,
    12,  9,  3,  6,  7,  2, 14, 11,
     5,  0,  1,  6, 14, 15, 11,  9,
     3, 10, 12,  8, 13,  4,  7,  2,
    10,  8,  2, 13, 12, 14,  1,  3,
     9, 15,  0,  4,  5,  7, 11,  6,
     3,  4,  8,  1, 12,  7, 14,  5,
    11,  2, 10, 13,  0, 15,  9,  6,
    10,  5,  3,  7, 14,  0,  2,  9,
    15,  4, 11,  8,  6, 12,  1, 13,
     1, 13, 15,  8, 14,  2,  9,  7,
     5,  6, 10,  3,  0,  4, 11, 12,
    15,  0,  3, 11,  9,  5,  1, 12,
     2, 13,  6,  8,  7, 10, 14,  4,
    14,  7,  4, 15, 11, 13,  0,  1,
     5,  3, 10, 12,  6,  9,  8,  2,
     2,  0,  1, 13,  9,  4,  6,  7,
    15, 10, 11,  8,  3, 14,  5, 12,
     9,  8, 14,  4,  7, 15,  2,  3,
     0,  5, 11, 10,  1,  6, 13, 12,
     9, 14,  5,  2,  7,  3,  6, 10,
     1, 13,  8, 15,  4, 12, 11,  0,
    11,  8,  9,  2,  0,  4, 13, 14,
     5,  6,  7, 15, 12, 10,  3,  1,
     3, 10, 12,  1, 13,  4, 15,  2,
     8, 11,  0,  7, 14,  6,  5,  9,
    11,  0, 13,  1,  9, 10,  2, 12,
    15,  6, 14,  3,  8,  7,  4,  5,
    12,  7, 14, 15,  2,  3, 11,  1,
     0,  4, 10,  5,  8, 13,  9,  6,
     9, 10,  5, 13,  6,  0,  4, 12,
     1,  7,  8, 15,  3, 14,  2, 11,
     5,  9,  0,  2,  8,  4, 13,  6,
    10,  7, 11,  1, 12, 14,  3, 15,
     0,  7, 15,  2, 10,  3, 14,  6,
    12, 11,  1,  9,  8,  5,  4, 13,
     5, 13,  9,  0,  1,  8,  2,  7,
    11, 14, 10,  6, 15,  3,  4, 12,
     6, 10,  5,  3,  9, 11, 14,  2,
     1,  8, 15,  0, 13,  4,  7, 12,
     2,  4, 10,  6, 12,  3, 11, 15,
     9, 14, 13,  7,  0,  8,  1,  5,
     3,  4,  9,  5, 10, 13, 14,  8,
     2, 11, 15,  6, 12,  0,  1,  7,
     6, 13, 12, 10,  7,  0,  4,  8,
     3,  2,  9,  1, 14,  5, 15, 11,
     5, 13,  1,  3,  8,  2, 11,  0,
    14, 15,  6, 12,  7,  9,  4, 10,
     5,  1,  0, 15,  9, 10, 13,  8,
     6,  7, 14,  2, 12, 11,  4,  3,
    12,  0,  9, 14,  6,  8,  3, 10,
    15,  2, 11,  1, 13,  4,  5,  7,
    14,  3, 12, 10, 13,  5, 15,  6,
     4,  1,  0,  9, 11,  2,  8,  7,
    12,  8, 11,  7,  9,  4, 10,  0,
     2,  5, 14,  6, 15,  1,  3, 13,
     8, 14, 10,  2,  3, 12,  0,  1,
    13,  7,  9,  4,  5,  6, 15, 11,
    13,  0,  2, 15, 14,  5, 10, 11,
     1,  9, 12,  6,  8,  4,  3,  7,
     7,  4, 11,  1, 10,  2,  3,  9,
    15, 14,  5,  6, 13,  0,  8, 12,
     4,  0,  3, 13, 10, 11,  7, 12,
     6,  9, 14, 15,  1,  2,  8,  5,
     7, 13,  5, 15,  3,  9, 12,  4,
    14,  2,  6,  8, 10, 11,  1,  0,
     5,  7, 14,  0, 11, 15, 12,  3,
     6, 13, 10,  2,  1,  4,  8,  9,
     3, 14,  1,  0, 12,  7,  2,  9,
    15, 11,  6, 13,  4,  8, 10,  5,
     3,  4,  8, 15, 14, 12,  6,  5,
    10,  2,  1,  7, 13,  0,  9, 11,
     2,  6, 14,  5,  7,  4,  3,  1,
    11,  9, 13, 10, 15,  8, 12,  0,
     7,  4,  3, 11,  0,  9, 12, 14,
     2,  6,  8, 10, 15,  1,  5, 13,
     0,  7, 11,  6,  1,  4, 15,  5,
     3, 13, 14, 10,  2, 12,  9,  8,
    11, 15,  6,  1,  4,  5, 13,  9,
    12, 14,  0,  7,  3, 10,  8,  2,
    10, 15,  8,  1, 14,  4, 11,  0,
     5, 12,  3, 13,  6,  2,  7,  9,
    15,  7, 13,  2,  0,  3,  9, 14,
     8, 12, 11,  4,  6,  1, 10,  5,
     4,  6, 13,  1,  8, 14, 11,  9,
     0,  2, 15, 12,  3, 10,  7,  5,
    11, 12, 10,  0,  5, 14, 15,  1,
     4,  9,  7, 13,  2,  3,  8,  6,
     4,  3, 15,  6,  8,  2, 12,  9,
     5, 14, 11,  0, 10,  7,  1, 13,
    10,  8,  0,  5, 12, 14,  3,  9,
    11,  4, 13,  2,  7,  6,  1, 15,
    10,  9, 12,  8, 11,  4,  3, 14,
     2,  0,  1,  6,  5, 15, 13,  7,
     1,  6,  7, 10,  8,  0,  3,  5,
    11, 13, 15,  2,  4, 14,  9, 12,
     3,  7, 15,  1,  5,  8,  9, 12,
    13, 11,  4, 14,  2,  0,  6, 10,
     7,  5, 15, 14, 13,  0,  6, 12,
     3,  1,  8,  2,  9,  4, 10, 11,
    13, 15, 14,  1,  0,  9,  4,  7,
     6, 11, 12,  2,  5,  8,  3, 10,
    12,  7,  9,  1, 15,  0,  6, 13,
     5,  4,  2, 11, 10,  3, 14,  8,
     2,  0,  9,  1, 10,  7, 14, 11,
     4,  6, 15, 13, 12,  5,  3,  8,
     4, 11,  1,  2, 15,  7, 10, 14,
    13,  5, 12,  8,  3,  6,  9,  0,
    12,  6, 13,  7,  5, 11,  4,  2,
    15,  0,  3,  9,  1,  8, 10, 14,
     9,  1, 14,  6, 13,  5, 12,  4,
     7, 10,  2, 15,  8, 11,  3,  0,
     7,  3, 15, 10,  1,  5, 13, 11,
     4,  9,  0, 12, 14,  6,  2,  8,
    14,  7,  6, 11,  0,  9,  2, 13,
    15,  5, 10,  4,  1,  3,  8, 12,
     9, 15, 13, 14,  7,  3,  8,  4,
     1, 10, 11,  6,  5,  0,  2, 12,
    11, 12, 14,  4, 13, 15,  6,  5,
     0,  8,  2,  3,  1, 10,  7,  9,
    14,  1, 13,  3,  5,  0, 10, 15,
    12,  2,  6,  9,  4,  7, 11,  8,
    13,  0,  3,  7,  2,  8,  9, 15,
    12, 10, 14,  1,  4,  6,  5, 11,
     4,  5, 15,  7,  3,  6, 10, 11,
     2,  9, 14,  1, 13, 12,  0,  8,
     6,  3,  5, 13,  7, 10, 14,  2,
     0,  4, 12,  1,  8,  9, 15, 11,
    15, 12, 13, 10, 11,  2,  8,  1,
     4, 14,  7,  0,  3,  6,  5,  9,
    10,  9, 11,  4, 12,  0,  3,  1,
     6, 14,  8,  7, 13,  5,  2, 15,
     5,  3,  8,  9,  6, 11,  4, 15,
    13,  7, 14,  2,  1, 12,  0, 10,
     5,  9, 12, 15,  3,  0,  4, 10,
    14,  1,  6,  2,  7, 13,  8, 11,
     8, 15, 11,  4, 14, 12,  9,  3,
     2,  5,  0,  1,  7, 10, 13,  6,
     3,  0, 14,  6, 13,  2, 15, 12,
     9,  7, 11,  4,  5, 10,  8,  1,
     6, 10, 15, 12,  1,  0, 13,  7,
     2,  4,  8,  5,  9, 14, 11,  3,
    15,  6,  2,  5, 13, 11,  7,  4,
     0,  8,  9,  1,  3, 14, 12, 10,
     3,  1,  6,  9,  7,  4, 13, 11,
     8, 14,  5,  2, 12,  0, 10, 15,
     4,  9, 10, 11,  5,  8,  3,  7,
    13, 15,  0,  6,  2, 14, 12,  1,
     0,  5,  9, 12,  1, 10, 14,  6,
     8, 13, 15,  3,  7, 11,  2,  4,
     5, 15,  4, 11,  0,  1,  2,  3,
     8, 10, 14, 13,  6,  9,  7, 12,
     0,  3,  6, 11,  8,  9, 12,  1,
     4, 13,  2, 14,  5, 15,  7, 10,
     9,  2, 10,  7, 14,  4, 13, 11,
     0,  6, 15, 12,  5,  1,  8,  3,
     8, 12,  9, 10,  0,  4,  3, 14,
    11,  1,  2,  5, 15,  7, 13,  6,
     1,  9,  4, 11,  0,  8,  2, 15,
     6, 14, 10,  7,  3, 12, 13,  5,
     5,  0,  7,  3, 14, 11,  1,  9,
     2,  6, 13, 12,  4, 15, 10,  8,
     7,  4,  0,  8,  9,  2, 11, 14,
    13, 12, 10, 15,  5,  6,  3,  1,
     3,  6, 14,  4, 11, 13,  8, 15,
     0,  9,  7,  2, 12,  5, 10,  1,
     8, 10, 11,  7,  4, 14,  9, 12,
     5,  0,  3, 13,  1,  2,  6, 15,
     3,  5, 15,  0,  8,  2,  9, 10,
     1,  6,  4, 11, 13, 12, 14,  7,
    15,  1,  6,  7,  8, 14,  4, 10,
    12,  9, 13,  0,  3, 11,  5,  2,
    12,  5,  4, 15, 13, 10,  0,  9,
     2, 14,  8, 11,  3,  6,  1,  7,
     4,  5, 14,  1,  2, 13,  8,  3,
    12, 11,  9, 10,  6,  0,  7, 15,
    14, 11, 10,  3, 13,  6,  9,  5,
    15,  0,  7, 12,  4,  2,  8,  1,
    11, 14, 15,  9,  3,  7,  6, 10,
     5,  4,  2, 12,  8, 13,  0,  1,
     3,  1, 10,  6, 14, 12,  9,  7,
     0, 15,  8,  5, 11,  4, 13,  2,
    10, 15,  4,  3,  9, 13,  6,  1,
     2,  8, 11, 12,  0,  7,  5, 14,
     8,  9,  7,  6,  2, 11, 14,  4,
    12, 13,  0,  5,  1,  3, 10, 15,
     1,  6,  8, 14,  9, 15, 12, 13,
     2,  5,  4, 11,  7,  0,  3, 10,
     8, 10,  5,  6, 12, 11,  4,  0,
    13,  7, 15,  1,  9,  3, 14,  2,
     5,  1, 14,  9,  6, 10, 11,  2,
     7, 15, 13,  0, 12,  8,  3,  4,
    14,  7, 12,  5,  3, 10,  8, 15,
     2, 11,  4,  1,  6, 13,  9,  0,
    12,  0,  9,  2,  4,  8, 11,  5,
    14, 10,  7,  1,  3,  6, 15, 13,
    10,  8,  2,  5,  6, 13,  4,  0,
    14, 12,  7,  3, 15,  9, 11,  1,
     4, 14,  7,  1, 11,  2,  5,  3,
    10,  0, 15, 13,  6, 12,  9,  8,
    12,  7, 11,  8,  4, 13,  1,  0,
     6, 15, 10,  3,  5,  2, 14,  9,
     9,  4, 11, 13,  3, 14, 10,  8,
    12,  5,  7,  6,  1,  0,  2, 15,
    15, 10, 12,  9,  5,  8,  0,  6,
     2, 14,  1,  7,  4, 11,  3, 13,
    13, 14,  7,  5,  8,  9,  4, 15,
    12,  6,  0, 11, 10,  1,  2,  3,
     2,  3, 13,  4,  5,  1, 11, 12,
     9,  6, 15, 10,  8,  7, 14,  0,
    14,  8,  3,  6, 10, 13,  2, 15,
     0,  4,  7,  9, 12, 11,  5,  1,
     5,  4,  2,  1,  0,  3,  7, 13,
    14,  8, 11, 10,  6, 15,  9, 12,
    14,  7,  1,  3, 13, 12,  6,  8,
     0,  9,  4, 11,  5,  2, 10, 15,
    10,  1, 15,  5,  0, 13,  7,  2,
     6,  9,  8, 12,  4,  3, 11, 14,
     7,  4, 15,  2, 13, 10,  3, 12,
     9, 14,  0,  8,  5, 11,  1,  6,
    15,  7, 11,  6,  4,  0, 14,  2,
    10,  8,  9,  5,  3, 12,  1, 13,
     0,  2, 15,  7, 11,  1,  9, 10,
     5, 14,  6, 12,  8,  3,  4, 13,
     5,  7,  1, 13, 12,  9,  4, 11,
     6, 15, 10,  8,  0, 14,  3,  2,
     6,  1, 14,  3,  8, 10,  4,  5,
     9, 12, 15, 11,  7,  2,  0, 13,
     5,  9,  6,  4,  1, 10,  7,  3,
    12,  0, 15, 13, 14, 11,  2,  8,
    14,  7,  9,  8, 10,  2, 13,  5,
     1, 15, 12,  6,  0, 11,  4,  3,
    10,  0,  8, 11,  3, 15,  1, 14,
     6,  2,  9,  7,  4, 12, 13,  5,
     9,  7,  1, 15,  2,  6, 14,  8,
     5,  3,  0, 12,  4, 11, 13, 10,
     2,  8,  6,  3, 15,  7, 12,  4,
    11, 10,  5,  1, 14, 13,  9,  0,
     8, 10,  1,  6, 13, 11,  2, 14,
     9,  7, 15,  3,  5,  0, 12,  4,
     6,  5, 11,  0,  8,  1, 13,  2,
     4,  7, 15, 10, 12,  3, 14,  9,
    12, 11,  1,  0,  2,  8,  3,  4,
     6,  7, 15,  5, 14,  9, 10, 13,
     1, 15,  4,  5,  8, 13, 11, 12,
     6,  0,  3,  9,  7,  2, 10, 14,
    10,  5,  9,  6, 11,  0, 15, 13,
     3, 12,  7,  2, 14,  1,  8,  4,
    10,  2,  7, 14,  5,  3,  8, 11,
    13,  0,  6,  1,  9, 12, 15,  4,
    13,  6,  8,  5,  9, 14, 11,  7,
    12,  3, 15,  1, 10,  4,  0,  2,
     6,  9,  1, 15, 10,  2, 13,  7,
     4, 11,  0, 12,  8,  5,  3, 14,
    15, 13, 12,  2, 11, 10,  8,  7,
     0,  3,  5,  4,  1, 14,  9,  6,
     3,  9,  4, 13,  1,  0,  2, 11,
    10,  8,  7,  5, 15,  6, 12, 14,
     9, 10,  6,  2,  3,  4, 11, 14,
     8, 15,  0,  1,  7, 13,  5, 12,
     4,  3,  8,  7,  9, 10,  6, 15,
     0, 13, 12,  1,  5, 14,  2, 11,
     9,  8, 14,  1,  4, 15, 13,  5,
     6,  7,  2,  3,  0, 10, 12, 11
};

// Function prototype:
void Hash
(
    mby_ppe_entropy_map const * const entropy_map,
    mbyClassifierToHash const * const in,
    mbyHashToNextHop          * const out
);

#endif // MBY_HASH_H

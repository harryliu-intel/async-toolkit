...
int BSIM4polyDepletion(double phi, double ngate,double coxe,
                       double Vgs, double *Vgs_eff, double *dVgs_eff_dVg);

/*** Evaluate a transistor, including diodes and various nonlinear resistors ***/
int BSIM4_Evaluate(BSIM4model *model, BSIM4instance *here, CKTcircuit *ckt,
                   double Q[6], double I[6], double dQ[36], double dI[36]) {

int nS,nD,nG,nB,nSP,nDP,i;

double dgstot_dvd, dgstot_dvg, dgstot_dvs, dgstot_dvb;
double dgdtot_dvd, dgdtot_dvg, dgdtot_dvs, dgdtot_dvb;
double Rs, Rd, gspr, gdpr, ceqgstot, ceqgdtot;
...
double vges, vgms, vged, vgmd, vgmb;
double gcgmgmb, gcgmdb, gcgmsb, gcdgmb, gcsgmb;
double gcgmbb, gcbgmb, qgmb, qgmid=0;

double vbd, vbs, vds, vgb, vgd, vgs;
double vdbs, vdbd, vsbs;
double vbs_jct, vbd_jct;

double SourceSatCurrent, DrainSatCurrent;
double ag0, qgb, VgstNVt, ExpVgst;
double ceq, geq;
double cdrain;
...
double qgate=0, qbulk=0, qdrn=0, qsrc=0;
double Vdb, Vds, Vgs, Vbs;
double Igidl, Ggidld, Ggidlg, Ggidlb;
double Voxacc=0, dVoxacc_dVg=0, dVoxacc_dVb=0;
double Voxdepinv=0, dVoxdepinv_dVg=0, dVoxdepinv_dVd=0, dVoxdepinv_dVb=0;
double VxNVt=0, ExpVxNVt, Vaux=0, dVaux_dVg=0, dVaux_dVd=0, dVaux_dVb=0;
...
double Vgs_eff, Vfb=0, Vth_NarrowW;
...
int error, Check, Check1, Check2;

/* unused SPICE data structures */
double CKTgmin =0;
...
/* compute voltage differences */
	  ...
	  {   switch(model->BSIM4dioMod)
		      ...
		      here->BSIM4gbs = SourceSatCurrent * (evbs + T1) / Nvtms + CKTgmin;
			  ...
			  here->BSIM4gbs = devbs_dvb * T2 + T1 * here->BSIM4SslpRev + CKTgmin;
                          here->BSIM4cbs = T1 * T2 + CKTgmin * vbs_jct;
                      }         
                      else if (vbs_jct <= here->BSIM4vjsmFwd)
                      {   T0 = vbs_jct / Nvtms;
                          if (T0 < -EXP_THRESHOLD)
		      ...
		      {   here->BSIM4gbs = here->BSIM4SslpFwd + CKTgmin;
                          here->BSIM4cbs = here->BSIM4IVjsmFwd + here->BSIM4SslpFwd * (vbs_jct
					 - here->BSIM4vjsmFwd) + CKTgmin * vbs_jct;
		      }
                      break;
                  default: break;
              }
	  ...
	  {   here->BSIM4gbd = CKTgmin;
          ...
          {   switch(model->BSIM4dioMod)
              {   case 0:
                      ...
                      /* WDLiu: Magic T1 in this form; different from BSIM4 beta. */
                      here->BSIM4gbd = DrainSatCurrent * (evbd + T1) / Nvtmd + CKTgmin;
		      ...
		      T2 = vbd_jct / Nvtmd;
                  ...
                  case 2:
                      ...
                      {   T0 = vbd_jct / Nvtmd;
                          ...
                          here->BSIM4gbd = devbd_dvb * T2 + T1 * here->BSIM4DslpRev + CKTgmin;
                          here->BSIM4cbd = T1 * T2 + CKTgmin * vbd_jct;
                      }
                      else if (vbd_jct <= here->BSIM4vjdmFwd)
                      {   T0 = vbd_jct / Nvtmd;
                          if (T0 < -EXP_THRESHOLD)
                      ...
                      {   here->BSIM4gbd = here->BSIM4DslpFwd + CKTgmin;
                          here->BSIM4cbd = here->BSIM4IVjdmFwd + here->BSIM4DslpFwd * (vbd_jct
                                         - here->BSIM4vjdmFwd) + CKTgmin * vbd_jct;
                      }
                      break;
                  default: break;
              }
	...
	/* JX: Correction to forward body bias  */
	  ...
	  {   T4 = 1.0 / (3.0 + 8.0 * T0);
          ...
          TempRatio =  model->temp / model->BSIM4tnom - 1.0;
...
finished: 
  ...
  /*** check if SP/DP were swapped ***/
  nS=0; nD=1; nG=2; nB=3;
  if (here->BSIM4mode > 0) { nSP=here->BSIM4sNodePrime-1; nDP=here->BSIM4dNodePrime-1; }
  else                     { nSP=here->BSIM4dNodePrime-1; nDP=here->BSIM4sNodePrime-1; }

  /*** clear vectors and matrices ***/
  for (i=0; i<6; i++)  I[i]=Q[i]=0;
  for (i=0; i<36; i++) dI[i]=dQ[i]=0;
  
  /*** diode current from B to SP ***/
  I[nSP]        += here->BSIM4cbs;   I[nB]         -= here->BSIM4cbs;
  dI[6*nSP+nB]  += here->BSIM4gbs;   dI[6*nB+nB]   -= here->BSIM4gbs;

  /*** diode current from B to DP ***/
  I[nDP]        += here->BSIM4cbd;   I[nB]         -= here->BSIM4cbd;
  dI[6*nDP+nDP] -= here->BSIM4gbd;   dI[6*nB+nDP]  += here->BSIM4gbd;
  dI[6*nDP+nB]  += here->BSIM4gbd;   dI[6*nB+nB]   -= here->BSIM4gbd;

  /*** channel current from DP to SP ***/
  I[nSP]        += here->BSIM4cd;    I[nDP]        -= here->BSIM4cd;
  dI[6*nSP+nDP] += here->BSIM4gds;   dI[6*nDP+nDP] -= here->BSIM4gds;
  dI[6*nSP+nG]  += here->BSIM4gm;    dI[6*nDP+nG]  -= here->BSIM4gm;
  dI[6*nSP+nB]  += here->BSIM4gmbs;  dI[6*nDP+nB]  -= here->BSIM4gmbs;

  /*** substrate current from DP to B ***/
  I[nB]        += here->BSIM4csub;   I[nDP]        -= here->BSIM4csub;
  dI[6*nB+nDP] += here->BSIM4gbds;   dI[6*nDP+nDP] -= here->BSIM4gbds;
  dI[6*nB+nG]  += here->BSIM4gbgs;   dI[6*nDP+nG]  -= here->BSIM4gbgs;
  dI[6*nB+nB]  += here->BSIM4gbbs;   dI[6*nDP+nB]  -= here->BSIM4gbbs;

  /*** GIDL substrate current from DP to B ***/
  I[nB]        += here->BSIM4Igidl;  I[nDP]        -= here->BSIM4Igidl;
  dI[6*nB+nDP] += here->BSIM4ggidld; dI[6*nDP+nDP] -= here->BSIM4ggidld;
  dI[6*nB+nG]  += here->BSIM4ggidlg; dI[6*nDP+nG]  -= here->BSIM4ggidlg;
  dI[6*nB+nB]  += here->BSIM4ggidlb; dI[6*nDP+nB]  -= here->BSIM4ggidlb;

  /*** GIDL substrate current from SP to B ***/
  I[nB]        += here->BSIM4Igisl;  I[nSP]        -= here->BSIM4Igisl;
  dI[6*nB+nDP] += here->BSIM4ggisld; dI[6*nSP+nDP] -= here->BSIM4ggisld;
  dI[6*nB+nG]  += here->BSIM4ggislg; dI[6*nSP+nG]  -= here->BSIM4ggislg;
  dI[6*nB+nB]  += here->BSIM4ggislb; dI[6*nSP+nB]  -= here->BSIM4ggislb;

  if (model->BSIM4igcMod) {

    /*** gate tunneling current from G to D directly ***/
    I[nDP]        += here->BSIM4Igd;    I[nG]        -= here->BSIM4Igd;
    dI[6*nDP+nDP] += here->BSIM4gIgdd;  dI[6*nG+nDP] -= here->BSIM4gIgdd;
    dI[6*nDP+nG]  += here->BSIM4gIgdg;  dI[6*nG+nG]  -= here->BSIM4gIgdg;
    
    /*** gate tunneling current from G to S directly ***/
    I[nSP]        += here->BSIM4Igs;    I[nG]        -= here->BSIM4Igs;
    dI[6*nSP+nG]  += here->BSIM4gIgsg;  dI[6*nG+nG]  -= here->BSIM4gIgsg;

    /*** gate tunneling current from G to D via channel ***/
    I[nDP]        += here->BSIM4Igcd;   I[nG]        -= here->BSIM4Igcd;
    dI[6*nDP+nDP] += here->BSIM4gIgcdd; dI[6*nG+nDP] -= here->BSIM4gIgcdd;
    dI[6*nDP+nG]  += here->BSIM4gIgcdg; dI[6*nG+nG]  -= here->BSIM4gIgcdg;
    dI[6*nDP+nB]  += here->BSIM4gIgcdb; dI[6*nG+nB]  -= here->BSIM4gIgcdb;
    
    /*** gate tunneling current from G to S via channel ***/
    I[nSP]        += here->BSIM4Igcs;   I[nG]        -= here->BSIM4Igcs;
    dI[6*nSP+nDP] += here->BSIM4gIgcsd; dI[6*nG+nDP] -= here->BSIM4gIgcsd;
    dI[6*nSP+nG]  += here->BSIM4gIgcsg; dI[6*nG+nG]  -= here->BSIM4gIgcsg;
    dI[6*nSP+nB]  += here->BSIM4gIgcsb; dI[6*nG+nB]  -= here->BSIM4gIgcsb;
  }

  if (model->BSIM4igbMod) {

    /*** gate tunneling current from G to B ***/
    I[nB]        += here->BSIM4Igb;    I[nG]        -= here->BSIM4Igb;
    dI[6*nB+nDP] += here->BSIM4gIgbd;  dI[6*nG+nDP] -= here->BSIM4gIgbd;
    dI[6*nB+nG]  += here->BSIM4gIgbg;  dI[6*nG+nG]  -= here->BSIM4gIgbg;
    dI[6*nB+nB]  += here->BSIM4gIgbb;  dI[6*nG+nB]  -= here->BSIM4gIgbb;
  }
    
  /*** compute dI[6*i+nSP] since all voltages are relative to Vs ***/
  dI[6*nSP+nSP] = -(dI[6*nSP+nDP] + dI[6*nSP+nG] + dI[6*nSP+nB]);
  dI[6*nDP+nSP] = -(dI[6*nDP+nDP] + dI[6*nDP+nG] + dI[6*nDP+nB]);
  dI[6*nG+nSP]  = -(dI[6*nG+nDP]  + dI[6*nG+nG]  + dI[6*nG+nB]);
  dI[6*nB+nSP]  = -(dI[6*nB+nDP]  + dI[6*nB+nG]  + dI[6*nB+nB]);

  /*** remaining calculations don't swap SP/DP ***/
  nSP=here->BSIM4sNodePrime-1; nDP=here->BSIM4dNodePrime-1;

  /*** source/drain resistors ***/
  if (model->BSIM4rdsMod == 1) {
    // this assumes no dependence on Vg and Vb, which seems true for tsmc65gp
  ...
  ceqgstot = -vses*gspr;      // current from sNodePrime to sNode
  ceqgdtot = (vdes-vds)*gdpr; // current from dNode to dNodePrime

  // printf("ceqgstot=%g ceqgdtot=%g gspr=%g gdpr=%g\n", ceqgstot,ceqgdtot,gspr,gdpr);

  I[nS]        += ceqgstot; I[nSP]        -= ceqgstot;
  dI[6*nS+nS]  -= gspr;     dI[6*nSP+nS]  += gspr;
  dI[6*nS+nSP] += gspr;     dI[6*nSP+nSP] -= gspr;

  I[nD]        -= ceqgdtot; I[nDP]        += ceqgdtot;
  dI[6*nD+nD]  -= gdpr;     dI[6*nDP+nD]  += gdpr;
  dI[6*nD+nDP] += gdpr;     dI[6*nDP+nDP] -= gdpr;

#if 0
  /* This gives 0 currents.  I can't figure out why. */
  ...
  if (model->BSIM4rdsMod == 1) {
    ...
    ceqgstot = (here->BSIM4gstotd * vds + here->BSIM4gstotg * vgs + here->BSIM4gstotb * vbs);
    ...
    ceqgdtot = -(here->BSIM4gdtotd * vds + here->BSIM4gdtotg * vgs + here->BSIM4gdtotb * vbs);
    ...
    /** Debugging **/
    printf("ceqgstot=%g ceqgdtot=%g Is=%g Id=%g\n",ceqgstot,ceqgdtot,Is,Id);
    printf("gstot=%g gstots=%g gstotd=%g gstotg=%g gstotb=%g\n",
           gstot,gstots,gstotd,gstotg,gstotb);
    printf("gdtot=%g gdtots=%g gdtotd=%g gdtotg=%g gdtotb=%g\n",
           gdtot,gdtots,gdtotd,gdtotg,gdtotb);

    /** current from SP to S **/
    I[nS]        += ceqgstot; I[nSP]        -= ceqgstot;
    dI[6*nS+nSP] -= gstots;   dI[6*nSP+nSP] += gstots;
    dI[6*nS+nDP] -= gstotd;   dI[6*nSP+nDP] += gstotd;
    dI[6*nS+nG]  -= gstotg;   dI[6*nSP+nG]  += gstotg;
    dI[6*nS+nB]  -= gstotb;   dI[6*nSP+nB]  += gstotb;
    dI[6*nS+nS]  -= gstot;    dI[6*nSP+nS]  += gstot;

    /** current from D to DP **/
    I[nDP]        += ceqgdtot; I[nD]        -= ceqgdtot;
    dI[6*nDP+nDP] += gdtotd;   dI[6*nD+nDP] -= gdtotd;
    dI[6*nDP+nSP] += gdtots;   dI[6*nD+nSP] -= gdtots;
    dI[6*nDP+nG]  += gdtotg;   dI[6*nD+nG]  -= gdtotg;
    dI[6*nDP+nB]  += gdtotb;   dI[6*nD+nB]  -= gdtotb;
    dI[6*nDP+nD]  += gdtot;    dI[6*nD+nD]  -= gdtot;
  }

#endif

  /*** correct currents for transistor type ***/
  for (i=0; i<6; i++) I[i] *= model->type;

  /*** Q results ***/
  Q[nSP] = model->type * qsrc;
  Q[nDP] = model->type * qdrn;
  Q[nG]  = model->type * qgate;
  Q[nB]  = model->type * qbulk;

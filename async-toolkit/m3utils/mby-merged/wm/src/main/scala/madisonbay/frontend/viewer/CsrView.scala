...
object CsrView {
  ...
  def genView(csrTree: Map[String, Any], path: String): List[String] = {
    ...
    // aa.bb/0-.cc -> aa.bb(0).cc
    val convertedPath = path.replaceAll(IndexOpen, "(").replaceAll(IndexClose, ")")

    val regView = s"Register View: $BR$BR"

    val regBody = getRegBody(csrTree, convertedPath, path)
    ...
    case Some(node: Map[_, _]) => node.asInstanceOf[Map[String, Any]].toList.map {
      ...
      case (k, l: List[_])  =>
        val elements = l.zipWithIndex.map { case (_, index) =>
            val pathToId = s"$PathHref$uriPath.$k$IndexOpen$index$IndexClose"

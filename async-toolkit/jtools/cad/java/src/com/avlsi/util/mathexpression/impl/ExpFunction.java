...
/**
   Default implmentation of exponential function.
 */
public class ExpFunction extends DoubleOperatorCommon {

    protected MathExpression m_x;

    /**
       Construct a call to the exponential funciton.
       @param x Power to raise e to.
     */
    public ExpFunction( MathExpression x ) {
	m_x = x;
    }
	...
	if ( isConstant() ) {
	    ...
	    ret = new ExpFunction( m_x.evaluate( varBindings ) );
	}
	if ( ret.isConstant() ) {
	    try {
	...
	return java.lang.Math.exp( m_x.getConstantValue() );
    }

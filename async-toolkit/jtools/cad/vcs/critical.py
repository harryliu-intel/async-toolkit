...
class OutputNode:
    ...
    def get_transition(self, time):
        ...
        print("No more transitions found for time %f" % time)
        return (None, None)
...
def critical_path(hist_name, connect_name, start, verbose=False):
    #Read the transition history file
    hist_file = open(hist_name, 'r')
    node_dict = parse_hist_file(hist_file)
    hist_file.close()

    #Read the connectivity file
    connect_file = open(connect_name, 'r')
    conn_dict = parse_conn_file(connect_file)
    connect_file.close()
    ...
    while cur_depth < max_depth:
        ...
        if cycle_time > 0:
            print("Depth %2d: %d, %3d, %s +Cycle %d" % (cur_depth, cur_time, path[-1][1]-cur_time, convert_vcs_to_apr_name(cur_node), cycle_time))
        else:
            if cur_depth == 0:
                print("Depth %2d: %d, %3d, %s" % (cur_depth, cur_time, 0, convert_vcs_to_apr_name(cur_node)))
            else:
                print("Depth %2d: %d, %3d, %s" % (cur_depth, cur_time, path[-1][1]-cur_time, convert_vcs_to_apr_name(cur_node)))
...
def main():
    ...
    parser.add_argument('hist_file', help="Pointer to the transition history file")
    parser.add_argument('connect_file', help="Pointer to the netlist connectivity file")
    parser.add_argument('start', help="Starting node at starting time")
    parser.add_argument('-v', '--verbose', action="store_true", help="Verbose output")
    args = parser.parse_args()

    #Call the main function
    critical_path(args.hist_file, args.connect_file, args.start, args.verbose)

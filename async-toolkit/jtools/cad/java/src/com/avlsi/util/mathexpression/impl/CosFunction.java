...
/**
   Default implmentation of the cosine function.
 */
public class CosFunction extends DoubleOperatorCommon {

    protected MathExpression m_x;

    /**
       Construct a call to the cosine function.
       @param x Expression that evaluates to an angle
       in radians.
     */
    public CosFunction( MathExpression x ) {
	m_x = x;
    }
	...
	if ( isConstant() ) {
	    ...
	    ret = new CosFunction( m_x.evaluate( varBindings ) );
	}
	if ( ret.isConstant() ) {
	    try {
	...
	return java.lang.Math.cos( m_x.getConstantValue() );
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof CosFunction) {
            CosFunction o = (CosFunction) obj;

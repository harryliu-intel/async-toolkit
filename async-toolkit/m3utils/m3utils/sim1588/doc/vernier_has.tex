...
\arraycolsep=0.4pt\def\arraystretch{1.1}
...
asserted), errors will be signalled on {\tt o\_vernier\_error} as a nonzero value.  Error
...
ERR\_BAD\_DIVISOR & 3'h3 & attempt to command a bad $B$ clock divisor \\
      \hline
\bf ---reserved--- & 3'h4--3'h7 & reserved \\
      \hline
    \end{tabular}
...

{
  \begin{table}
    ...
    \begin{tabular}{|>{\tt}l|>{\tt}l|c|>{\it}c|>{\raggedright\arraybackslash}p{2.4in}|}
      ...
      \hline
    \end{tabular}
    \caption{Pinout of TSU RTL. MRS/WRS means ``must/will remain stable''}\label{tab:pinout}
  \end{table}
}

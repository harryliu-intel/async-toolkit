...
\begin{abstract}
  ...
  using vernier clocks, is presented.  The theory of time measurement, including
...
data and control are always updated relative to some clock, but where there is
...
The interface and usage of the module presented here is available in a
shorter and more explicit Intel HAS form\cite{vernier-has}.
...
such a configuration (at rest, same gravity field, etc.) that
...
attempt to define a scale of chronometer time is the International Atomic Time
...
used, but non-uniform, Universal Time on January~1, 1958.\footnote{TAI and UTC diverge
...
While we do not discuss the matter much in this paper, a typical
application of the algorithms we investigate is the accurate
synchronizing via network protocols of chronometers to some global,
trusted measure of TAI.  For further details, consult the definition
of IEEE Standard~1588\cite{ieee1588}.  Furthermore, another reason we
mention TAI here is that while it is generally not necessary for all
the parties to have direct access to an accurate chronometer reading
TAI in order to contribute to these algorithms, our exposition will be
easier to follow if we can assume that events are measured on a global
timescale, thereby dispelling any confusion about the nature of the
timestamps that are processed by the system: they can be taken to be
measured on a global and uniform timescale such as TAI.  Many issues
relating to the intricacies of maintaining ``normal'' human calendar time
(the Gregorian calendar, leap seconds, etc.) are therefore entirely
outside of the scope of our discussion.

\rotins_epsfig_listcaption{4in}{chronometer}{A chronometer showing twelve past three o'clock and nineteen seconds.  Source: Wikipedia.}{270}{A chronometer.}
...
This is the event representation shall use for the rest of the paper.
...
The representation of the event can then be given as a Verilog record as follows:
\begin{verbatim}
typedef struct {
  logic             v;
  logic [PREC-1:0]  dt;
...
\listcaption_ins_epsfig{3in}{exevent}{Hardware representation of an example event that occurred at 16:50:39 exactly.  The $dt$ timescale is here in picoseconds, i.e., $\tau=1$~ps.}{Hardware representation of an example event that occurred at 16:50:39 exactly.}
...
\subsection{Convergence Time and Precision}\label{sec:conv_and_prec}
...
The principle is illustrated in figure~\ref{fig:beat}, where one clock is running at a period of 4 time units and the other at 5 time units.  Since $GCD(4,5)=1$ the precision is~1 and the convergence time is~20.

\ins_epsfig{6in}{beat}{Two beating clocks.}

The important observation is that for suitably chosen $p_A$, $p_B$, it
is the case that $GCD(p_A) \ll p_A, p_B$.  The technique is normally
credited to Pierre Vernier, who published it in
1631\cite{vernier-orig}, and it is commonly used in the construction
of calipers, most commonly with $p_A=1000\mu$m and $p_B=900\mu$m so
that the precision is $GCD(p_A,p_B)=100\mu$m, but existing with precisions down to $20\mu$m, yet the instrument can
easily be read by the unaided eye.  See figure~\ref{fig:Principle_vernier}.
\rotins_epsfig_listcaption{3.9in}{Principle_vernier}{Illustration of
  operation of vernier calipers.  Source: Wikipedia.}{0}{Vernier
  calipers.}
...
This is the problem of measuring the time interval between two events.  The
basic problem is illustrated in figure~\ref{fig:timedelta}.

\ins_epsfig{5in}{timedelta}{Two events in the same system.}

The case that is the most difficult is when the two events occur in asynchronous clock
...
\rotins_epsfig_listcaption{3in}{bresenham}{Bresenham line.}{270}{Bresenham line.}
...
\subsubsection{Choosing a clock divisor}

The choice of clock divisor is not entirely straightforward.  The formulas
in section~\ref{sec:conv_and_prec} apply with $p_B$ as the {\em divided\/} clock
period.  If we want to consider $p_B$ as the input, undivided clock period we
instead must use the transformation
\begin{equation}
  p_B \longrightarrow d \, p_B
\end{equation}
throughout section~\ref{sec:conv_and_prec}, where $d$ is the divider.

To keep things simple, we wish only to divide by an even divisor, so that
we do not need to make changes on negative edges of $B$.  The divisors are
thus the set
\begin{equation}
{\mathcal S} = \{ 1, 2, 4, \ldots , 2^{N}-2\}
\end{equation}
where $N$ is the number of bits used to represent our divisor.
($2^N-1$ cannot be used since it is odd.)

We can for example use the algorithm that we choose $d$ so that $d$ is the smallest value that satisfies 
\begin{equation}\label{eq:min_d}
d \in S  \land  2 p_A < p_B \, d 
\end{equation}
with the additional proviso that $p_B$ here must include any allowance for
jitter, asymmetry, and speed offset.  (It must not be possible for
the $A$ clock to miss any single transition of $B$.)  Among these values of $d$
we can choose the smallest.

However, this algorithm can still introduce precision problems.  The
precision of the divided vernier block is
\begin{equation}
  \epsilon_q = LCM(p_A,d \, p_B)  \quad .
\end{equation}
and its convergence time is
\begin{equation}
  \xi = LCM(p_A,d \, p_B)  \quad .
\end{equation}
We note that all we can really say about the GCD and LCM here is that
\begin{equation}
GCD(p_A, p_B)   \le GCD(p_A,d \, p_B)  \le d \, GCD(p_A, p_B) \quad .
\end{equation}
Two special cases are
\begin{equation}
  d \perp p_A \, \Rightarrow \, GCD(p_A,d \, p_B)  = GCD(p_A, p_B)
\end{equation}
and
\begin{equation}
  d | p_A \land GCD(p_A,p_B) = 1 \, \Rightarrow \, GCD(p_A,d \, p_B)  = d  \, GCD(p_A, p_B) \quad .
\end{equation}
In the former case, the absolute time precision is maintained but the convergence time increases by a factor of $d$; in the latter case, the absolute time precision deteriorates by a factor of $d$ but the convergence time is maintained.  The former case can be achieved by increasing $d$ from its minimum value according to equation~\ref{eq:min_d} to a larger number that is coprime to $p_A$. 
...
\listcaption_ins_epsfig{6in}{divtiming}{Timing diagram of stable clock divider.
  ...
  (undivided case) or by $e$ (divided cases).}{Timing diagram of stable clock divider.}
...

{
  \begin{table}
    ...
    \begin{tabular}{|l|l|>{\raggedleft\arraybackslash}p{1in}>{$=\,$}l|>{\raggedleft\arraybackslash}p{1in}|}
      ...
      \hline
    \end{tabular}
    \caption{Some Ethernet speeds of interest.M$=10^6$, G=$10^9$}\label{tab:ethspd}
  \end{table}
}
...
\rotins_epsfig_listcaption{5in}{hp5370a}{Rack mounted HP~5370A Universal Time
  ...
  the 10-MHz master clock (OCXO), last calibrated in 1988, is not known to be
  ...
  widths.}{270}{Photo of rack-mounted HP~5370A Universal Time Interval Counter.}
...
reduced need for integration teams to understand the internals of our
...
evaluated the performance of the scheme in terms of convergence time and
precision.
...
\bibitem{vernier-has}{M.~Nystr\"om and J.~Yan.  {\it Vernier Timestamping Unit High-Level Architecture Specification (HAS) 0.49.} Intel DCG DSD Mount Steller Team, 2018.  Available from the authors.}

...
check_for_empty_arg "$view_name"                  \
    "You must specify the view name that you want to add for each cell."       2
        ...
        for bin_file in $cell_view_bin_files ; do
          ...
          if [ -n "$depot_path" ] ; then
            ...
            if [ -n "$curr_action" ] ; then
              ...
              if [[ -z "$no_sync" || ( ! -e "$bin_file" ) ]] ; then
                $p4_sync_command -f "$escaped_bin_file"
              fi
              $p4_edit_command -t "binary+l" "$escaped_bin_file"
            fi
          else
            if [[ ! ( -f "$bin_file"  &&
                      -r "$bin_file"  &&
        ...
        for text_file in $cell_view_text_files ; do
          ...
          if [ -n "$depot_path" ] ; then
            ...
            if [ -n "$curr_action" ] ; then
              ...
              if [[ -z "$no_sync" || ( ! -e "$text_file" ) ]] ; then
                $p4_sync_command -f "$escaped_text_file"
              fi
              $p4_edit_command -t "text+l" "$escaped_text_file"
            fi
          else
            if [[ ! ( -f "$text_file"  &&
                      -r "$text_file"  &&
    ...
    if [[ ( "$oa" == "1" ||
      ...
      for text_file in $lib_text_files ; do
        ...
        if [ -n "$depot_path" ] ; then
          ...
          message "curr_action $curr_action"
          if [ -n "$curr_action" ] ; then
            curr_change_list=`$p4_fstat_command "$text_file" | $grepcmd -e "^\.\.\. change" | $gawkcmd -- " { print \\$3 } "`
            message "curr_change_list $curr_change_list"
            if [[ "$change_list" != "$curr_change_list" ]] ; then
      ...
      echo "ERROR: \"$lib\" in \"$lib_dir\" is not a valid library."
    fi    
  done

...
IOP( "version", -1, IF_REAL,    "Model version (ignored)"), // added by Fulcrum
IOP( "level",   -1, IF_INTEGER, "Model level (ignored)"),   // added by Fulcrum
...
int BSIM4_ParseParameter(BSIM4model *m, char *s, LEX *lex, LIST *assignments)
  ...
  for (i=0; p[i].name!=NULL; i++)
    {
    if (strcasecmp(p[i].name,s)==0)
      {
      if (p[i].type==IF_INTEGER || p[i].type==IF_FLAG)
        {
        if (lex_is_integer(lex)) v.iValue=lex_eat_integer(lex);
        else v.iValue=(int) MODEL_ParseExpression(lex,assignments);
        err=BSIM4mParam(p[i].num,&v,m);
        return (err==OK);
        }
      else if (p[i].type==IF_REAL)
        {
        v.rValue=MODEL_ParseExpression(lex,assignments);
        err=BSIM4mParam(p[i].num,&v,m);
        return (err==OK);
        }
      }
...
/*** print all bsim4 model parameters ***/
void BSIM4_PrintModel(BSIM4model *m, FILE *fout)
  ...
  int i,err;
  p = BSIM4mPTable;
  fprintf(fout,".MODEL %s %s (\n",
          m->type==NMOS ? "NCH" : "PCH",
          m->type==NMOS ? "NMOS" : "PMOS");
  fprintf(fout,"+wmin=%g wmax=%g lmin=%g lmax=%g\n",
          m->Wmin,m->Wmax,m->Lmin,m->Lmax);
  for (i=0; p[i].name!=NULL; i++)
    {
    fprintf(fout,"+%s=",p[i].name);
    err=BSIM4mAsk(m,p[i].num,&v);
    if (err!=OK) fprintf(fout,"ERROR");
    else if (p[i].type==IF_INTEGER) fprintf(fout,"%d",v.iValue);
    else if (p[i].type==IF_FLAG)    fprintf(fout,"%d",v.iValue);
    else if (p[i].type==IF_REAL)    fprintf(fout,"%g",v.rValue);
    fprintf(fout,"\t* %s\n",p[i].comment);
    }
  fprintf(fout,"+)\n\n");
  }

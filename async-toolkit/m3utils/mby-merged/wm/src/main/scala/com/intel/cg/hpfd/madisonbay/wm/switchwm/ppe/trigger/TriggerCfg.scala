...
/**
  ...
  * @param usage_map
  */
class TriggerCfg(val apply_map: mby_ppe_trig_apply_map,
...
object TriggerCfg {
  implicit def tcfgToApplyMap(x: TriggerCfg): mby_ppe_trig_apply_map = x.apply_map
  implicit def tcfgToApplyMiscMap(x: TriggerCfg): mby_ppe_trig_apply_misc_map = x.apply_misc_map
  implicit def tcfgToUsageMap(x: TriggerCfg): mby_ppe_trig_usage_map = x.usage_map
}

 ...
 *                  ''api.stp.defaultState.vlanMember'' API attribute.
 *
...
fm_status fmDeleteVlanPort(fm_int sw, fm_uint16 vlanID, fm_int port)
{
    ...
    for (cnt = 0 ; cnt < numMembers ; cnt++)
    {
        ...
        if ( (port != 0) && (switchPtr->stpMode == FM_SPANNING_TREE_PER_VLAN) )
        {
    ...
    if (fmGetBoolApiAttribute(FM_AAK_API_MA_FLUSH_ON_VLAN_CHANGE, 
 ...
 *                  Management'' functions.
 *
 * \note            This function will operate correctly on FM6000 devices
 *                  for backwards compatibility, but is deprecated for those
 *                  devices. Instead, set the ''FM_SPANNING_TREE_MODE'' switch 
 *                  attribute to ''FM_SPANNING_TREE_MULTIPLE'', use a single 
 *                  spanning tree instance (instance 0) and then use 
 *                  ''fmSetSpanningTreePortState'' to set the port's forwarding 
 *                  state in that spanning tree instance.
 *
                                        ...
                                        fm_int   maxPhysPorts)
{
    ...
    *numPhysPorts = 0;
    ...
    for (listIndex = 0 ; listIndex < numVlanPorts ; listIndex++)
    {
        ...
        /* Get the physical ports for this logical port. */
        err = fmGetPhysicalMemberPortList(sw,
                                          port,
                                          &numMembers,
                                          memberList,
        ...
        /* Add each physical port to the port mask. */
                                  ...
                                  fm_int *  sVlan)
{
    ...
    err = switchPtr->GetSVlanFromPortCVlan(sw, port, cVlan, sVlan);
...
fm_status fmGetCVlanFirst(fm_int sw, fm_int *firstPort, fm_int *firstCVlan)
{
    ...
    err = switchPtr->GetCVlanFirst(sw, firstPort, firstCVlan);

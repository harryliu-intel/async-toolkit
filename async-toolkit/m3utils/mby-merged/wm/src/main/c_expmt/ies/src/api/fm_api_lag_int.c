 ...
 * \ingroup intLag
 *
 * \desc            Adds a port to the LAG member structure.
 *
 ...
 *
 * \param[out]      memberIndex points to a caller-supplied variable to
 *                  receive the index of the port in the member array.
 *
 * \return          FM_OK if successful.
 *
 *****************************************************************************/
static fm_status AddLagMember(fm_int  sw,
                              fm_int  lagIndex,
                              fm_int  port,
                              fm_int *memberIndex)
{
    ...
    *memberIndex = -1;
    ...
    {
        *memberIndex = portPtr->memberIndex;
        err = FM_OK;
        goto ABORT;
    }
    ...
    /* Find an unused entry in the member table. */
    for (i = 0 ; i < FM_MAX_NUM_LAG_MEMBERS ; i++)
    {
        if (lagPtr->member[i].port == FM_LAG_UNUSED_PORT)
        {
            /* Add the port to the LAG. */
            lagPtr->member[i].port = port;
            lagPtr->member[i].isActive = FALSE;
 ...
 * \ingroup intLag
 *
 * \desc            Returns the number of ports in the specified LAG.  A port
 *                  is considered to be a LAG member even if the port is
 *                  currently down.
 *
 * \note            This is an internal function. The caller is assumed
 *                  to have protected the switch and claimed the LAG lock.
 ...
 * \ingroup intLag
 *
 * \desc            Returns the number of ports in the specified LAG that
 *                  are in the "up" state.
 *
 * \note            This is an internal function. The caller is assumed
 *                  to have protected the switch and claimed the LAG lock.
...
fm_status fmInitLAGTable(fm_int sw)
{
    ...
    switchPtr->perLagMgmt = 
                fmGetBoolApiAttribute(FM_AAK_API_PER_LAG_MANAGEMENT, 
                                      FM_AAD_API_PER_LAG_MANAGEMENT);

    for (port = 0 ; port <= FM_MAX_LOGICAL_PORT ; port++)
    {
        switchPtr->lagInfoTable.portIsUp[port] = FALSE;
    }
    ...
    switchPtr->lagInfoTable.dropLACPFrames = TRUE;
...
fm_status fmInformLAGPortDown(fm_int sw, fm_int port)
{
    ...
    if (!switchPtr->lagInfoTable.portIsUp[port])
    {
        goto ABORT;
    }
...
fm_status fmInformLAGPortUp(fm_int sw, fm_int port)
{
    ...
    switchPtr->lagInfoTable.portIsUp[port] = TRUE;
...
fm_status fmCheckLACPFilter(fm_int sw, fm_eventPktRecv *event, fm_bool *filter)
{
    ...
    *filter   = FALSE;
    ...
    if (destAddr == FM_LACP_DEST_ADDRESS)
    {
        ...
        if (switchPtr->lagInfoTable.dropLACPFrames)
        {
            *filter = TRUE;
        }
    }
...
ABORT:
    DROP_LAG_LOCK(sw);
    FM_LOG_EXIT_CUSTOM(FM_LOG_CAT_LAG, err, "lagNumber = %d\n", *lagNumber);
...
fm_status fmAddLAGMember(fm_int sw, fm_int lagIndex, fm_int port)
{
    ...
    err = AddLagMember(sw, lagIndex, port, &memberIndex);
 ...
 *
 * \param[in]       active TRUE  indicates to return active member ports only.
 *                         FALSE indicates to return all the member ports.
 * 
 * \return          FM_OK if successful.
 *
 *****************************************************************************/
                              ...
                              fm_bool active)
{
    ...
    *numPorts = 0;
    ...
    {
        if (lagPtr->member[i].port != FM_LAG_UNUSED_PORT && 
            (!active || lagPtr->member[i].isActive))
        {
...
/*****************************************************************************/
/** fmGetPhysicalMemberPortList
 * \ingroup intLag
 *
 * \desc            Return all physical member ports of a given LAG, or
 *                  the port itself if a physical port.
 *
 * \note            This is an internal function. The caller is assumed
 *                  to have protected the switch and claimed the LAG lock.
 *
                                      ...
                                      fm_int maxPorts)
{
    ...
    else
    {
        ...
        for (i = 0 ; i < numMem ; i++)
        {
            ...
            {
                members[numPhysMembers++] = members[i];
            }
        }

        *numMembers = numPhysMembers;
    }

    return FM_OK;

}   /* end fmGetPhysicalMemberPortList */
 ...
 *
 * \return          FM_OK if successful.
 * \return          FM_ERR_INVALID_PORT if the LAG has no members.
 * \return          FM_ERR_UNSUPPORTED if the LAG has no physical members.
 *
 *****************************************************************************/
fm_status fmGetLAGMemberIndex(fm_int sw, fm_int memberPort, fm_int *memberIndex)
{
    ...
    /* TODO: Probably it is more efficient to save the index
     *       than to do this every time. Since the number of
     *       members are not that many, then not too bad here.
     */
    err = fmSortPortByGlort(sw, portList, numPorts, portList);
    FM_LOG_EXIT_ON_ERR(FM_LOG_CAT_LAG, err);
    ...
    {
        numPorts--;
        if (portList[numPorts] == memberPort)
        {
            /* member index starts from 1,
             * 0 is for the LAG itself
             */
            *memberIndex = numPorts + 1;
            FM_LOG_EXIT_CUSTOM(FM_LOG_CAT_LAG,
                               FM_OK,
                               "memberIndex=%d\n",
                               *memberIndex);
    ...
    FM_LOG_EXIT(FM_LOG_CAT_LAG, FM_ERR_NOT_FOUND);
...
fm_status fmRemoveLAGMember(fm_int sw, fm_int lagIndex, fm_int port)
{
    ...
    if (portPtr->lagIndex == lagIndex &&
        portPtr->memberIndex >= 0)
    {
        /* Remove the port from the LAG member table. */
        lagPtr->member[portPtr->memberIndex].port = FM_LAG_UNUSED_PORT;
        lagPtr->member[portPtr->memberIndex].isActive = FALSE;
        portPtr->lagIndex = -1;
...
/*****************************************************************************/
/** fmAddLAGActiveMember
 * \ingroup intLag
 *
 * \desc            Adds an active member to the LAG internal software structure.
 *
 * \note            This is an internal function. The caller is assumed
 *                  to have protected the switch and claimed the LAG lock.
 *
...
fm_status fmAddLAGActiveMember(fm_int sw, fm_int lagIndex, fm_int port)
{
    ...
    {
        lagPtr = GET_LAG_PTR(sw, lagIndex);
        lagPtr->member[memberIndex].isActive = TRUE;
    }

    FM_LOG_EXIT(FM_LOG_CAT_LAG, err);

}   /* end fmAddLAGActiveMember */




/*****************************************************************************/
/** fmDeactivateLagMember
 * \ingroup intLag
 *
 * \desc            Deactivates a LAG member without removing it from the
                                  ...
                                  fm_portmask *activeDestMask)
{
    ...
    if (!fmIsCardinalPort(sw, internalPort))
    {
        ...
        while (intPortCount)
        {
            ...
            if (switchPtr->lagInfoTable.portIsUp[memberPort])
            {
    ...
    else
    {
        ...
        if (switchPtr->lagInfoTable.portIsUp[internalPort])
        {
                                 ...
                                 fm_bool   tag)
{
    ...
    stp = fmGetIntApiAttribute(FM_AAK_API_STP_DEF_STATE_VLAN_MEMBER,
...
fm_status fmLAGPortAddDelete(fm_int sw, fm_int lagIndex)
{
    ...
    for (cnt = 0 ; cnt < numPorts ; cnt++)
    {
             ...
             (nPorts < switchPtr->info.maxPortsPerLag) )
        {
            ...
            err = fmAddLAGActiveMember(sw, lagIndex, port);
            FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_LAG, err);
        }
        else
        {
            ...
            (void)fmDeactivateLagMember(sw, lagIndex, port);

            /**************************************************
             * Don't need to check for error since this might
             * remove members not in active list 
             **************************************************/
        }
    ...
    {
        /**************************************************
         * We must sort the ports in order to guarantee 
        ...
        err = fmSortPortByGlort(sw, ports, nPorts, ports);
        FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_LAG, err);

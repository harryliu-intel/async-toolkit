...
import madisonbay.tcp._ // TODO: move it to common
...
object crc extends crc32Table with crc32cTable{

  // TODO: Do sth with ???
  private def reverseBytesOf[A: ByteArrayEncoder : ByteArrayDecoder](i: A): A = {
    implicit val _ = EitherT.eitherTMonadError[Id, Throwable]

    ByteArrayDecoder[A].decode[EitherT[Id, Throwable, ?]].eval(
      ByteArrayEncoder[A].encode(i).reverse
    ).fold(_ => ???, identity)
  }

  private def crc32(initial: Int, bytes: Array[Byte], table256: Array[Int]) =
    bytes.foldRight(initial) {
      case (byte, crc) => table256((crc ^ byte).toByte) ^ crc >> 8
    }

  private def mbyCrc32Math(bytes: Array[Byte]): Int = crc32(0xffffffff, bytes, mbyCrc32Table)

  private def mbyCrc32CMath(bytes: Array[Byte]): Int = crc32(0, bytes, mbyCrc32CTable)

  def mbyCrc32ByteSwap(bytes: Array[Byte]): Int = (mbyCrc32Math _ andThen reverseBytesOf[Int])(bytes)

  def mbyCrc32CByteSwap(bytes: Array[Byte]): Int = (mbyCrc32CMath _ andThen reverseBytesOf[Int])(bytes)

...
/**
 * Class to represent a transistor in a .aspice file.
 *
 * @author Jesse Rosenstock
 * @version $Name:  $ $Date$
 **/
public final class Transistor implements DeviceInterface,TransistorInterface {
    ...
    /** Name of gate node. **/
    private final HierName gate;

    /** Name of bulk node. **/
    private final HierName bulk;
   ...
   /**
     ...
     *
     * @throws IllegalArgumentException  If <code>type</code> is bad.
     **/

    public Transistor(final int type,
    ...
    /**
     * Get name of gate node.
     * @return name of gate node
     **/
    public HierName getGate() {
        return gate;
    }

    /**
     * Get name of bulk node.
     * @return name of bulk node
     **/
    public HierName getBulk() {
        return bulk;
    }
    ...
    public String getAspiceString() {
        return (type == DeviceTypes.N_TYPE ? "nmos (" : "pmos (")
            ...
            + getGate().getAspiceString() + ","
            + getDrain().getAspiceString() + ","
            + getBulk().getAspiceString() + ") ("
     ...
     *   <code>data.length != NUM_PORTS</code> or 
     *   <code>voltage.length != NUM_PORTS</code>.
  ...
  /* Note that we need to unswap source and drain for I and dI,
      but not for Q and dQ */

        data[0].setVoltage(voltage[0]);

        data[0].setCurrent(m.type * Ids);
        data[0].setCurrentDerivative(nS, -Gds - Gm - Gmb);
        data[0].setCurrentDerivative(nD, Gds);
        data[0].setCurrentDerivative(2, Gm);
        data[0].setCurrentDerivative(3, Gmb);

        data[0].setCharge(m.type * qsrc);
        data[0].setChargeDerivative(0, gcssb);
        data[0].setChargeDerivative(1, gcsdb);
        data[0].setChargeDerivative(2, gcsgb);
        data[0].setChargeDerivative(3, -gcssb - gcsdb - gcsgb);


        data[1].setVoltage(voltage[1]);

        data[1].setCurrent(-m.type * (Ids + Isub));
        data[1].setCurrentDerivative(nS, Gds + Gm + Gmb + Gbd + Gbg + Gbb);
        data[1].setCurrentDerivative(nD, -Gds - Gbd);
        data[1].setCurrentDerivative(2, -Gm  - Gbg);
        data[1].setCurrentDerivative(3, -Gmb - Gbb);

        data[1].setCharge(m.type * qdrn);
        data[1].setChargeDerivative(0, gcdsb);
        data[1].setChargeDerivative(1, gcddb);
        data[1].setChargeDerivative(2, gcdgb);


        data[2].setVoltage(voltage[2]);

        data[2].setCurrent(0);
        data[2].setCurrentDerivative(0, 0);
        data[2].setCurrentDerivative(1, 0);
        data[2].setCurrentDerivative(2, 0);
        data[2].setCurrentDerivative(3, 0);

        data[2].setCharge(m.type * qgate);
        data[2].setChargeDerivative(0, gcgsb);
        data[2].setChargeDerivative(1, gcgdb);
        data[2].setChargeDerivative(2, gcggb);
        data[2].setChargeDerivative(3, -gcgsb - gcgdb - gcggb);


        data[3].setVoltage(voltage[3]);

        data[3].setCurrent(m.type * Isub);
        data[3].setCurrentDerivative(0, -Gbd - Gbg - Gbb);
        data[3].setCurrentDerivative(1,  Gbd);
        data[3].setCurrentDerivative(2,  Gbg);
        data[3].setCurrentDerivative(3,  Gbb);

        data[3].setCharge(m.type * qbulk);
        data[3].setChargeDerivative(0, gcbsb);
        data[3].setChargeDerivative(1, gcbdb);
        data[3].setChargeDerivative(2, gcbgb);
        data[3].setChargeDerivative(3, -gcbsb - gcbdb - gcbgb);

...
package Applications::TestPoint::Common::FM2000::QoSCore;
use strict;
use warnings;
...
our @EXPORT = qw(
    tpFM2000HandleShowQos
    tpFM2000HandleSetQos
    tpFM2000IsPerLagPortAttribute
);


sub tpFM2000ShowQosNumeric;

sub tpFM2000ValidatePrivatePauseOffWatermark;
sub tpFM2000ValidatePrivatePauseOnWatermark;
sub tpFM2000ValidateRxSmpHogWatermark;
sub tpFM2000ValidateRxSmpPrivateWatermark;
sub tpFM2000ValidateTxSmpHogWatermark;
sub tpFM2000ValidateTxSmpPrivateWatermark;


###############################################################################
#
# FM2000_qos_attribute_map hash definition
#
...
# array elements:
# 1st: attribute ID.
# 2nd: reference to a display function.
# 3rd: reference to a validation function.
#
###############################################################################

my %FM2000_qos_attribute_map = (
    'private_pause_off_wm' => [$FM_QOS_PRIVATE_PAUSE_OFF_WM,
                               \&tpFM2000ShowQosNumeric,
                               \&tpFM2000ValidatePrivatePauseOffWatermark],

    'private_pause_on_wm'  => [$FM_QOS_PRIVATE_PAUSE_ON_WM,
                               \&tpFM2000ShowQosNumeric,
                               \&tpFM2000ValidatePrivatePauseOnWatermark],

    'rx_smp_hog_wm'        => [$FM_QOS_RX_HOG_WM,
                               \&tpFM2000ShowQosNumeric,
                               \&tpFM2000ValidateRxSmpHogWatermark],

    'rx_smp_private_wm'    => [$FM_QOS_RX_PRIVATE_WM,
                               \&tpFM2000ShowQosNumeric,
                               \&tpFM2000ValidateRxSmpPrivateWatermark],

    'tx_smp_hog_wm'        => [$FM_QOS_TX_HOG_WM,
                               \&tpFM2000ShowQosNumeric,
                               \&tpFM2000ValidateTxSmpHogWatermark],
);
...
sub tpFM2000ValidatePrivatePauseOffWatermark
{
    ...
    if ( !defined($wm) || ($wm < 0) || ($wm > 4193280) )
    {
        print("Invalid watermark value for $attrStr. ");
        print("The range must be between 0-4193280.\n");
        return $FM_FAIL;
    }
...
sub tpFM2000ValidateRxSmpHogWatermark
{
    ...
    if ( !defined($wm) || ($wm < 0) || ($wm > 1047552) )
    {
        print("Invalid watermark value for $attrStr. ");
        print("The range must be between 0-1047552.\n");
        return $FM_FAIL;
    }
...
##@cmethod public void tpFM2000HandleShowQos(int sw, char *portListRef)
#
...
sub tpFM2000HandleShowQos
{
    ...
    foreach my $attrStr (sort(keys(%FM2000_qos_attribute_map)))
    {
        my $attrInfoRef = $FM2000_qos_attribute_map{$attrStr};

        &{$attrInfoRef->[1]}($self, $sw, $portListRef, $attrStr,
                             $attrInfoRef->[0], 1);
    }
...
##@cmethod public void tpFM2000HandleSetQos(int sw, char attrStr,
#                                           char *portListRef,
...
# @return       None.
# 
sub tpFM2000HandleSetQos
{
    my ($self, $sw, $attrStr, $portListRef, $index, $watermark) = @_;


    if ( !exists $FM2000_qos_attribute_map{$attrStr} )
    {
    ...
    {
        print("Invalid index value. The value must be 0.\n");
        return $FM_FAIL;
    }


    # Validate specific QoS attribute watermark
    my $status = &{$attrInfoRef->[2]}($self, $attrStr, $watermark);
...
##@cmethod public bool tpFM2000IsPerLagPortAttribute(int sw, char attrStr)
#
...
# 
sub tpFM2000IsPerLagPortAttribute
{

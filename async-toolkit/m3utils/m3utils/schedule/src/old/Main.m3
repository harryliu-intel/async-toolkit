...
PROCEDURE SaveJitter(try : [-1..LAST(CARDINAL)]) : JitterState =
  VAR res : JitterState;
  BEGIN
    IF try # -1 THEN
      res.old       := jitter[try];
...
PROCEDURE AssertJitterOK(try : [-1..LAST(CARDINAL)]) =
  BEGIN
    IF try # -1 THEN
...
PROCEDURE SatisfyAssertions(READONLY ni : ARRAY OF CARDINAL) : BOOLEAN =
  ...
  PROCEDURE Recurse(p         : CARDINAL; 
    ...
    PROCEDURE OKtoTry(try : [-1..LAST(CARDINAL)]; VAR whyNot : TEXT) : BOOLEAN=
      VAR
        cnt := 0;
    ...
    BEGIN
      ...
      FOR incr := 2 TO NUMBER(ports^)+2 DO
        ...
        BEGIN
          ...
          IF OKtoTry(try,whyNot) AND SetJitterOK(p,try,whyNot) THEN

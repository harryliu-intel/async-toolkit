...
class PackageDatabase(WithCm3):
    ...
    def __init__(self, cm3):
        ...
        # before the index.
        self._load_package_sets() # KY7
        self._load_package_index() # KY8
...
# KY12: the initialization function is at KY13
class FullUpgradeCommand(UpgradeCommand):
    ...
    def execute(self):
        ...
        # KY14: upgrade core libraries: base_packages = ["+front", "+m3bundle", "-m3cc"]
        super().execute()
        ...
        # KY15, KY16-packages(), KY17-buildship()
        self.buildship(self.packages())

    # KY16: read package list in the command-line, and return them with HEADLESS packages
    def packages(self):
        "Return the packages requested on the command-line"
        pkgs = super().packages()
        if not pkgs or pkgs[0].startswith("-"):
...
class InstallCommand(ConciergeCommand):
    ...
    # KY10: Install command will build compiler from the bootstrap folder first, and then Fullupgrade all packages
    def execute(self):
    ...
    def full_upgrade(self):
        ...
        # KY12: _upgrade is initialized as FullUpgradeCommand(cm3, package_db)
...
class Concierge:
    ...
    def __init__(self, args = None):
        ...
        self._parse_args(args) # KY2: update initial args here

    # KY3
    def main(self):
        "Carry-out the requested command"
        command = self._command(self.cm3(), self.package_db()) # KY: _command is decided during _parse_args() when initializing
        # KY9: InstallCommand as an example
        command.set_options(self)
        command.execute()

    def cm3(self):
        if not self._cm3: # KY: initialize CM3 build environment, go to KY4 (class Cm3)
            self._cm3 = Cm3(
                script=self._script,
            ...
            self._cm3.set_options(self) # KY5
        return self._cm3

    def package_db(self):
        if not self._package_db:
            self._package_db = PackageDatabase(self.cm3()) # KY6
        return self._package_db
    ...
    def _parse_command(self, args):
        ...
        self._command = constructor # KY: class InstallCommand for "install"

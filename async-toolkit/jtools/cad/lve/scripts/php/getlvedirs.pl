...
my @entries=();
foreach my $dbfile (`find /p/work/*/ -maxdepth 6 -name lvedb.db -size +0 2>/dev/null`) {
    chomp $dbfile;
    next if ! -r $dbfile;
...
my $dbt=DBI->connect("dbi:mysql:lve:host=tsyvweb03.ts.intel.com", "lve", "lve");
my $ref=$dbt->selectall_arrayref("SELECT root from root");
foreach my $value (@{$ref}) {
    my @x=@{$value};
    my $root=$x[0];
    $root =~ s:/nfs/site/disks/wdisk.\d+/::;
    $root =~ s:/nfs/site/disks/::;
    push @entries, $root;
}
$dbt->disconnect;
open (OUT, ">/srv/www/htdocs/lvedb/lvedirs.tmp");
select OUT;
print join ("\n", sort @entries);
close OUT;
rename "/srv/www/htdocs/lvedb/lvedirs.tmp", "/srv/www/htdocs/lvedb/lvedirs.txt";

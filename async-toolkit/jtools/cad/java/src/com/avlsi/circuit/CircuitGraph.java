...
/**********************************************************************
 * Circuit graph data structures and associated data mining algorithms.
 * The top-level CircuitGraph class represents an entire (flat) cell
 * of transistors, resistors, and capacitors.  It can be built either
 * from an existing {@link com.avlsi.file.aspice.AspiceCell}, or 
 * using the <tt>addDevice()</tt> 
 * methods from the {@link AbstractCircuit} parent class.
 * <p>
 * Once the CircuitGraph is built, the {@link #identifyNets()} method can be
 * used to partition the graph into resistive subnetworks 
 * ({@link ResistiveSubnet}).  The ResistiveSubnet class defines various
 * useful mining algorithms, such as <tt>identifyVDN()</tt> to identify the 
 * net's virtual driver node, and <tt>findMaxGateRC()</tt> to determine the
 * maximum resistive path on that net and its associated wire and gate 
 * capacitance.
 * <p>
 ...
 **********************************************************************/
public class CircuitGraph extends AbstractCircuit {
     ...
     ***********************************************/
    public CircuitGraph(String cellType) {
        super(cellType);
        nodeMap = new HashMap();
    ...
    /*************************************************************
     * CircuitGraph constructor.  Builds a circuit graph from the
     * given AspiceFile, ignoring any of its subcells.
     *************************************************************/
    public CircuitGraph(AspiceFile aspiceCell) throws Exception
    {
        ...
        //
        // Add all resistors in circuit to CircuitGraph
        //
        ...
        //
        // Add all capacitors to the CircuitGraph
        //
        ...
        //
        // Add all transistors to the CircuitGraph
        //
     ...
     * called after calling {@link #identifyNets()}.
     *****************************************************************/
    public Iterator getResistiveSubnets() { 
        return resSubnets.iterator(); 
    }

    /*****************************************************************
     * Searches the graph for all nodes resistively connected to hn.
     * Return ArrayList contains ResDevice types.
     * 
    public ArrayList findResistiveNetwork(final HierName hn) throws Exception
    {
        Node n = (Node)nodeMap.get(hn);
        if (n == null) throw new Exception("Node "+hn.getAsString('.')
                                           +" doesn't exist.");
        ArrayList reslist = new ArrayList();
        n.resDepthFirstSearch(reslist);
        clearMarks();
        return reslist;
    }
     *****************************************************************/
    ...
    public static class Repository extends AbstractCircuit.Repository {
        ...
        protected AbstractCircuit newCircuitGenerator(final String cellType) {
            return new CircuitGraph(cellType);
        }
    }

...
It will turn out to be useful to consider the case that {\em exactly\/} $k$ out of $N$ subsystems are functional, and we will write this analogosly to (\ref{eq:sysyield}) as $P_{S(N)}(x = k)$.
...
%\begin{equation}
  \begin{equation}\label{eq:pi}\begin{aligned}
    \pi_{1\ldots11} = P(&U_0 &\cap \  U_1 &\cap \cdots \cap U_{N-1} \quad )   \\
    \pi_{1\ldots10} = P(&U_0'&\cap \ U_1 &\cap \cdots \cap U_{N-1} \quad )   \\
    \ldots\\
    \pi_{0\ldots00} = P(&U_0'&\cap \ U_1' &\cap \cdots \cap U_{N-1}' \quad)  , \\
  \end{aligned}\end{equation}
%\end{equation}
...
in $\tau$.  In other words, we may write
  ...
  \end{equation}
  whose value is given by equation \ref{eq:master} as
\begin{equation}
\pi_{1\ldots10} + \pi_{11\ldots1} = \pi_{1\ldots01} + \pi_{11\ldots1} = \cdots = \pi_{01\ldots11} + \pi_{11\ldots1} = Y(N-1)
\end{equation}
...
subsystems that need to be functional.  The so-returned procedure
...
expected yield of large semiconductor ICs relatve to what would be expected
...
\end{equation}
irrespective of the statistics (we have multiplied the size of the units by 4 so all the arguments are multiplied by 4).
...
die---into consideration ``at one''), and doing so with computational
efficiency.

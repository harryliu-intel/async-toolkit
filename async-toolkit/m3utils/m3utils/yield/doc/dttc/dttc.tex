...
It never rains but it pours --- {\em British proverb}

When it rains, it pours --- {\em American advertising slogan}
...
formula and the Bose-Einstein formula, are special cases of
...
For small values of Stapper's $\alpha$, and in particular for large chips,
...
sophisticated method (\ref{eq:murphy}) already in 1964.  Big
...
particular refined Murphy's approach and applied Gamma distribution
statistics to the yield problem in
...
constant rate of defects, the Stapper model allows for a variable defect rate, to account for defect clustering. 
...
or something similar, which assumes that yields are clustered and not
independent.  The two formulations are incompatible.
...
subsystems $u_i \in \Sigma$ will have been fabricated correctly, with the balance defective.  Let's define $C$ to be the set of correctly fabricated subsystems.  It is clear that $C \subseteq \Sigma$.
...
from a certain design transformation.  For example, consider moving from a system of $N$ equal subsystems of area 1, all of which must work, to a system of $N+1$ equal subsystems, where still $N$ of which must work.  We can write the yield improvement
\begin{equation}
...
In practice we make a change of variables and evaluate
\begin{equation}
...
section~\ref{sec:general}.\footnote{We have also developed two more
  ...
  formulation described here: a generalized polynomial formulation and
  a finite-differences formulation.  We do not have the space to
  describe these approaches here.}

\subsection{Example}
A generic deep-submicron process circa 2020 has $n=30$, $D_0 = 0.10$
(defects per square inch per layer), $\alpha$ in the range 0.02--0.10.
...
(``reasonable'' value, contemporary fabrication processes).
...
in the figure, representing a
specific hardware design, with the $f(D)$ corresponding to the
...
of the HAL1000 subprocessors to be faulty.
...
\begin{table}
  ...
  \begin{tabular}{| c | c | c | c | c | c | c | c |}
    ...
    & $\alpha\gg 1$ & $\alpha\gg 1$ & $\alpha\gg 1$ & $\alpha=0.02$ & $\alpha=0.02$& $\alpha=0.02$ & \\
    \hline
...
models overestimate the efficiacy of adding hardware redundancy and

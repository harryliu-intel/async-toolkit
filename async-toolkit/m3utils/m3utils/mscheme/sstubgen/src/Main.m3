...
CONST ScmFlag = "scm";
BEGIN
  M3Args.RegisterString(StubGenTool.tool_g, ScmFlag, "scheme command file", shared := FALSE);
  WITH res = ABS(M3ToolFrame.Startup(
    ...
    WITH  args = ScmArgsOrZeroLength()^,
          pc = NUMBER(args)+1,
          arr = NEW(REF ARRAY OF Pathname.T, pc) DO
      arr[0] := "require";
      FOR i := 1 TO pc-1 DO arr[i] := args[i-1] END;
      TRY

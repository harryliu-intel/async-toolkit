...
fm_status fmDeleteMirror(fm_int sw, fm_int group)
{
    ...
    else
    {
        ...
        if ( (err == FM_OK) && (switchPtr->DeleteMirror != NULL) )
        {
                                    ...
                                    fm_portIdentifier *mirrorPortId)
{
    ...
    {
        /* Note that this has to happen outside the state lock, because
...
fm_status fmAddMirrorPort(fm_int sw, fm_int group, fm_int port)
{
    ...
    /* We have to drop the state lock here since GET_PORT_MIRROR_GROUP can
       execute return if the group is invalid. */
    FM_DROP_STATE_LOCK(sw);
                          ...
                          fm_mirrorVlanType direction)
{
    ...
    err = fmAddMirrorVlanInternal(sw, group, FM_VLAN_SELECT_VLAN1, vlanID, direction);
                                     ...
                                     fm_uint16     vlanID)
{
    ...
    else
    {
        ...
        err = fmTreeRemove(vlanTree, vlanID, NULL);
        FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_MIRROR, err);
                           ...
                           fm_mirrorType *mirrorType)
{
    ...
    for (i = 0 ; i < fmRootApi->fmSwitchStateTable[(sw)]->mirrorTableSize ; i++)
    {
        grp = &fmRootApi->fmSwitchStateTable[(sw)]->mirrorGroups[(i)];
    ...
    if (i >= fmRootApi->fmSwitchStateTable[(sw)]->mirrorTableSize)
    {
                          ...
                          fm_mirrorType *mirrorType)
{
    ...
    for (i = (currentGroup + 1) ;
         i < fmRootApi->fmSwitchStateTable[(sw)]->mirrorTableSize ;
         i++)
    {
        grp = &fmRootApi->fmSwitchStateTable[(sw)]->mirrorGroups[(i)];

...
int BSIM4_SetupModel(BSIM4model *model) {
  /* inherit generic model defaults */
  model->BSIM4type = model->type;
	...
	/* Cross-term dependence */
        ...
        if (!model->BSIM4tnomGiven)  
            model->BSIM4tnom = 27; // was ckt->CKTnomTemp; 
...
int BSIM4_SetupInstance(BSIM4model *model, BSIM4instance *here, CKTcircuit *ckt, int warn) {
  ...
  double Rtot, DMCGeff, DMCIeff, DMDGeff;
	     ...
	     */
            ...
            {   here->BSIM4rbodyMod = model->BSIM4rbodyMod;
                if (warn) fprintf(stderr, "WARNING: rbodyMod has been set to its global value %d.\n",
	        model->BSIM4rbodyMod);
            }
            ...
            {   here->BSIM4rgateMod = model->BSIM4rgateMod;
                if (warn) fprintf(stderr, "WARNING: rgateMod has been set to its global value %d.\n",
                model->BSIM4rgateMod);
            }
            ...
            {   here->BSIM4trnqsMod = model->BSIM4trnqsMod;
                if (warn) fprintf(stderr, "WARNING: trnqsMod has been set to its global value %d.\n",
                model->BSIM4trnqsMod);
            }
            ...
            {   here->BSIM4acnqsMod = model->BSIM4acnqsMod;
                if (warn) fprintf(stderr, "WARNING: acnqsMod has been set to its global value %d.\n",
                model->BSIM4acnqsMod);
            }
	    ...
	    if (!here->BSIM4scGiven)
            ...
            {   error = CKTmkVolt(ckt,&tmp,here->BSIM4name,"drain");
                ...
                here->BSIM4dNodePrime = tmp;
            }
            else here->BSIM4dNodePrime = here->BSIM4dNode;
            ...
            {   error = CKTmkVolt(ckt,&tmp,here->BSIM4name,"source");
                if(error) return(error);
                here->BSIM4sNodePrime = tmp;
            }
            else here->BSIM4sNodePrime = here->BSIM4sNode;

            if ((here->BSIM4rgateMod > 0) && (here->BSIM4gNodePrime == 0))
            {   error = CKTmkVolt(ckt,&tmp,here->BSIM4name,"gate");
                if(error) return(error);
                   here->BSIM4gNodePrime = tmp;
            }
            else

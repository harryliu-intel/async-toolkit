...
Channels may be elements of arrays.

\subsection{Nodes}\label{sec:nodeport}

A {\em node\/} is a shared boolean variable that is attached to a node
port of one or more processes.  It can be read and written as a local
variable, and it can be used in any context where a scalar boolean local
variable could be used.  Nodes may be {\em input nodes, output nodes,}
or {\em inout nodes,} and like channels are declared externally to the
CSP language.

Nodes may be elements of arrays.

If a node is assigned  to (possibly repeatedly) by a single process
\verb!P! (in which the node is attached to an output or inout node
port) and also available to a set of other observing processes \verb!Q_0..Q_n-1!
(in each of which the node is attached to an input or inout node port) then the final assignment within \verb!P! will eventually be available
to \verb!Q_0..Q_n-1!, but it is not specified in what order the
observers will observe the update.

If a node is assigned to by more than one process \verb!P_0..P_n-1!, and
all the assigned values \verb!v_i! are the same \verb!v!, the observing processes will eventually observe the node to be \verb!v!, but it is not specified
in what order the observers will observe the update.

If a node is assigned to by more than one process \verb!P_0..P_n-1!,
and the final assigned values \verb!v_i! are different, the observing
processes will eventually observe the node to be some value \verb!w!,
and that value will be the same across all processes, but it is not
specified what the value will be other than that it will be one of the
\verb!v_i!, nor is it specified in what order the observers will observe
the update.  (Since nodes are boolean, this just means that the
observed values will eventually converge to \verb!true! or
\verb!false! across all the \verb!Q_i!.)
...
channel.  $cR$ is not incremented for the channel.  The peek is
thus a receive expression that does not increment $cR$, and allows
the same value to be inspected more than once without using a variable
to store that value between uses.
See sections~\ref{sec:receive} and~\ref{sec:receive-expression}.

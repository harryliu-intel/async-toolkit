...
#
# Usage:
# % rdlgen.rb python_reg_api5 top.rdl registers.pm
#

class RegistersPyView < RDL::ViewGen
  def initialize(top_rdl_file, view_file, template_file)
    ...
    update(pythonRegParamsApi5,         # insert new code between delimiters
  ...
  def printAmParam(code, amName, amBase, amSize)
    ...
    code.push "#{amName}_BASE".ljust(64)+"= 0x%07X;" % amBase
    code.push "#{amName}_SIZE".ljust(64)+"= 0x%07X;" % amSize
    code.push ""
    return code
  end
  ...
  def printRegParam(code, amName, regName, nDims, regWidth, entries0, entries1, entries2, stride0, stride1, stride2, regOffset, default)
    ...
    code.push "#{regName}_WIDTH".ljust(74)+"= %d;" % numQuadWords
    if(nDims == 3) 
        code.push "#{regName}_ENTRIES_0".ljust(74)+"= %d;" % entries0  
        code.push "#{regName}_ENTRIES_1".ljust(74)+"= %d;" % entries1 
        code.push "#{regName}_ENTRIES_2".ljust(74)+"= %d;" % entries2 
        if(regWidth > 1)
            code.push "def #{regName}(index2, index1, index0, qword) :\n\treturn ((0x%07X * index2)" % stride2 + " + (0x%07X * index1)" % stride1 + " + (0x%07X * index0) + (qword * 8)" % stride0 + " + 0x%07X + #{amName}_BASE);\n" % regOffset
        else
            code.push "def #{regName}(index2, index1, index0) :\n\treturn ((0x%07X * index2)" % stride2 + " + (0x%07X * index1)" % stride1 + " + (0x%07X * index0)" % stride0 + " + 0x%07X + #{amName}_BASE);\n" % regOffset
        end        
    end
    if(nDims == 2) 
        code.push "#{regName}_ENTRIES_0".ljust(74)+"= %d;" % entries0  
        code.push "#{regName}_ENTRIES_1".ljust(74)+"= %d;" % entries1 
        if(regWidth > 1)
            code.push "def #{regName}(index1, index0, qword) :\n\treturn ((0x%07X * index1)" % stride1 + " + (0x%07X * index0) + (qword * 8)" % stride0 + " +  0x%07X + #{amName}_BASE);\n" % regOffset
        else
            code.push "def #{regName}(index1, index0) :\n\treturn ((0x%07X * index1)" % stride1 + " + (0x%07X * index0)" % stride0 + " + 0x%07X + #{amName}_BASE);\n" % regOffset
        end        
    end
    if(nDims == 1)
        code.push "#{regName}_ENTRIES".ljust(74)+"= %d;" % entries0  
        if(regWidth > 1)
            code.push "def #{regName}(index, qword) :\n\treturn ((0x%07X * index) + (qword * 8)" % stride0 + " + 0x%07X + #{amName}_BASE);\n" % regOffset
        else
            code.push "def #{regName}(index) = :\n\treturn ((0x%07X * index)" % stride0 + " + 0x%07X + #{amName}_BASE);\n" % regOffset
        end 
    end
    if(nDims == 0)
        if(regWidth > 1)
            code.push "def #{regName}(qword) :\n\treturn ((qword * 8) + 0x%07X + #{amName}_BASE);\n" % regOffset
        else
            code.push "def #{regName}() :\n\treturn (0x%07X + #{amName}_BASE);\n" % regOffset
        end
    end
    code.push "def #{regName}_GET_DEFAULT() :\n\treturn \"0x%X\";\n" % default
    code.push ""
    return code
  end
  ...
  def printRegField(code, regName, fld)
    ...
    if fld.lsb == fld.msb
      code.push "#{regName}_b_#{fldname}".ljust(74)+"= %d;" % fld.lsb
    else
      code.push "#{regName}_l_#{fldname}".ljust(74)+"= %d;" % fld.lsb
      code.push "#{regName}_h_#{fldname}".ljust(74)+"= %d;" % fld.msb
    end
    return code
  end
  ...
  def printRegVersion(code)
    ...
    if reg_ver != nil and reg_tag != nil
      ...
      code.push "FPPS_REG_VERSION".ljust(69)+"= %s;" % reg_ver
      code.push "FPPS_REG_TAG".ljust(68)+"= %s;" % reg_tag
      code.push ""
    end
  end

  def pythonRegParamsApi5()
    code = Array.new
    nDims = 0;
    stride0 = 0;
    stride1 = 0;

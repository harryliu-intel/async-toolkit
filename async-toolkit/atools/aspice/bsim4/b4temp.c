...
int BSIM4_TempModel(BSIM4model *model) {
  ...
  double delTemp, Temp, TRatio, Vtm0, Tnom;
	 ...
	 delTemp = model->temp - model->BSIM4tnom;
...
int BSIM4_TempInstance(BSIM4model *model, BSIM4instance *here, int warn) {
  struct bsim4SizeDependParam *pParam=NULL;
  ...
  pParam = here->pParam;
		  ...
		  pParam->NFinger = here->BSIM4nf;
	          ...
	          {   IFuid namarray[2];
                      ...
                      if (warn) fprintf(stderr,"WARNING: mosfet %s, model %s: Effective channel length <= 0",namarray[0],namarray[1]);
                      return(E_BADPARM);
                  }
	          ...
	          {   IFuid namarray[2];
                      namarray[0] = model->BSIM4modName;
                      namarray[1] = here->BSIM4name;
                      if (warn) fprintf(stderr,"WARNING: mosfet %s, model %s: Effective channel width <= 0",
	          ...
	          {   IFuid namarray[2];
                      namarray[0] = model->BSIM4modName;
                      namarray[1] = here->BSIM4name;
                      if (warn) fprintf(stderr,"WARNING: mosfet %s, model %s: Effective channel length for C-V <= 0",
                              namarray[0],namarray[1]);
                      return(E_BADPARM);
                  }
	          ...
	          {   IFuid namarray[2];
                      namarray[0] = model->BSIM4modName;
                      namarray[1] = here->BSIM4name;
                      if (warn) fprintf(stderr,"WARNING: mosfet %s, model %s: Effective channel width for C-V <= 0",
                              namarray[0],namarray[1]);
                      return(E_BADPARM);
                  }
                  ...
                  {   IFuid namarray[2];
                      namarray[0] = model->BSIM4modName;
                      namarray[1] = here->BSIM4name;
                      if (warn) fprintf(stderr,"WARNING: mosfet %s, model %s: Effective channel width for S/D junctions <= 0",
                              namarray[0],namarray[1]);
                      return(E_BADPARM);
                  }
		  ...
		  if (T1 < 0.0)
		  {   T1 = 0.0;
                    if (warn) fprintf(stderr, "WARNING: Rdw at current temperature is negative; set to 0.\n");
		  }
		  if (T2 < 0.0)
                  {   T2 = 0.0;
                    if (warn) fprintf(stderr, "WARNING: Rdwmin at current temperature is negative; set to 0.\n");
                  }
		  pParam->BSIM4rd0 = T1 / PowWeffWr;
                  pParam->BSIM4rdwmin = T2 / PowWeffWr;
                  if (T3 < 0.0)
                  {   T3 = 0.0;
                    if (warn) fprintf(stderr, "WARNING: Rsw at current temperature is negative; set to 0.\n");
                  }
                  if (T4 < 0.0)
                  {   T4 = 0.0;
                    if (warn) fprintf(stderr, "WARNING: Rswmin at current temperature is negative; set to 0.\n");
                  }
                  ...
                  {   pParam->BSIM4eu = 0.0;
                    fprintf(stderr, "WARNING: eu has been negative; reset to 0.0.\n");
		  }
		  ...
		  if (pParam->BSIM4ngate > 0.0)
	          ...
	          {   if (!model->BSIM4k1Given)
	              {   if (warn) fprintf(stderr, "WARNING: k1 should be specified with k2.\n");
                          pParam->BSIM4k1 = 0.53;
                      }
                      if (!model->BSIM4k2Given)
	              {   if (warn) fprintf(stderr, "WARNING: k2 should be specified with k1.\n");
                          pParam->BSIM4k2 = -0.0186;
                      }
                      if (model->BSIM4nsubGiven)
                          if (warn) fprintf(stderr, "WARNING: nsub is ignored because k1 or k2 is given.\n");
                      if (model->BSIM4xtGiven)
                          if (warn) fprintf(stderr, "WARNING: xt is ignored because k1 or k2 is given.\n");
                      if (model->BSIM4vbxGiven)
                          if (warn) fprintf(stderr, "WARNING: vbx is ignored because k1 or k2 is given.\n");
                      if (model->BSIM4gamma1Given)
                          if (warn) fprintf(stderr, "WARNING: gamma1 is ignored because k1 or k2 is given.\n");
                      if (model->BSIM4gamma2Given)
                          if (warn) fprintf(stderr, "WARNING: gamma2 is ignored because k1 or k2 is given.\n");
                  }
                  else
	          {   if (!model->BSIM4vbxGiven)
		  ...
		  /* stress effect */
	          ...
	          {   if (warn) fprintf(stderr, "WARNING: WLOD = %g is less than 0. 0.0 is used\n",model->BSIM4wlod);
               	      wlod = 0.0;
	          }
		  ...
		  if (model->BSIM4kvsat < -1.0 )
	          {   if (warn) fprintf(stderr, "WARNING: KVSAT = %g is too small; -1.0 is used.\n",model->BSIM4kvsat);
	       	      kvsat = -1.0;
            	  }
            	  if (model->BSIM4kvsat > 1.0)
            	  {   if (warn) fprintf(stderr, "WARNING: KVSAT = %g is too big; 1.0 is used.\n",model->BSIM4kvsat);
           	      kvsat = 1.0;
            	  }
		...
		{   if((here->BSIM4scGiven) && (here->BSIM4sc > 0.0) )
			...
			here->BSIM4scb = ( (0.1 * here->BSIM4sc + 0.01 * model->BSIM4scref) 
					...
					* exp(-10.0 * T1 * T2) ) / Wdrn;
                        ...
                        if (warn) fprintf(stderr, "WARNING: No WPE as none of SCA, SCB, SCC, SC is given and/or SC not positive.\n");
		    }
		}
		...
		if (T3 <= 0.0) 
		{ 	T3 = 0.0;
                  if (warn) fprintf(stderr, "WARNING: ku0we = %g is negatively too high. Negative mobility! \n", pParam->BSIM4ku0we);
		}
                here->BSIM4u0temp *= T3; 
              }
	      ...
	      if(here->BSIM4rbodyMod == 2)
		{
		  ...
		  if (here->BSIM4rgateMod != 0)
                    if (warn) fprintf(stderr, "WARNING: The gate conductance reset to 1.0e3 mho.\n");
              }
	      ...
	      /* Processing S/D resistance and conductance below */
              if(here->BSIM4sNodePrime != here->BSIM4sNode)
              {
                 ...
                 else
                 {
                     ...
                     // if (warn) fprintf (stderr, "WARNING: Source conductance reset to 1.0e3 mho.\n");
                 }
              } else
              {
                  here->BSIM4sourceConductance = 0.0;
              ...
              if(here->BSIM4dNodePrime != here->BSIM4dNode)
              {
                 ...
                 else
                 {
                     ...
                     // if (warn) fprintf (stderr, "WARNING: Drain conductance reset to 1.0e3 mho.\n");
                  }
              } else
              {
                  here->BSIM4drainConductance = 0.0;
			  ...
			  if (T2 < 1.0)
			  {   T2 = 10.0;
			      if (warn) fprintf(stderr, "WARNING: ijthsrev too small and set to 10 times IsbSat.\n");
			  } 
			  ...
			  here->BSIM4IVjsmRev = SourceSatCurrent * (1.0 + T1);
                          ...
                          break;
                  default:
                    if (warn) fprintf(stderr, "WARNING: Specified dioMod = %d not matched\n", model->BSIM4dioMod);
                  }
              }
              ...
              {   switch(model->BSIM4dioMod)
                  {   case 0:
                      ...
                      case 2:
                          ...
                          if (T2 < 1.0) 
                          {   T2 = 10.0;
                              if (warn) fprintf(stderr, "WARNING: ijthdrev too small and set to 10 times IdbSat.\n");
                          }

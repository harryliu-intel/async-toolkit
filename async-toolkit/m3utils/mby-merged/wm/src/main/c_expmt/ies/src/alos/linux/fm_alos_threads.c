 ...
 *
 *****************************************************************************/
static fm_thread *fmGetCurrentThreadState()
{
    fm_status  err;
                         ...
                         fm_thread *   thread)
{
    ...
    FM_LOG_ENTRY(FM_LOG_CAT_ALOS_THREAD, "name=%s qs=%d "
                                         "argument=%p handle=%p\n",
    ...
    {
        FM_LOG_EXIT(FM_LOG_CAT_ALOS_THREAD, FM_ERR_NO_MEM);
    }

    /* create waiting lock */
    if ( ( err = fmCreateLock(threadName, &thread->waiter) ) != FM_OK )
    {
        /* cleanup first! */
    ...
    if ( ( err = fmEventQueueInitialize(&thread->events,
                                        eventQueueSize,
                                        threadName) ) != FM_OK )
    {
        /* cleanup first! */
    ...
    if ((pthread_condattr_init(&attr) != 0) )
    {
        ...
        FM_LOG_EXIT(FM_LOG_CAT_ALOS_THREAD, FM_ERR_LOCK_INIT);
    }
    ...
    if ( ( posixError = pthread_cond_init(thread->cond, &attr) ) != 0 )
    {
        ...
        FM_LOG_EXIT(FM_LOG_CAT_ALOS_THREAD, FM_ERR_UNABLE_TO_CREATE_COND);
    }
    ...
    if (posixError != 0)
    {
        ...
        FM_LOG_EXIT(FM_LOG_CAT_ALOS_THREAD, FM_ERR_UNABLE_TO_CREATE_THREAD);
    }

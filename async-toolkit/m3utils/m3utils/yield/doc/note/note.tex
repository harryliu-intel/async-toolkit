...
\end{equation}
and $\Gamma$ is Euler's gamma function.\footnote{Note that Stapper uses opposite terminology to many authors.  Some authors would say that Stapper's Gamma distribution is parameterized over $(k,\theta)$ where $k=\alpha$ and $\theta=`/\beta$.  In this document, we shall consistently use Stapper's version, as the literature on semiconductor yield tends to follow his example.}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
And furthermore, we have everywhere that $F<1$ and $\Pi<1$.
...
integrand than in (\ref{eq:final}) but since we do not need to compute $\Pi'(D)$ it can be a computational win.

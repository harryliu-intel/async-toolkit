...
.PRECIOUS: $(SPICE_DIR)/alint/%/cell.paths_top.50
.PRECIOUS: $(SPICE_DIR)/alint/%/cell.paths_top.52
...
$(SPICE_DIR)/alint/%/cell.slack : \
    ...
    $(SPICE_DIR)/alint/%/cell.paths_top.50 \
    $(SPICE_DIR)/alint/%/cell.paths_top.52
	getslack '$(@D)/cell.timing' '${@D}/cell.paths_top.50' 50 > '${@D}/cell.slack'
	getslack '$(@D)/cell.timing' '${@D}/cell.paths_top.52' 52 >> '${@D}/cell.slack'
...
$(eval $(call PATHS_TOP,50,32e-12,,--noFloorplan))
$(eval $(call PATHS_TOP,52,33.28e-12,,--noFloorplan))
...
$(eval $(call PATHS_TOP,50,32e-12,$(INSTANCE_DIR)/.instances,--layoutRoot=$(INSTANCE_DIR)))
$(eval $(call PATHS_TOP,52,33.28e-12,$(INSTANCE_DIR)/.instances,--layoutRoot=$(INSTANCE_DIR)))
...
$(SPICE_DIR)/%/lib.done: $(foreach a_num, $(ALINT_BIN_ENUM), $(SPICE_DIR)/%/lib_parallel/lib.done.$(a_num))
	    ...
	    cat $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/lib_parallel/lib.err.$(a_num)') >  '$(@D)/lib.err'
	    touch '$@'
...
$(SPICE_DIR)/alint/%/lib_parallel/lib.raw.$(1) : \
	...
	if [ "$(1)" = "0" -a -f '$$(@D)/lib.in.$(1)' -a ! -s '$$(@D)/lib.in.$(1)' ]; then \
	    ...
	    $$(EXEC_LOW_PACKAGE) $$(RAWIFY) --task=lib \
...
$(SPICE_DIR)/%/alint.done: $(foreach a_num, $(ALINT_BIN_ENUM), $(SPICE_DIR)/%/alint_parallel/alint.done.$(a_num))
	    ...
	    cat $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/alint_parallel/alint.err.$(a_num)') >  '$(@D)/alint.err'
	    touch '$@'
...
$(SPICE_DIR)/%/alint_parallel/alint.done.$(1) $(SPICE_DIR)/%/alint_parallel/alint.raw.$(1) : \
	...
	if ! ( grep -q SUCCESS '$$(word 9, $$(^))'); then cp '$$(word 9, $$(^))' '$$(@D)/alint.done.$(1)'; touch '$$(@D)/alint.raw.$(1)'; exit; fi; \
	cd '$$(dir $$(@D))' && \
	  if [[ -s '$$(@D)/alint.in.$(1)' ]]; then \
	    ...
	    $$(EXEC_PACKAGE) $$(RAWIFY) --task=alint \
	    --root='$$(ROOT_TARGET_DIR)' --dir='$$(@D)' \
	    --cell="$$(call GET_CAST_FULL_NAME,$$(@D))" \
	    --mode="$$(call GET_SPICE_TYPE,$$(@D))" \
      ...
      --static-switchV=$$(STATIC_SWITCH_V) \
	    --local-props='$$<' \
...
$(SPICE_DIR)/%/alint_parallel/alint.done.$(REDONODES) $(SPICE_DIR)/%/alint_parallel/alint.raw.$(REDONODES) : \
	...
	if ! ( grep -q SUCCESS '$(word 9, $(^))'); then cp '$(word 9, $(^))' '$(@D)/alint.done.$(REDONODES)'; touch '$(@D)/alint.raw.$(REDONODES)'; exit; fi; \
	cd '$(dir $(@D))' && \
...
$(SPICE_DIR)/alint/%/alint_parallel/alint.raw.$(1) : \
	...
	if [ "$(1)" = "0" -a -f '$$(@D)/alint.in.$(1)' -a ! -s '$$(@D)/alint.in.$(1)' \
	    ...
	    $$(EXEC_LOW_PACKAGE) $$(RAWIFY) --task=alint \
	    --root='$$(ROOT_TARGET_DIR)' --dir='$$(@D)' \
	    --cell="$$(call GET_CAST_FULL_NAME,$$(@D))" \
	    --mode="$$(call GET_SPICE_TYPE,$$(@D))" \
	    --view-mode-order="$$(VIEW_MODE_ORDER)" \
...
$(SPICE_DIR)/alint/%/alint.raw $(SPICE_DIR)/alint/%/directives.0 $(SPICE_DIR)/alint/%/directives.cap:\
	...
	for node in `echo "$(REDONODES)" | sed -e 's/:/ /g'`; do \
	  ...
	  for bin in  $(ALINT_BIN_ENUM); do \
	    ...
	    if [ -d "$$dir/alint.bin.$$bin" ]; then cat "$$dir/alint.bin.$$bin/"*/directives.0 >> "$$dir/directives.0"; fi; \
	  done; \
	done; \
...
$(SPICE_DIR)/alint/%/alint.raw $(SPICE_DIR)/alint/%/directives.0 $(SPICE_DIR)/alint/%/directives.cap:\
	...
	cat $(foreach a_num, $(ALINT_BIN_ENUM) , '$(@D)/alint_parallel/alint.raw.$(a_num)') > '$(@D)/alint.raw';
	...
	raw2db --root='$(ROOT_TARGET_DIR)' '$(@D)/alint.raw'
	( for bin in $(ALINT_BIN_ENUM); do \
	   infile="$(@D)/alint_parallel/alint.in.$$bin"; \
	   ncnt=0; fcnt=0; \
	   for node in `awk '/^alint / {print $$2}' "$$infile"`; do \
	      let ncnt=$$ncnt+1; \
	      if [[ -r "$(@D)/alint.bin.$$bin/$$node/directives.0" ]]; then \
	         cat "$(@D)/alint.bin.$$bin/$$node/directives.0";\
	         let fcnt=$$fcnt+1; \
	      fi;\
	     done;\
	...
	if [[ ! -s '$(@D)/directives.0' ]]; then \
	  for node in `awk '{print $$1}' "$(@D)/../../../../../../cell.localprops$(ROUTED_SUFFIX)$(ACCURATE_SUFFIX)"`; do \
	      if [[ -r "$(@D)/$$node/directives.0" ]]; then cat "$(@D)/$$node/directives.0"; fi; done > '${@D}/directives.0'; \
	fi; \
	mode='$(call GET_SPICE_TYPE,$(@D))'; \
	dir=measured_cap; \
	[[ "$$mode" == nogeometry || "$$mode" == estimated ]] && dir=estimated_cap; \
	sed -n 's/^PASS.* node=\([^ ]\+\).* cap=\([^ ]\+\).*/      '"$$dir"'(\1) = \2;/p' '$(@D)/alint.raw' > '$(@D)/directives.cap'; \
	cat $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/alint_parallel/alint.err.$(a_num)') >  '$(@D)/alint.err'
	/bin/rm -f $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/alint_parallel/alint.err.$(a_num)')
	clean=$(CLEAN_TRACE); \
	if [[ "$$clean" == 1 ]]; then \
	  if ! ( grep -q FAIL '$(@D)/alint.raw' ) ; then \
	    cellname=$$(echo '$(call GET_CAST_FULL_NAME,$(@D))'); \
  ...
  exit; fi; \
	...
	cd '$$(dir $$(@D))' && \
      ...
      --node-props='$$(word 3, $$(^))' \
	    ...
	    /bin/rm -f '$$(@D)/alint_PO.out.$(1)' '$$(@D)/alint_PO.err.$(1)' '$$(@D)/threshresp.result.$(1)'; \
	    /bin/rm -rf "$$(@D)/../alint_PO.bin.$(1)"; \
  ...
  exit; fi; \
	...
	cd '$$(dir $$(@D))' && \
      ...
      --node-props='$$(word 3, $$(^))' \
	    ...
	    /bin/rm -f '$$(@D)/alint_aspice.out.$(1)' '$$(@D)/alint_aspice.err.$(1)'; \
	    /bin/rm -rf "$$(@D)/../alint.bin.$(1)"; \
...
$(SPICE_DIR)/alint/%/threshresp.result:\
	...
	$(MERGE_ALINT_OUT) --postfix='_aspice' --num-of-bins=$(MAX_ALINT_BIN_NUM) --alint-dir='$(@D)'; \
	  cat $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/alint_parallel/alint_aspice.err.$(a_num)') >  '$(@D)/alint_aspice.err'; \
	/bin/rm -f $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/alint_parallel/alint_aspice.err.$(a_num)'); \
  fi;
...
$(SPICE_DIR)/alint/%/lib.raw:\
	...
	cat $(foreach a_num, $(ALINT_BIN_ENUM) , '$(@D)/lib_parallel/lib.raw.$(a_num)') > '$(@D)/lib.raw'
	...
	raw2db --root='$(ROOT_TARGET_DIR)' '$(@D)/lib.raw'
	$(MERGE_ALINT_OUT) --num-of-bins=$(MAX_ALINT_BIN_NUM) --alint-dir='$(@D)' --mode=lib
	cat $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/lib_parallel/lib.err.$(a_num)') >  '$(@D)/lib.err'
	/bin/rm -f $(foreach a_num, $(ALINT_BIN_ENUM), '$(@D)/lib_parallel/lib.err.$(a_num)')
	touch '$(@D)/'lib.done
...
$(SPICE_DIR)/alint/%/asta.raw : $(foreach tau,$(ASTA_TAU),$(SPICE_DIR)/alint/%/asta/$(tau)/flat.violations)
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=asta \
	    --root='$(ROOT_TARGET_DIR)' --dir='$(@D)' \
	    --cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	    --mode="$(call GET_SPICE_TYPE,$(@D))" \
...
$(SPICE_DIR)/alint/%/slint.raw : $(SPICE_DIR)/alint/%/slint/violations.all
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=slint \
	    --root='$(ROOT_TARGET_DIR)' --dir='$(@D)' \
	    --cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	    --mode="$(call GET_SPICE_TYPE,$(@D))" && \
	$(CASTFILES_DEQUEUE_TASK) && \
...
$(SPICE_DIR)/aspice/$(ENV)/%/aspice.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=aspice \
	--root='$(ROOT_TARGET_DIR)' --dir='$(@D)' \
	--cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	--mode="$(call GET_SPICE_TYPE,$(@D))" \
	--env-ntpc='$<'; \
...
$(SPICE_DIR)/hsim/$(ENV)/%/hsim.out: $(SPICE_DIR)/cell.spice_hsim \
	...
	qb $(HSIM) \
	  ...
	  --rc-reduction=$$reduce \
	  --minC=$(MINC) --minR=$(MINR) \
	  --start-time=$(START_TIME) --run-time=$$time --process-corner=$$corner --vdd=$$true \
	  --measure-nodes="$$nodes" \
	  --delete=0 \
...
$(SPICE_DIR)/hsim/$(ENV)/%/hsim.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=hsim \
...
$(SPICE_DIR)/xa/$(ENV)/%/xa.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=xa \
	--root='$(ROOT_TARGET_DIR)' \
	--dir='$(@D)' \
	--cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	--mode="$(call GET_SPICE_TYPE,$(@D))" \
...
$(SPICE_DIR)/hspice/$(ENV)/%/hspice.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=hspice \
	--root='$(ROOT_TARGET_DIR)' \
	--dir='$(@D)' \
	--cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	--mode="$(call GET_SPICE_TYPE,$(@D))" \
...
$(SPICE_DIR)/hsim/$(ENV)/%/totem.out: $(SPICE_DIR)/cell.spf \
	...
	work_dir2=`mktemp -d "$(WORKING_DIR)/totem.XXXXXX"`; \
	nodes=$$($(GNUGAWK) '{print $$7}' '$(word 3,$^)' | sort -u | \
		$(RENAME) --type=node --from=cast --to=gds2 | \
		$(GNUGAWK) '{print "Xenv.Xtest." $$1}' | \
	        tr '\n' ',' | $(GNUSED) 's/,$$//' ); \
...
$(SPICE_DIR)/hsim/$(ENV)/%/totem.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=totem \
	--root='$(ROOT_TARGET_DIR)' \
	--dir='$(@D)' \
	--cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	--env-ntpc='$<' \
...
$(SPICE_DIR)/rte/%/$(ENV)/rte.raw: $(SPICE_DIR)/rte/%/$(ENV)/rte.out 
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --task=rte \
	--root='$(ROOT_TARGET_DIR)' \
	--dir='$(@D)' \
	--cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	--mode="$(call GET_SPICE_TYPE,$(@D))" \

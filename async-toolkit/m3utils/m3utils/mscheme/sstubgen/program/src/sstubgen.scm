...
(define (spit-out intf-name proc-stubs converters env)
	(cons
	 (spit-out-intf intf-name proc-stubs (map car converters) env)
	 (spit-out-impl intf-name proc-stubs (map cdr converters) env)
	 )
)
...
(define (spit-out-impl intf-name proc-stubs converter-impls env)
	(let ((imports ((env 'get 'imports) 'copy)))
			...
			(let* ((sig (extract-field 'sig m))
					...
					(let ((vars (map 
						...
						(string-append
						 ...
						 "      " (if have-return "RETURN " "") m3tn"."name"("
						           (infixize (cons '__object vars) ", ") ")" dnl
				...
				(string-append
				 ...
				 (format-parenthesized-signature sig 
																				 env 
																				 (string-append
																					"__object : " surrogate-type-name)) 
         " = " dnl
				 "  BEGIN" dnl
				 "    IF __object." (scheme-slot-name m) " = NIL THEN" dnl
				 (format-punt-call)
				 "    ELSE" dnl
...
(define (make-standard-stuff intf-name)
	...
	(set! global-env (env-type 'new))
	(write-files intf-name the-procs global-env))

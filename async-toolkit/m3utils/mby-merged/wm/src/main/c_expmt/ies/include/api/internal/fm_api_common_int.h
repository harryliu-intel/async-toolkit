...
#define FM_LOG_PORT_USE_FTYPE_NORMAL     0
...
/* FIXME: add platform capability to return max # supported focalpoints, then
 *   use that value? */
...
#define ACL_PORT_SET_OUT_OF_BOUNDS(sw, portSet) \
    ( ( (portSet) < 0 ) || \
      ( (portSet) >= (fmRootApi->fmSwitchStateTable[(sw)]->maxACLPortSets) ) )
...
#define VALIDATE_ACL_PORT_SET_ID(sw, portSet)   \
    if ( ACL_PORT_SET_OUT_OF_BOUNDS(sw, portSet) )  \
    {                                           \
        if (swProtected)                        \
        {                                       \
            UNPROTECT_SWITCH(sw);               \
        }                                       \
        return FM_ERR_INVALID_ACL_PORT_SET;     \
    }

#define VALIDATE_PORT_MIRROR_GROUP(sw, grp)                                 \
    if ( ( (grp) < 0 ) ||                                                   \
        ( (grp) >= fmRootApi->fmSwitchStateTable[(sw)]->mirrorTableSize ) ) \
    {                                                                       \
        if (swProtected)                                                    \
        {                                                                   \
            UNPROTECT_SWITCH(sw);                                           \
...
extern fm_macaddr fmGetPacketDestAddr(fm_int sw, fm_buffer *pkt);
extern fm_macaddr fmGetPacketSrcAddr(fm_int sw, fm_buffer *pkt);

...
$$(SPICE_DIR)/alint/%/cell.paths_top.$(1) : $(3)
	#TASK=budget$(1) VIEW=$$(call GET_VIEW,$$(@D) CELL=$$(call GET_CAST_FULL_NAME,$$(@D))
...
ifeq ($(call GET_EXTRACT_MODE,$(CELL_DIR),1),nogeometry)
$(foreach tau,$(BUDGET_TAU),$(eval $(call PATHS_TOP,$(word 1,$(subst :, ,$(tau))),$(word 2,$(subst :, ,$(tau))),,--noFloorplan)))
else # nogeometry
INSTANCE_DIR := $(CELL_DIR)/$(call GET_EXTRACT_MODE,$(CELL_DIR))/instances
...
define COMMON_ASP
	...
	echo ".true=$$true;"; \
	if [[ $(call GET_EXTRACT_MODE,$(@D)) == accurate ]] ; then \
	  echo ".accurate=1;"; \
	else \
	  echo ".accurate=0;"; \
	fi; \
...
$(SPICE_DIR)/%/lib_parallel/lib.done.$(1) $(SPICE_DIR)/%/lib_parallel/lib.raw.$(1): \
	...
	export ASPICE_CORNER=$$(call GET_CORNER,$$(@D)) && \
	cd '$$(dir $$(@D))' && \
	  if [[ -s '$$(@D)/lib.in.$(1)' ]]; then \
	    ...
	    -seed $$(call GET_SEED,$$(call GET_SEED,$$(@D))) \
...
$(SPICE_DIR)/alint/%/slint.raw : $(SPICE_DIR)/alint/%/slint/violations.all
	...
	$(EXEC_LOW_PACKAGE) $(RAWIFY) --use-db=$(USEDB) --task=slint \
	    --root='$(ROOT_TARGET_DIR)' --dir='$(@D)' \
	    --cell="$(call GET_CAST_FULL_NAME,$(@D))" \
	    --mode="$(call GET_EXTRACT_MODE,$(@D))"  $(LVE_RAW_CONFIG) && \
	$(CASTFILES_DEQUEUE_TASK) && \
	touch '$@'
...
$(SPICE_DIR)/aspice/$(ENV)/%/aspice.in: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV).nodes	
	...
	$(CASTFILES_ENQUEUE_TASK) && \
	min=$(call GET_ASPICE_MIN,$(@D)); \
	max=$(call GET_ASPICE_MAX,$(@D)); \
	time=$(call GET_TIME,$(@D)); \
	$(MAKE_ASPICE_IN) "$$time" '$<' '$@' \
...
$(SPICE_DIR)/aspice/$(ENV)/%/aspice.done: \
	...
	#TASK=aspice ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
...
$(SPICE_DIR)/aspice/$(ENV)/%/aspice.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
					 $(SPICE_DIR)/aspice/$(ENV)/%/aspice.done
	#TASK=aspice_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
...
$(SPICE_DIR)/hsim/$(ENV)/%/hsim.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	#TASK=hsim_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \
...
$(SPICE_DIR)/xa/$(ENV)/%/xa.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	#TASK=xa_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \
...
$(SPICE_DIR)/hspice/$(ENV)/%/hspice.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	#TASK=hspice_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \
...
$(SPICE_DIR)/hsim/$(ENV)/%/totem.out: $(SPICE_DIR)/cell.spf \
			             ...
			             $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
			             $(CELL_DIR)/cell.portprops
	#TASK=totem ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	...
	QB_DIAG_FILE='$@.diag' QB_RUN_NAME='lve_totem' \
	  ...
	  --temp=$(call GET_TEMP,$(@D)) \
	  --dynamic-simulation-time=$(call GET_TIME,$(@D)) \
	  --work-dir="$$work_dir" \
	  $(PRS_DELAY) \
	  $(CAP_LOAD) \
	  --port-props='$(word 4,$^)' \
...
$(SPICE_DIR)/hsim/$(ENV)/%/cmm.out: $(SPICE_DIR)/hsim/$(ENV)/%/totem.out \
			             $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
			             $(CELL_DIR)/cell.portprops
	#TASK=cmm ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
...
$(SPICE_DIR)/hsim/$(ENV)/%/totem.raw: $(CELL_DIR)/jflat$(ROUTED_SUFFIX)/env-ntpc/$(ENV) \
	...
	#TASK=totem_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \
...
$(SPICE_DIR)/rte/%/$(ENV)/rte.out:  $(SPICE_DIR)/alint/%/plt.raw  
	#TASK=rte ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \
	mkdir -p '$(@D)'; \
...
$(SPICE_DIR)/rte/%/$(ENV)/rte.raw: $(SPICE_DIR)/rte/%/$(ENV)/rte.out 
	#TASK=rte_raw ENV=$(call GET_ENV,$(@D)) CELL=$(call GET_CAST_FULL_NAME,$(@D))
	$(CASTFILES_ENQUEUE_TASK) && \

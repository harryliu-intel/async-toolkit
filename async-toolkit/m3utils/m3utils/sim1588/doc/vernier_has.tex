...
asserted), errors will be signalled on {\tt o\_vernier\_error}.  Error
conditions detected include the following:
\begin{itemize}
...
periods $p_A={\tt i\_num}$ and $p_B={\tt i\_denom}$.
...
\end{equation}
it is the smallest member  $N \in {\mathcal S}$ that satisfies
\begin{equation}\label{eq:divconstraint}
  2 \, {\tt i\_num} < ( {\tt i\_denom} - {\tt i\_denom\_error} ) N
\end{equation}
If there is no smallest member $N \in {\mathcal S}$ that satisfies
equation~\ref{eq:divconstraint}, the TSU does not initialize and
instead raises {\tt o\_vernier\_error}.

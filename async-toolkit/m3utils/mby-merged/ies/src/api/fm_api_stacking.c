 ...
 *
 *****************************************************************************/
fm_status StackingPreamble(fm_int sw, 
                           fm_bool lockSwitch, 
                           fm_switch **switchPtr, 
 ...
 *
 *****************************************************************************/
fm_status StackingPostamble(fm_int sw, 
                            fm_bool lockSwitch, 
                            ...
                            fm_status retError)
{
    ...
    FM_LOG_ENTRY(FM_LOG_CAT_STACKING, "sw=%d, preambleError=%d retError=%d\n", 
 ...
 *
 *****************************************************************************/
fm_int CompareForwardingRules(const void *a, const void *b)
{
 ...
 *
 *****************************************************************************/
void DestroyForwardingRule(void *ptr)
{
...
fm_status fmInitStacking(fm_int sw)
{
    ...
    fm_stackingInfo *   stackingInfo = &switchPtr->stackingInfo;
...
fm_status fmSetStackGlortRange(fm_int sw, fm_uint32 glortBase, fm_uint32 mask)
{
    ...
    /* FIXME: This could be moved to platform so we know exactly how 
 ...
 *                  down state. Set a switch down with ''fmSetSwitchState''.
 *
 * \note            After calling this function, the conventional functions
 *                  for creating LAGs (''fmCreateLAG'' and ''fmCreateLAGExt''),
 *                  load balancing groups (''fmCreateLBG'') and multicast
 *                  groups (''fmCreateMcastGroup'') will create those
 *                  entities using the glorts specified in the glortRange
 *                  argument to this function and they will be private to
 *                  the switch. The Stacking API version of those functions, 
                                      ...
                                      fm_forwardRule *rule)
{
    ...
    FM_LOG_DEBUG(FM_LOG_CAT_STACKING, "Inserted rule at %p into tree with key %d\n", 
                                  ...
                                  fm_int mcastGroup)
{
    ...
    err = fmCreateMcastGroupInt(sw, &mcastGroup, TRUE);
...
*                   multicast group and it's associated logical port/glort,
 *                  i.e., for RPF failure groups, specify FM_HANDLE_NONE

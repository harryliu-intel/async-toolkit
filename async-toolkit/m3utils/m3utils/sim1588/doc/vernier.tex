...
The important observation is that for suitably chosen $p_A$, $p_B$, it is the case that $GCD(p_A) \ll p_A, p_B$.  The technique is normally credited to Pierre Vernier, who published it in 1631\cite{vernier-orig}, and it is commonly used
in the construction of calipers with $p_A=1000\mu$m and $p_B=900\mu$m so that the precision is $GCD(p_A,p_B)=100\mu$m, yet the instrument can easily be read by the unaided eye.
\rotins_epsfig_listcaption{4in}{Principle_vernier}{Illustration of operation of vernier calipers.  Source: Wikipedia.}{270}{Vernier calipers.}

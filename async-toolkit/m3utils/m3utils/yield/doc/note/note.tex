...
We write the Poisson system yield $Y_S(D)$ and compute the expected
system yield across the process ensemble,
\begin{equation}\label{eq:ensemble-yield}
  <Y_S> = \int_0^\infty Y_S(D) f(D) dD
\end{equation}
where each $Y_S(D)$ is straightforward to compute using the binomial formula.  We have implemented this
...
section~\ref{sec:discussion}, (\ref{eq:starred}).  We see that when
...
inefficient: at present it is not clearly more efficient to evaluate
this integral than to use the polynomial formalism we have developed

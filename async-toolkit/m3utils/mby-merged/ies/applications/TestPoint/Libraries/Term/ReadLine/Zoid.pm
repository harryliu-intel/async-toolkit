...
#		escape	=> 'switch_mode_command',       # modif Fulcrum 01/26/2011
		ctrl_R	=> 'switch_mode_isearch',
#		ctrl__	=> 'switch_mode_fbrowse',       # modif Fulcrum 01/26/2011
		right	=> 'forward_char',
		ctrl_F	=> 'forward_char',
		left	=> 'backward_char',
...
#		ctrl_P	=> 'previous_history',			# modif Fulcrum 01/26/2011
		down	=> 'next_history',
		...
		ctrl_K	=> 'kill_line',
#		ctrl_W	=> 'unix_word_rubout',			# modif Fulcrum 01/26/2011
		tab	=> 'complete',
...
sub readline {
	...
	# skip title() and new_line() to avoid Ansi Esc Sequences pollution.
#	$self->title($title);
#	$self->new_line();
...
sub complete {
	...
	# modif Fulcrum 02/09/2011
	# if @compl contients one or more options that do not include the current
	# last word, keep always this last one (whether or not the last word is
	# followed by a space).
	#
	if (@compl && length $compl == 0)
	{
		$compl = $word;
	}

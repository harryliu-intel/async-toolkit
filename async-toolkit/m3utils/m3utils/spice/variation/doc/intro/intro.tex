...
continuous random variable $X$ is described by its {\em probability
  density function (p.d.f.)\/} $phi_X$ with the property that
\begin{equation}
  Pr(a \le x \lt b) = \int_a^b \phi_X(x) \, dx \quad .
\end{equation}
The p.d.f.\ has many interesting elementary properties, for example:
...
\begin{equation}
  \mu = \int{-\infty}^{+\infty} x \phi(x) ,\ dx \quad ,
\end{equation}
and similarly, the variance
\begin{equation}
  \begin{array}{rl}
    \sigma^2_X &= \int{-\infty}^{+\infty} (x-\mu)^2 \phi(x) ,\ dx \\
               &= \int{-\infty}^{+\infty} x^2 \phi(x) ,\ dx - \mu^2 
  \end{array}
\end{equation}


The integrability of p.d.f.s is a big and advanced field of
mathematics and mostly out of the scope of our discussion.

\def\normal{\mathcal{N}}

\subsection{The normal distribution}
An example of a p.d.f.\ is the normal distribution $\normal$
\begin{equation}
  \phi_{\normal(\mu,\sigma^2)}(x)=
  \normal(\mu,\sigma^2)(x)   =
  {1 \over \sigma \sqrt{2 \pi} } e ^ { -{1 \over 2} \left( { x - \mu \over \sigma } \right) ^2 } \quad .
\end{equation}
Much of what will interest us will specifically not be normally distributed,
but we shall still use the properties of the normal distribution as a guide.

\subsection{Cumulative density function}
If we consider the one-sided integral of the p.d.f.\ of $X$, we call
this the {\em cumulative density function (c.d.f.)\/} $\Phi_X$:
\begin{equation}
  \Phi_X(x) = \int_{-\infty}^x \phi_X(x') \, dx'
\end{equation}
where we use $x'$ for the dummy integration variable for clarity.

The complement of $\Phi$ we call the {\em survival function\/} $S_X$:
\begin{equation}
  S_X(x) = 1 - \Phi_X(x) = \int_{x}^{+\infty} \phi_X(x') \, dx'
\end{equation}

The half-way point of $\Phi_X(x)$, i.e., the point at which it equals $1/2$, we
call the median and denote $\tilde{X}$:
\begin{equation}
{1 \over 2} = \Phi_X(\tilde{X}) = S_X(\tilde{X})
\end{equation}

By equation~\ref{eq:pdfprops}, the c.d.f.\ $\Phi$ integrates to 1 over
...
\end{equation}
and also the complement of the error function
\begin{equation}
  \erfc(x) = 1 - \erf(x) 
\end{equation}
...
Given two independent random variables $X ~ \phi_X(x)$ and $Y ~ \phi_Y(y)$, the sum $Z = X + Y$ is distributed as the convolution of the two
\begin{equation}
  \phi_Z(z) = \int_{-\infty}^{\infty}  \phi_X(x)\phi_Y(z - x) \, dx = \phi_X(z) * \phi_Y(z) \quad .
\end{equation}
Computationally this means that the Fourier transform of $\phi$ is interesting,
...
If we are given $X ~ \phi_X(x)$ and we wish to know the distribution of a
sum of $M$ i.i.d.\ instances of $X$, let's say $Z = X_0 + X_1 + \cdots + X_{M-1}$
then we can write this as the $M$-th convolutional power of $\phi_X$
\begin{equation}
  \phi_Z(x) = \phi_X^{*M}(x) \quad .
\end{equation}

\subsection{The maximum of a set of random variables}
Given $X ~ \phi_X$ and $Y ~ \phi_Y$, and $Z = \max(X, Y)$, we have that
\begin{equation}
  \Phi_Z(z) = \Phi_X(z) \Phi_Y(z) \quad .
\end{equation}

For the special case $X_0, X_1, \ldots , X_{N-1} ~ \phi_X$ and $Z = \max(X_0, X_1, \ldots , X_{N-1})$, 
\begin{equation}
  \Phi_Z(z) = \Phi_X^N(z) \quad .
\end{equation}
A convenient measure of the maximum of distributions is their median $\tilde{z}$ because
\begin{equation}
  \Phi_Z(\tilde{z}) = {1 \over 2}
\end{equation}
so that
\begin{equation}
\tilde{z} = \Phi^{-1}(2^-N) \quad .
\end{equation}
...
since for large $N$
\begin{equation}
  2^{-N} \approx 1 + {\ln 2 \over N}
\end{equation}
we have that ({\bf fill this in})
\begin{equation}
\tilde{z} = \erfc^{-1} \ldots 
\end{equation}
...
consists of $N$ identical speed paths, each of which is comprised of
...
running a circuit simulator on a test circuit description and
performing a physical measurement on the circuit).  We can, however,
...
and may have asymptotes to $+\infty$ near the surface of the ball.  Because
$\Delta$ represents the physical delay of a circuit, it is also bounded

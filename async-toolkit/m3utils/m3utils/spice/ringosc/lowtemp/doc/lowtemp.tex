...
\begin{equation}\label{eq:ctot}
  C_{tot} = C_E + C_{Si} = {\pi_E E \over \eta_E } + {\pi_{Si} A \over \Lambda f \eta_{Si}} \quad .
\end{equation}
If we now consider the locus of a fixed overall cost per operation $C_{tot} = C_0$,
...
An example is shown in figure~\ref{fig:ulvtlvt}.  The $E$-$f$ curves
...
\item $\pi_Si = \$360,000/{\mathrm m}^2$ silicon cost
\item Overall silicon cost efficiency $\eta_{Si} = \eta_E = 1/3$
...
If we consider the total expenses to be $C_{tot}$ as in equation~\ref{eq:ctot},
we can write the proportion of expense on silicon as
\begin{align*}
  \rho_{Si} &= {C_{Si}  \over C_{tot}} \\
...
For our study, we choose R717, that is, anhydrous ammonia, as the refrigerant.  This
is a readily available and inexpensive refrigerant with few
environmental problems, widely used for industrial refrigeration
projects.  We consider a ``hot side'' of an ambient temperature of
313K or +40$^\circ$C and consider two different target temperatures
for our datacenter: 233K (-40$^\circ$C) and 275K (+2$^\circ$C).
...
294K (+19$^\circ$C), 5K superheat.  Raw $\beta$ (COP) of main cycle: 6.748; $\beta$
including subcooling: 6.320; $\beta$ including 90\% compressor efficiency:
5.686.
...
power) at lower temperatures will work in the opposite direction.
...
\bibitem{dennard}{R.~H.~Dennard, F.~H.~Gaensslen, H.-N.~Yu, V.~L.~Rideout, E.~Bassous, and A.~R.~LeBlanc. Design of ion-implanted MOSFETs with very small physical dimensions.  {\it JSSC}. {\bf SC-9} (5): 256–268, October 1974.}

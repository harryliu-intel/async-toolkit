...
public class Utility {
    ...
    public static String possibleUnderscoreToLowerCase(String in) {
        String out = "";

        Matcher m = Pattern.compile("\\p{Lower}\\p{Upper}").matcher(in);
        int index = 0;
        while(m.find()) {
            out += in.substring(index, m.start()+1).toLowerCase() + "_";
            index = m.end()-1;
        }
        out += in.substring(index).toLowerCase();

...
\section{Dense data structures in systems programming languages}\label{sec:dense_system}
...
occasion to use:
\begin{itemize}
\item The (byte) offset of any member of a given {\tt RECORD} type from the beginning address of that {\tt RECORD} is always the same, no matter what the context of the declaration.

\item The (byte) stride of any {\tt ARRAY} type is always constant, regardless of how complex the type of the {\tt ARRAY} element is.
\end{itemize}
In other words, if we know the byte offset of a {\tt RECORD} member
from the beginning of its {\tt RECORD} and if we know the byte delta
...
\section{Dense data structures in hardware models}
...
Languages such as RDL provide finer control over addressing (i.e., user control
over padding rules) than do most software environments, but while these controls
certainly change the addresses that are generated, they do not in fact destroy
the two properties of section~\ref{sec:denseproperties}.

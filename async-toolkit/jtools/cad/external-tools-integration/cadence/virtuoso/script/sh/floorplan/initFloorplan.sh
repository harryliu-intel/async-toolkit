...
# $Id: //depot/sw/main/cad/external-tools-integration/cadence/virtuoso/script/sh/nano/initFloorplan.sh#4 $
# $DateTime: 2006/08/31 09:01:40 $
# $Author: aubrey $
...
function usage() {
  ...
  echo "  [ --cadence-log=initFloorplan.log]"
  echo "  [ --view=layout ]"
  echo "  [ --verbose ]"
  echo "  [ --max-heap-size=1800 ]"
  echo "  [ --64 ]"
}

layout=`mywhich layoutPlus` || myexit "$layout" 2
sedcmd=`mywhich sed` || myexit "$sedcmd" 2
gawkcmd=`mywhich gawk` || myexit "$gawkcmd" 2
grepcmd=`mywhich grep` || myexit "$grepcmd" 2
bashcmd=`mywhich bash` || myexit "$bashcmd" 2
findcmd=`mywhich find` || myexit "$findcmd" 2
...
check_for_empty_arg "$cell"                                                  \
    "The cell that you want to process must be specified."                                   2
...
check_for_empty_arg "$view"                                                      \
    "You must specify the view of \"$cell\" that you want to process."              2
...
cdsWD=`mktemp -d $workingDir/initFloorplan.XXXXXX`

if [ -n "$verbose" ] ; then
  echo "Making cadence working directory."
  echo "$mkcdswd \"--dfII-dir=$dfIIDir\" \"--fulcrum-pdk-root=$fulcrumPDKRoot\" \"--cast-path=$castPath\" \"--target-dir=$cdsWD\" \"--force\" "
fi
...
mainIL=`mktemp $workingDir/initFloorplan.XXXXXX`

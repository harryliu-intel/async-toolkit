...
while (<SF>) {
   ...
   ($m7) = $_ =~ /.*i_md_delay_fifo_rd_data.*md_delay_fifo_rd_data\[1:0\]\[\(IGR_PPE_MD_WIDTH\+IGR_PPE_MD_ECC_WIDTH\)-1:0\].*/;
   ($m8) = $_ =~ /.*o_q_md_delay_fifo_wr_data.*md_delay_fifo_wr_data\[1:0\]\[\(IGR_PPE_MD_WIDTH\+IGR_PPE_MD_ECC_WIDTH\)-1:0\].*/;
   if($m1) {
   ...
   if($m7) {
      $_ =~ s/md_delay_fifo_rd_data\[1:0\]\[\(IGR_PPE_MD_WIDTH\+IGR_PPE_MD_ECC_WIDTH\)-1:0\]/md_delay_fifo_rd_data/;
   }
   if($m8) {
      $_ =~ s/md_delay_fifo_wr_data\[1:0\]\[\(IGR_PPE_MD_WIDTH\+IGR_PPE_MD_ECC_WIDTH\)-1:0\]/md_delay_fifo_wr_data/;
   }
   printf TF ("%s",$_);
}
close (TF);

...
sub new {
	...
	$self->{_lintDir}   = "$self->{_options}{Ace::UDF::get_base_scope()}{-model_compile_results_dir}/lint";
...
#VR
	...
	$self->{_lintDir}   = "$self->{_options}{Ace::UDF::get_base_scope()}{-results}/lint";
...
sub create_scrag {
  ...
  SWITCH: {
    /^lint$/ && do {
	my $exectest_scrag;
	$exectest_scrag .= $self->_create_lint_scrag(%args);
...
#  my $lint_opts = join(' ', @{$self->{_options}->{$base_scope}->{-lint_tool_opts}});
  ...
  my $modelRoot = $ENV{TLM1_ROOT};
  my $modelRootStr = 'TLM1_ROOT';
  ...
  return $self->_usage(%args) if ($self->{_options}->{$base_scope}->{-lint_help} == 1);
  $self->_create_res_dir(%args) unless($self->{_options}->{$base_scope}->{-lint_show_libs});
  my @Tlibs = ();
  push(@Tlibs, @$model_libs);
  $args{run_libs} = \@Tlibs;

  foreach my $lib (@Tlibs) {
    next if(exists $lint_ignore{$lib} && $lint_ignore{$lib});
    if(exists $hdlspec->{_datahash}->{$cur_scope}->{libs}->{$lib}->{-preCompiled}) {
      $lint_ignore{$lib} = 1;
    } else {
	...
	# if super lib contain local hdl spec add it to run lib list
	my $Hdl_Spec_lib = $hdlspec->{_datahash}->{$cur_scope}->{libs}->{$lib}->{-hdl_spec};
	if(ref ($Hdl_Spec_lib) eq 'ARRAY' && scalar @{$Hdl_Spec_lib}) {
	...
	push(@{$super{Run_libs}}, $lib);
      ...
      if($lint_tool eq 'spyglass' && !$self->{_options}->{$base_scope}->{-lint_show_libs}) {
	  $self->_create_mapfile(%args, $lib);
      }
      $self->{_Dlibs} = [];
    }
  }
  if($self->{_options}->{$base_scope}->{-lint_show_libs}) {
    $self->_show_libs_structure(%args);
    print "\n$model MODEL LIBS:\n---------------\n".join('  ', @$model_libs)."\n\n" ;
    print "total ".scalar @$model_libs." libs\n";
    if(scalar keys %lint_ignore) {
      print "\nignore libs: ";
      foreach my $ignore_lib (keys %lint_ignore) { print "$ignore_lib "; }
      print "\n";
    }
    return;
  } 
  ...
  $self->_create_prj_file(%args) if($lint_project_mode);
  $self->_create_elab_mapfile(%args);
#  $self->_check_mapfiles(%args);
  $cmd = Ace::GenericScrag::chdir("$self->{_lintDir}/$lint_tool");
  #########################
  ##Create Spglass Scrag
  #########################
  switch ($lint_tool) {
        case 'spyglass' {
	  if($lint_project_mode) {
	      my $show_goals = $self->{_options}->{$base_scope}->{-lint_show_goals} ? 1 : 0;
	      $cmd .= "spyglass -project ";
	      my $batch = $lint_gui ? "" : "-batch";
	      $lint_goal =~ s/ /,/g;
	      if($show_goals) {
		  $cmd .= "${lint_top}_elab.prj -batch -showgoals\n";
	      } elsif($lint_build_only) {
		  $cmd .= "pre_comp.prj $batch -designread";
	      } else {
		  $cmd .= "${lint_top}_elab.prj $batch -goal $lint_goal";
	      }
	      unless($show_goals) {
		  $cmd .= " > /dev/null 2>&1\n";
		  my $post_cmd = "$modelRoot/bin/spyglass_post_comp.pl -project ".
		      ($lint_build_only ? "pre_comp " : "${lint_top}_elab -goal $lint_goal ").
		      "-top_module $lint_top ".
		      "-res_path $self->{_results} ".
		      "-max_warning $lint_max_warnings ";
		  $cmd .= "$post_cmd\n";
		  $cmd .= "checkrc \"$post_cmd\"\n";
	      }
	      if($lint_license_check && !$show_goals) {
		  $args{cmd} = $cmd;
		  $cmd = $self->_check_spyglass_license(%args);
	      }
	      return $cmd;
	  } else {
#	  $cmd .= "echo \"\nIgnore Libs: $lint_ignore\n\"\n";
		  ...
		  foreach my $dlib ( @{$super{$lib}{dep_libs}} ) {
			  ...
			  foreach my $subLib (@{$hdlspec->{_datahash}->{$cur_scope}->{libs}->{$lib}->{-sub_libs}}) {
				  ...
				  $dcmd = "spyglass -batch -f $super{sub_libs}{$subLib}{super_lib}_S_${subLib}_filelist.f".
				      ((-e "$self->{_lintDir}/$lint_tool/$super{sub_libs}{$subLib}{super_lib}_map.f")?" -f $super{sub_libs}{$subLib}{super_lib}_map.f":"").		      
			  ...
			  if($self->_check_filelist(%args, filelist => "${dlib}_filelist.f")) {
				  ...
				  ((-e "$self->{_lintDir}/$lint_tool/${lib}_map.f")?" -f ${lib}_map.f":"").
				  ((-e "$self->{_lintDir}/$lint_tool/${dlib}_map.f")?" -f ${dlib}_map.f":"").
		  ...
		  if(exists $super{sub_libs}{$lib} && exists $super{sub_libs}{$lib}{super_lib} && 
		     defined $super{sub_libs}{$lib}{super_lib} && $self->_check_filelist(%args, filelist => "$super{sub_libs}{$lib}{super_lib}_S_${lib}_filelist.f") ) {
			  ...
			  ((-e "$self->{_lintDir}/$lint_tool/$super{sub_libs}{$lib}{super_lib}_map.f")?" -f $super{sub_libs}{$lib}{super_lib}_map.f":"").
...
#	  $cmd .= "$modelRoot/bin/spyglass_post_comp.pl -res_path $self->{_results} -dc -run_libs \"@{$super{Run_libs}}\"\n";
	      ...
	      my @Map_files = `/bin/ls $self->{_lintDir}/$lint_tool/\*map.f`;
	      ...
	      my $waive_files = "";
	      my %lint_wave_file = $self->get_array_option('-lint_waive_file');
	      for my $fub (keys %lint_wave_file) {
		  if($fub eq 'global' or $fub eq $lint_top) {
		      foreach my $file (@{$lint_wave_file{$fub}}) {
			  my $find_file = $self->find_file($file, $base_scope);
			  $waive_files .= " -waiver $find_file ";
		      }
		  }
	      }
	      # -gui=sde -run
      ...
      ###### PUT LINTRA CMD CODE HERE: 
      case 'lintra' {
	  print "\n*** Info: LINTRA SUPPORT IS UNDER CONSTRUCTION ****\n\n";
      }
      else { print "\n*** Error: undefined \'-lint_tool\' option $lint_tool should be spyglass or lintra ****\n\n"; }
  }
}
#-------------------------------------------------------------------------------
...
sub _create_res_dir {
  ...
  my $cmd;
  if(!$args{lint_elab_only} && -d "$self->{_lintDir}/$args{lint_tool}") {
      if(!$demo && $args{lint_build_only}) {
	  aprint "===> Removing existing lint results ($args{lint_tool})\n";
	  $cmd = "rm -Rf $self->{_lintDir}/$args{lint_tool}";
	  Utilities::System::execute_cmd($cmd, -quiet=>$quiet); #, -demo=>$demo);
      } else {
	  aprint "===> Removing existing lint results files ($args{lint_tool})\n";
	  $cmd = "rm $self->{_lintDir}/$args{lint_tool}/\* > /dev/null 2>&1 ";
	  Utilities::System::execute_cmd($cmd, -quiet=>$quiet);
      }
  }
  unless(-d "$self->{_lintDir}/$args{lint_tool}") {  
      $cmd = "mkdir -p $self->{_lintDir}/$args{lint_tool}";
      Utilities::System::execute_cmd($cmd, -quiet=>$quiet);
  }
}
#-------------------------------------------------------------------------------
...
sub _create_filelist {
  ...
  foreach my $lib (@{$super{Run_libs}}) {
      ...
      $fh->open("> $self->{_lintDir}/$args{lint_tool}/$super{sub_libs}{$lib}{super_lib}_S_${lib}_filelist.f") ||
	die "can\'t open  $self->{_lintDir}/$args{lint_tool}/$super{sub_libs}{$lib}{super_lib}_S_${lib}_filelist.f for writing. $!";
    } else {
      $fh->open("> $self->{_lintDir}/$args{lint_tool}/${lib}_filelist.f") ||
	die "can\'t open  $self->{_lintDir}/$args{lint_tool}/${lib}_filelist.f for writing. $!";
    }
    my $filelist = {};
    # hdl handling
    ###############
    ...
    push(@{$filelist->{-vlog_opts}}, "-enableSV\n");
...
sub _create_mapfile {
  my $lib = pop(@_);
  my ($self, %args) = @_;
  return if(-e "$self->{_lintDir}/$args{lint_tool}/${lib}_map.f");
  my $fh = new FileHandle;
  return if(
  ...
  $fh->open("> $self->{_lintDir}/$args{lint_tool}/${lib}_map.f") ||
      die "can't open  $self->{_lintDir}/$args{lint_tool}/${lib}_map.f for writing ?: $!";
  if(exists $super{$lib}{dep_libs} && ref $super{$lib}{dep_libs} eq 'ARRAY') {
      foreach my $dlib (@{$super{$lib}{dep_libs}}) {
	  next if(exists $args{lint_ignore}->{$dlib} && $args{lint_ignore}->{$dlib});
	  print $fh "-lib $dlib $self->{_lintDir}/$args{lint_tool}/$dlib\n";
      }
  }
  if(exists $args{hdlspec}->{_datahash}->{$args{cur_scope}}->{libs}->{$lib}->{-sub_libs} && 
     ref $args{hdlspec}->{_datahash}->{$args{cur_scope}}->{libs}->{$lib}->{-sub_libs} eq 'ARRAY') {
	  ...
	  print $fh "-lib $subLib $self->{_lintDir}/$args{lint_tool}/$lib\n";
      }
      print $fh "-lib $lib $self->{_lintDir}/$args{lint_tool}/$lib\n";
  }
  ##AG print $fh "-lib SPY_DW_WORK ".$self->{_lintDir}."/".$args{lint_tool}."/SPY_DW_WORK\n";
  $fh->close;
}
#--------------------------------------------------------------------------------
...
sub _create_elab_mapfile {
  my ($self, %args) = @_;
  my $fh = new FileHandle;
  my $lint_top = exists $args{lint_top} ? $args{lint_top} :
    "/////.......NO LINT TOP ......//////";
  $fh->open("> $self->{_lintDir}/$args{lint_tool}/${lint_top}_elab_map.f") ||
    die "can\'t open  $self->{_lintDir}/$args{lint_tool}/${lint_top}_elab_map.f for writing ?: $!";
  foreach my $rlib (@{$args{run_libs}}) {
    next if(exists $args{lint_ignore}->{$rlib} && $args{lint_ignore}->{$rlib});
    print $fh "-lib $rlib $self->{_lintDir}/$args{lint_tool}/$rlib\n";
  }
  ##AG print $fh "-lib SPY_DW_WORK ".$self->{_lintDir}."/".$args{lint_tool}."/SPY_DW_WORK\n";
  $fh->close;
}
...
sub _show_libs_structure {
  ...
  print "\n$args{cur_scope} lint libs structure:\n";
  print   "-----------------------\n";
...
sub _check_waive_file {
  my ($self, %args) = @_;
  my $waive_fh = new FileHandle;
  my $err = 0;
  foreach my $file (@{$self->{_options}->{$args{base_scope}}->{-lint_waive_file}}) {
      my $f_location = $self->find_file($file, $args{base_scope});
      $waive_fh->open("< $f_location") || die "can\'t open $f_location for reading. $!";
      while(!$err and my $line = $waive_fh->getline) {
	  next unless($line =~ /\s+\-rule\s+\"(\S+)\"/);
	  $err = 1;
      }
      $waive_fh->close;
      last if($err);
  ...
  if($err) {
      print "Error: waiver file contains illegal rule waiver\n";
      return 1;
  } else {
      return 0;
  }
...
sub _check_mapfiles {
    ...
    my @Map_files = `/bin/ls $self->{_lintDir}/$args{lint_tool}/\*_map.f`;
...
#	    print $fh_new $line if($self->_check_filelist(%args, filelist => "${lib}_filelist.f") );
	...
	##AGprint $fh_new "-lib SPY_DW_WORK ".$self->{_lintDir}."/".$args{lint_tool}."/SPY_DW_WORK\n";
	$fh->close;
	$fh_new->close;
...
sub _check_filelist {
  ...
  if(-e "$self->{_lintDir}/$args{lint_tool}/$args{filelist}") {
    $fh->open("< $self->{_lintDir}/$args{lint_tool}/$args{filelist}") || die "can\'t open $args{filelist} for reading. $!";
    while(my $line = $fh->getline) {
...
sub _check_spyglass_license {
  my ($self, %args) = @_;
  my @Commands = split(/\n/, $args{cmd});
  my $max_idx = scalar @Commands;
  my @License_CMD = ();
  my $idx = 0;
  my $sh_cmd;
  my $lib;
  my $process = "compilation";
  while($idx < $max_idx) {
      $sh_cmd = $Commands[$idx];
      if( $sh_cmd =~ /^\s*spyglass\s+.*-noelab/ || 
	  $sh_cmd =~ /^\s*spyglass\s+.+_elab_wdir/ || 
	  $sh_cmd =~ /^\s*spyglass\s+.*-project/ ) {
	  if($sh_cmd =~ /^\s*spyglass\s+.*-noelab/) {
	      ($lib) = ($sh_cmd =~ /(\S+)_filelist.f/);
	  } else {
	      ($lib) = ($sh_cmd =~ /^\s*spyglass\s+.*\-wdir\s+(\S+)_elab_wdir/);
	      $process = "elaboration"
	  }
	  my $lic_chk = $sh_cmd;
	  $lic_chk .= "
...
while [[ \$stat == 2 ]] ; do
  ...
  then
    sleep 20
    echo License Failure for $lib spyglass $process - retrying
    $sh_cmd
  fi
  perl -e \'
    \$status = system(\"$Commands[++${idx}] -quiet\");
    \$status = hex(\$status) >> 9;
    exit \$status
  \'
  stat=\$?
  ...
  then
    ...
    $Commands[${idx}]\n".
($args{lint_error_exit} ? "    $Commands[++${idx}]\n" : "").
"  fi
done\n\n";
	  push(@License_CMD, $lic_chk);  
      } else {
	  push(@License_CMD, $sh_cmd."\n");
      }
      $idx++;
  }
  my $lic_cmd = join('', @License_CMD);
  return $lic_cmd;
}
#--------------------------------------------------------------------------------
  ...
  foreach my $lib (@{$super{Run_libs}}) {
	  ...
	  foreach my $dlib (@{$super{$lib}{dep_libs}}) {
		  ...
		  unless(exists $stacked->{$dlib}) {
		      unshift(@Src_prj, "set_option libhdlf $dlib ${dlib}_filelist.f");
		      $stacked->{$dlib} = 1;
		  }
	      }
	  }
	  ...
	  unshift(@Src_prj, "set_option libhdlf $lib ${lib}_filelist.f");
	  $stacked->{$lib} = 1;
      }
  }
  # handle super libs
...
#      foreach my $runLib (@{$args{hdlspec}->{_datahash}->{$args{cur_scope}}->{libs}->{$supLib}->{-sub_libs}}) {
	  ...
	  if(exists $super{sub_libs}{$runLib}{super_lib} && $super{sub_libs}{$runLib}{super_lib} eq $supLib) {
		  ...
		  push(@Src_prj, "set_option libhdlf $supLib $super{sub_libs}{$runLib}{super_lib}_S_${runLib}_filelist.f");
	      } else {
#	print "DEBUG Error Can\'t find the sublib $runLib $super{sub_libs}{$runLib}{super_lib}_S_${runLib}_filelist.f file \n";
  ...
  my %lint_wave_file = $self->get_array_option('-lint_waive_file');
  for my $fub (keys %lint_wave_file) {
      if($fub eq 'global' or $fub eq $args{lint_top}) {
	  foreach my $file (@{$lint_wave_file{$fub}}) {
	      my $find_file = $self->find_file($file, $args{base_scope});
	      $waivers .= "read_file -type waiver $find_file\n";
	  }
      }
  }
  ...
  if(exists $self->{_options}->{$args{base_scope}}->{-lint_prj_tool_opts} && 
    ...
    foreach my $opt (@{$self->{_options}->{$args{base_scope}}->{-lint_prj_tool_opts}}) {
      ...
      next if(($args{lint_gui} || !$self->get_option(-lint_gen_audit_rpt)) 
	  && $opt =~ /set_option\s+report\s+{\s+Audit\s+}/);
      $prj_tool_opts .= "$opt\n";	  
    }
  }
  ...
  my $prj_file = $args{lint_build_only} ? "$self->{_lintDir}/$args{lint_tool}/pre_comp.prj" :
      "$self->{_lintDir}/$args{lint_tool}/$args{lint_top}_elab.prj";
  $fh->open("> $prj_file") ||
  ...
  my $topPrj_str = "read_file -type sourcelist $args{lint_top}_elab_map.f\n".
      ...
      (
       ...
       $waivers.
       "set_option current_methodology \$env($args{modelRootStr})/config/lint/spyglass/intel-methodology\n".
       $prj_spp_files.

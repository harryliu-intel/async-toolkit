                                     ...
                                     fm_int  vlanID)
{
    ...
    if ( (vlanID != -1) && VLAN_OUT_OF_BOUNDS(vlanID) )
    {
 ...
 *                  FM_MAX_STP_INSTANCE.  If the instance argument is NULL, then
 *                  a lookup is performed to find the instance structure.
 *
 * \param[in]       instance is the pointer to the instance structure representing
 *                  the instance.  If stpInstance is NULL, then this value is used
 *                  to skip the lookup.
 *
 * \param[in]       vlanID is the vlan to add, from 0 to FM_MAX_VLAN.
 *
 * \return          FM_OK if successful.
 ...
 *                  FM_MAX_STP_INSTANCE.
 *
 * \param[in]       vlanID is the vlan to delete, from 0 to FM_MAX_VLAN.
 *
 * \return          FM_OK if successful.
 * \return          FM_ERR_INVALID_STP_MODE if the mode is invalid.
 * \return          FM_ERR_INVALID_ARGUMENT if the instance is invalid.
...
fm_status fmInitStpInstanceTree(fm_switch *switchPtr)
{
    ...
    err = InitDefaultInstanceVlans(&instance->vlans);    
    FM_LOG_ABORT_ON_ERR(FM_LOG_CAT_STP, err);
...
fm_status fmResetMultipleSpanningTreeState(fm_int sw)
{
    ...
    err = StpInstancePreamble(sw, -1, -1);
 ...
 *                  returned. Use ''fmSetVlanPortState'' when
 *                  ''FM_SPANNING_TREE_MODE'' is set to any value other than
...
fm_status fmCreateSpanningTree(fm_int sw, fm_int stpInstance)
{
    ...
    err = StpInstancePreamble(sw, stpInstance, -1);
 ...
 *                  may range from 0 to ''FM_MAX_STP_INSTANCE''.
 *
                                ...
                                fm_int vlanID)
{
    ...
    err = StpInstancePreamble(sw, stpInstance, vlanID);
 ...
 *                  delete the VLAN and may range from 1 to 
 *                  ''FM_MAX_STP_INSTANCE''.
 *
                                    ...
                                    fm_int *instance)
{
    ...
    err = StpInstancePreamble(sw, -1, vlanID);
 ...
 * \param[in]       sw is the switch on which to operate.
 *
 * \param[in]       stpInstance is the spanning tree instance number and may
 *                  range from 0 to ''FM_MAX_STP_INSTANCE''.
 *
                                     ...
                                     fm_int stpState)
{
    ...
    if (fmIsInternalPort(sw, port))
    {
        ...
        if ( !fmGetBoolApiAttribute(FM_AAK_API_STP_ENABLE_INTERNAL_PORT_CTRL, 
                                     ...
                                     fm_int *stpState)
{
    ...
    /* FIXME: move this into the preamble */
    if ( !fmIsCardinalPort(sw, port) )
    {

    ...
    if [ -n "$celltargetdir" ] ; then
      ...
      if [[ -r "$path_to_edit" && -w "$path_to_edit" && -d "$path_to_edit" ]] ; then
        ...
        if [[ ! ( -r "$path_to_edit" && -f "$path_to_edit" ) ]] ; then
          echo "\"$path_to_edit\" is not a file or directory." 1>&2
          path_to_edit=
        fi
      fi

      if [ -n "$path_to_edit" ] ; then
        escaped_path_to_edit=`echo "$path_to_edit" | sed -e 's/#/%23/g'`
        curr_p4_sub_command="$p4_sub_command $escaped_path_to_edit"
        message "$cur_p4_sub_command"

...
if (preg_match(":^/nfs/site/disks/wdisk.\d+/:", $lveroot)) {
    $lveroot=preg_replace(":^/nfs/site/disks/wdisk.\d+/:", "", $lveroot);
}
...
function canon ($path) {
    $x=popen("cd $path; /p/rrc/tools/apache2/cgi-bin/cwd", "r");
    $p=fread($x, 2048);
    pclose($x);
    return(str_replace("\n", "", $p));
}
...
function shownewbumps ($bumps, $lveroot, $path, $task, $addresp) {
    ...
    $skip=array("status","node","cc","tau","bump_up","bump_dn","resp","additive_resp");
    foreach ($bumps as $line) {
        ...
        $bumpdir="unknown";
        $fail_type="pass";
        if (isset($ret["fail_type"])) {
            $fail_type="";
            foreach ($ret as $key=>$value) {
                if (!in_array($key,$skip)) $fail_type = "$fail_type<br>$key=$value";
            }
            $fail_type = substr($fail_type, 4);
        }
        $bump="&nbsp;";
        if (isset($ret["bump_dn"])) {
            $bump=intval(preg_replace("/@.*/", "", $ret["bump_dn"]));
            $bumpdir="dn";
        }
        elseif (isset($ret["bump_up"])) {
            $bump=intval(preg_replace("/@.*/", "", $ret["bump_up"]));
            $bumpdir="up";
        }
        $out="out";
        $err="err";
        ...
        $resp="&nbsp;";
        if (isset($ret["$resp_key"])) $resp=$ret["$resp_key"];
        print "<tr><td$style>$node<td>$out<td>$err<td>$cc<td>$tau<td>$bumpdir<td>$bump<td>$resp<td>$fail_type\n";
        $cnt++;
        if ($cnt > $limit) {
...
function getnewbumps ($lveroot,$path,$task, $addresp) {
    ...
    while ($fp !== false and ! feof($fp)) {
        $line=fgets($fp);
        $matchline=0;
        if (strstr($line, " $resp_key=") !== false ) $matchline=1;
...
function quicklink() {
    ...
    $md5=md5($q);
    $_SESSION[$md5]=$q;
    $qdb=new SQLite3("/p/rrc/tools/apache2/scratch/quick.sqlite");
    $success=0;
    if ($qdb) {

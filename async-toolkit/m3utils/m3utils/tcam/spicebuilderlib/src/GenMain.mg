...
GENERIC MODULE GenMain(TheSimulation);
...
[-m <model-name>] 
[-pm <probe-mode>] 
<simulation-params...> (* see TcamSimulation.m3 *)
*)
...
BEGIN
  ...
  TRY
    ...
    IF pp.keywordPresent("-corner") THEN
  ...
  *)
  ...
  TheSimulation.Build(pp, simParams, modelName);
  ...
  WITH wr    = FileWr.Open(outPfx & ".spice"),
    ...
    WITH ass =    SimDumper.AddDigitalModel(model, clk, aWr, simParams) DO
      SimDumper.FinishDump(wr, probeMode, ass, simParams, sim)
    END;

...
int BSIM4_CheckModel(BSIM4model *model, FILE *fplog) {
  ...
  /*** aspice limitations ***/
  if (model->BSIM4trnqsMod!=0)
    {
    fprintf(fplog,"ERROR: aspice requires TRNQSMOD=0\n");
    Fatal_Flag = 1;
    }
  if (model->BSIM4rbodyMod!=0)
    {
    fprintf(fplog,"ERROR: aspice requires RBODYMOD=0\n");
    Fatal_Flag = 1;
    }
  if (model->BSIM4rgateMod!=0)
    {
    fprintf(fplog,"ERROR: aspice requires RGATEMOD=0\n");
    Fatal_Flag = 1;
    }
...
/* Check overlap capacitance parameters */
	...
	{   fprintf(fplog, "Warning: Njts = %g is negative at temperature = %g.\n",
		    model->BSIM4njtstemp, model->temp);
	...
	{   fprintf(fplog, "Warning: Njtssw = %g is negative at temperature = %g.\n",
		    model->BSIM4njtsswtemp, model->temp);
	...
	{   fprintf(fplog, "Warning: Njtsswg = %g is negative at temperature = %g.\n",
		    model->BSIM4njtsswgtemp, model->temp);
...
/*** sanity check instance parameters ***/

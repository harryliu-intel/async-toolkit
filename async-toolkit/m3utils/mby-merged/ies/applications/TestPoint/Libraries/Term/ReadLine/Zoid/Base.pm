...
our @_key_buffer;
# modif Fulcrum 06/06/2011
# command line buffer.
our $currentLineBuffer;
our $changeEditMode;
...
#	"\e"   => 'escape',    # modif Fulcrum 01/26/2011
	"\cH"  => 'backspace',
	"\cI"  => 'tab',
	"\cJ"  => 'return',    # line feed
...
sub loop {
	...
	# modif Fulcrum 01/26/2011
	#  Use TermSize() to get the terminal size.
	...
	while ($$self{_loop}) {
		...
		if(defined($$self{_loop}) && $$self{_loop} > 0)
		{
			$self->draw();
		}
	}
...
sub read_key { die "deprecated warning" if $_[1];
	...
	# ('normal' causes some problems with the regression tests)
	#ReadMode('normal', $$self{IN});
	ReadMode('noecho', $$self{IN});
...
sub do_key {
  ...
  # modif Fulcrum 06/06/2011
  # Clear the current command line buffer after calling the help function
  # to force rewrite the full command line.
  my $re = ref($sub);
  if ($re) {
	  $sub->($self, $key, @_);
      clearCurrentBuffer();
...
#	print STDERR Dumper $lines, $pos;
	...
	# modif Fulcrum 06/06/2011
	# Force high values in $width & $higth to avoid problems with very long
	# command lines in the regression tests.
	$width = 500;
	$higth = 100;
...
#	print STDERR Dumper \@nlines, $pos;
	...
	# check if $currentLineBuffer is a prefix of the whole command line to be printed
	# If so, print only the incremental part.
	if (rindex($$lines[-1],$currentLineBuffer,0) == 0 &&
		$changeEditMode == 0 &&
        $$pos[0] >= length($$lines[-1]) )
	{
		# print only the incremental part of the command line
		my $incrementalStr = $$lines[-1];
		$incrementalStr = substr($incrementalStr, length($currentLineBuffer));
		print "$incrementalStr";
		$currentLineBuffer = $$lines[-1];
	}
	else
	{
...
sub TermSize
{
	# check if terminal width and hight values are forced by env. variables
	my ($width,$higth);

	$width = $ENV{'READLINE_TERMINAL_COLUMNS'};
	$higth = $ENV{'READLINE_TERMINAL_LINES'};

	# both values, width and higth, must be defined; 
	# otherwise get their values from the terminal
	if (!defined $width || !defined $higth) {
		($width,$higth) = (GetTerminalSize($_[0]{IN}))[0,1];
	}

	if ($width == 0)
	{
		# If the number of columns reported by GetTerminalSize is 0,
		#  then use a hardcoded value
		# The number of rows reamains equal to 0.
		$width = 80;
	}
	($width,$higth);
}
...
sub output {
	...
	# modif Fulcrum 01/26/2011
	#  validate that {term_size}[1] (terminal's row number) is greater than 0
	#if (@items > $$self{term_size}[1]) {
	if (@items > $$self{term_size}[1] && $$self{term_size}[1] > 0) {
	...
	# modif Fulcrum 06/06/2011
	# Clear the command line buffer (to force rewrite the whole command line) if
	# the autocomplete function returns more than 1 option.
	if ($cnt > 1) {
		clearCurrentBuffer();
	}
}
...
# Fulcrum modification 01-26-2011
#
#  It follows the original version of this method: 
#  sub cursor_at {print { $_[0]{OUT} } "\e[$_[2];$_[1]H" } # ($x, $y) 1-based !
#
# The original version uses both 'columns' and 'lines' information, to set the
# cursor position. 
# The Zoid library uses the terminal size to set the 'coloums' and
# 'lines' variable. When telnet is used, the terminal size is reported as 
# as (0;0) by ReadKey and as result, the cursor is placed at the wrong place. 
#
# The modified version of 'cursor_at' only uses the 'columns' parameter.
sub cursor_at
{
	...
	{
		$cursorBack = @ENV{'COLUMNS'};
	}

	# ignore the 'row' information.
	print { $$self{OUT}}"\e[$cursorBack"."D";
	if (--$column > 0)
	{
		print { $$self{OUT}} "\e[$column"."C";
	}
}
...
# modif Fulcrum 01/26/2011
# clear_screen()
# Added ANSI sequence "move cursor to home" (\e[0;0H)
# Original code:
#    sub clear_screen { print { $_[0]{OUT} } "\e[2J" }
#
sub clear_screen { print { $_[0]{OUT} } "\e[2J\e[0;0H" }

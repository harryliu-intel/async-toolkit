...
See the report~\cite{too}.  The TSU is called ``delay-calculating synchronizing flip-flop'' in the report.
...
While the TSU is enabled to operate ({\tt i\_vernier\_en} is asserted), errors will be signalled on {\tt o\_vernier\_error}.  Error conditions detected include the following:
\begin{itemize}
\item assertion of $B$ clock reset during operation
\item overflow of mark buffer (attempts to send marks at a higher rate
  than one per two $A$ cycles)
\end{itemize}

\section{Parameters}

The parameters listed in table~\ref{tab:params} are specified in instantiating the TSU soft
macro.
...
The parameter {\tt i\_denom\_error} deserves special mention.  It is
...
calculate the required divider, the allowable error in the $B$ clock
...

{
  \begin{table}
    \centering
    \begin{tabular}{|>{\tt}l|>{\tt}l|c|>{\it}c|>{\raggedright\arraybackslash}p{2in}|}
      \hline
      \bf Name \rule{0pt}{2.6ex}& \bf Type & \bf Dir & \bf Dom & {\bf Description} \\
      \hline
      ...
      o\_vernier\_error & logic & O & A & TSU has detected an error and outputs are idle until the next initalization cycle \\
      \hline
      \multicolumn{5}{|l|}{\bf CONFIGURATION INPUTS} \\
      \hline
      i\_num & logic[RAT\_PREC\_BITS-1:0] & I & A & period of $A$ clock in TUs.  Must remain stable while {\tt i\_vernier\_en} is asserted \\
      i\_denom & logic[RAT\_PREC\_BITS-1:0] & I & A & period of $B$ clock in TUs.  Must remain stable while {\tt i\_vernier\_en} is asserted \\
      i\_denom\_err & logic[RAT\_PREC\_BITS-1:0] & I & A & maximum error in period of $B$ clock (relative to $A$ clock) in TUs.  Must remain stable while {\tt i\_vernier\_en} is asserted \\
      \hline
      \multicolumn{5}{|l|}{\bf FUNCTIONAL INTERFACES} \\
      \hline
      i\_fclk\_mark & logic & I & B & indicated input mark \\
      i\_fclk\_mark\_phase & logic[RAT\_PREC\_BITS-1:0] & I & B & indicated input event age in TUs \\
      o\_mark & logic & O & A & indicated output mark \\
      o\_mark\_phase & logic[RAT\_PREC\_BITS-1:0] & O & A & indicated output event age in TUs \\
      \hline
    \end{tabular}
    \caption{Pinout of TSU RTL.}\label{tab:pinout}
  \end{table}
}
...
conversions.  (It is not guaranteed that the TSU will catch every attempt
...
All aspects of the performance of the TSU is related to the clock periods $p_A={\tt i\_num}$ and $p_B={\tt i\_denom}$.

\section{Initialization time}
...
\begin{equation}
  t_{init} \le LCM(p_A,p_B) + (2^{\tt FCLK\_DIV\_BITS}+10)\, {\tt RAT\_PREC\_BITS} \, p_A + {\tt SAMPLE\_TIMES}\, (2p_B + 2p_A)\quad .
\end{equation}
This expression arises from the following sources:
\begin{itemize}
\item Settling time of the vernier algorithm
\item Search for $B$ clock divisor
\item Computation of various constants (using a bit-at-a-time divider)
\item Synchronization time for initialization handshake between $A$ and $B$ clock domains (estimated at two complete handshakes between the clock domains)
\end{itemize}
...
maximum drift error (e.g., 100~p.p.m.~for a standard Ethernet clock), and $\xi$ is the vernier settling component of the
initialization time equal to $LCM(p_A,p_B)$ TUs, and $\iota$ is the clock jitter.
(When performing the summation, ensure all terms have the same units!)

\section{Mark Throughput}
The throughput of marks is strictly limited.  The $B$ clock is divided down
such that it is slower than twice the period of the $A$ clock.  The
algorithm for computing the divisor is as follows:

Choose $N$ as divisor such that out of the set

\begin{equation}
{\mathcal S} = \{ 1, 2, 4, \ldots , 2^{FCLK\_DIV\_BITS}-2\}
\end{equation}
such that it is the smallest member  $N \in {\mathcal S}$ that satisfies
\begin{equation}
  2 \, {\tt i\_num} < ( {\tt i\_denom} - {\tt i\_denom\_error} ) N
\end{equation}

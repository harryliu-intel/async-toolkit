...
function usage() {
  ...
  echo "  [ --view=layout]"
  echo "  [ --flatten-view=flatten ]"
  echo "  [ --power-grid-cellname=powerGridTieOff ]"
  echo "  [ --flatten-cells-list=flatten_cells_list ]"
  echo "  [ --cadence-log=createFlattenView.log]"
  echo "  [ --verbose ]"
  echo "  [ --64 ]"
}
...
powerGridCellName="powerGridTieOff"
flattenCellsList="flatten_cells_list"
cadenceName=
cell=
lib=
dfIIDir=
...
outputRootCellName=
cadenceLog="createFlattenView.log"
view="layout"
flattenView="flatten"
verbose=
debug=
...
for arg in $@ ; do
  ...
  --view=* )
    ...
    ;;
  --power-grid-cellname=* )
    powerGridCellName=`echo "$arg" | $sedcmd -e "s/--power-grid-cellname=//"`
    ;;
  --flatten-cells-list=* )
    flattenCellsList=`echo "$arg" | $sedcmd -e "s/--flatten-cells-list=//"`
    ;;
  --flatten-view=* )
    flattenView=`echo "$arg" | $sedcmd -e "s/--flatten-view=//"`
    ;;
  --verbose )
    verbose=1
    ;;
...
check_for_empty_arg "$cell"                                                  \
    "The cell that you want to flatten must be specified."                                   2
check_for_empty_arg "$lib"                                                  \
    "The library that you want to flatten must be specified."                                   2
...
check_for_empty_arg "$view"                                                      \
    "You must specify the view of \"$cell\" that you want to flatten out."              2
...
cdsWD=`mktemp -d $workingDir/flat.XXXXXX`
...
mainIL=`mktemp $workingDir/flat.XXXXXX`
...
( load "$skillAutoLoad" )
( load "$pdkInfoIL" )
CellView= dbOpenCellViewByType( "$lib" "$cell" "$view" )
flattenView=nrFlattenView( CellView "$flattenView" "$powerGridCellName" "$flattenCellsList"
                        ?Verbose $verbose
                        ?OverWrite t
                        ?DeleteWires t
                        ?DeleteVias t
                      )
Status = if( flattenView 1 0 )

...
static FILE *ingress_msg_fp = NULL;
static FILE  *egress_msg_fp = NULL;

fm_status openMsgDump(const msg_direction_t msg_dir, const char *msg_file)
{
    fm_status status = FM_OK;
    
    FILE *msg_fp = fopen(msg_file, "w");
    ...
    switch(msg_dir)
    {
    case INGRESS: ingress_msg_fp = msg_fp; break;
    case  EGRESS:  egress_msg_fp = msg_fp; break;
    }
...
fm_status writeMsgDump(const msg_direction_t msg_dir, fm_modelMessage *msg)
{
    ...
    switch(msg_dir)
    {
    case INGRESS: msg_fp = ingress_msg_fp; break;
    case  EGRESS: msg_fp = egress_msg_fp;  break;
    }

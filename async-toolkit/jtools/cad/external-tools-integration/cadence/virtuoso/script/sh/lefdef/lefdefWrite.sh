...
for arg in $@ ; do
  ...
  --working-dir=* )
    ...
    ;;
  --lef-output=* )
    lefOutput=`echo "$arg" | $sedcmd -e "s/--lef-output=//" -e "s=(=\\\\\\(=g" -e "s=)=\\\\\\)=g"`
    ;;
  --def-output=* )
    defOutput=`echo "$arg" | $sedcmd -e "s/--def-output=//" -e "s=(=\\\\\\(=g" -e "s=)=\\\\\\)=g"`
    ;;
  --cadence-log=* )
...
check_for_empty_arg "$view"                                                      \
    "You must specify the view of \"$rootCell\" that you want to lef & def out."              2
...
techonly_lef="$fulcrumPDKRoot/share/Fulcrum/lefdef/tech_only.lef"
check_readable_file "$techonly_lef"                                                 \
   "techonly.lef: \"$techonly_lef\' is not a readable file."                                2
...
defTempFile=`mktemp "$workingDir/def.XXXXXX"`
...
( load "$skillAutoLoad" )
( load "$pdkInfoIL" )
lefStatus=lefLefOut( "$libName" 
           "$rootCell"
           "$view"
           "$lefTempFile"
           ?Verbose $verbose)
defStatus=defDefOut( "$libName" 
           "$rootCell"
           "$view"
           "$defTempFile"
           ?Verbose $verbose)
Status = (cond ( !lefStatus 1 ) ( !defStatus 2 ) ( 0 ) )
...
cat $techonly_lef $lefTempFile > $lefOutput
mv $defTempFile $defOutput
EOF

...
)
{
    ...
    for (fm_int i = 0; i < 20/* Nmber of ports? */; i++) {
...
//  set_parser_ext_reg(0x0, 0x0, 0x0, 0, 0x6, &parser_map->PARSER_EXT[31][5]);
    ...
    for (fm_uint i = 0; i < 20/* Nmber of ports? */; i++) {
        mbyWriteReg(0, MBY_PARSER_PORT_CFG(i, 0x0), 0xc0e000078000000);
        mbyWriteReg(0, MBY_PARSER_CSUM_CFG(i, 0x0), 0x0);
    }
...
)
{
    ...
    for (fm_uint i = 0; i < 20/* Nmber of ports? */; i++) {
        mbyWriteReg(0, MBY_GLORT_DEST_TABLE(i, 0x0), (1 << i));
    }

    /* Flood GLORT allocation. */
    mbyWriteReg(0, MBY_GLORT_RAM(0x1, 0x0), 0x20100053);
    mbyWriteReg(0, MBY_GLORT_CAM(0x1, 0x0), 0xfb000400);
    mbyWriteReg(0, MBY_GLORT_DEST_TABLE(0x14, 0x0), 0x11fffff);
    mbyWriteReg(0, MBY_FLOOD_GLORT_TABLE(0x0, 0x0), 0x40004000400);

    for (fm_uint i = 0; i < 20/* Number of ports? */; i++) {
        mbyWriteReg(0, MBY_MAP_PORT_DEFAULT(i, 0x0, 0x0), 0x1ff52);
        mbyWriteReg(0, MBY_MAP_PORT_DEFAULT(i, 0x1, 0x0), 0x1ff0f);
        mbyWriteReg(0, MBY_MAP_PORT_DEFAULT(i, 0x2, 0x0), 0xff2c);
        mbyWriteReg(0, MBY_MAP_LEN_LIMIT(i, 0x0), 0x4ad);
        mbyWriteReg(0, MBY_MAP_PORT_CFG(i, 0x0), 0x2010 + (i * 0x20));
    }
#endif
}

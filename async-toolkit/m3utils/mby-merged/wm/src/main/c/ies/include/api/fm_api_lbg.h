 ...
 *                  API is not valid on FM2000 deviecs.
 *
 * INTEL CONFIDENTIAL
 ...
 *  \ingroup constSystem */
#define FM_LBG_DEFAULT_BINS_PER_DISTRIBUTION 512

/** The value to assign a bin to when it is unassigned */
#define FM_LBG_UNASSIGNED_BIN             (-1)
...
enum _fm_lbgAttr
{
    /** Type fm_int:  Deprecated. Setting this attribute has no effect.
     *
     *  \chips  FM3000, FM4000 */
    FM_LBG_VLAN,
     ...
     *  mode is ''FM_LBG_MODE_MAPPED''.  This attribute is write-only.
     *                                                                  \lb\lb
     *  If an ACL is actively routing traffic to an LBG and the distribution 
...
{
    /** Indicates that the port is active and can forward traffic */
    FM_LBG_PORT_ACTIVE = 0,

    /** Indicates that the port is down or otherwise unavailable.  In 
     *  certain modes, this may result in this port's traffic being
     *  redirected to other ports.
     */
    FM_LBG_PORT_FAILOVER,

    /** Indicates that this port normally does not forward traffic, but is
     *  available for use if other ports become unavailable.  */
    FM_LBG_PORT_STANDBY,

    /** Indicates that this port is not available for use */

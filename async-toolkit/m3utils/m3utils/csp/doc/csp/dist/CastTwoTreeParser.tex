...
bodyStatementList
        :                #( BODY_STATEMENT_LIST ( bodyStatementenv, globalP, cell, envContainer, paramEnv, prsEnv,
...
block
        :                #( BLOCK bodyStatementListenv, false, cell, envContainer, paramEnv, prsEnv,
                                 subcellsEnv, subtypesEnv ) 
        ;
    
    
variableDeclarationStatement
        :                #( VAR_DECL id:IDENT  varName = generateName(id.getText());  ( INLINE  inlineP = true;  | ) ( FLATTEN  flattenP = true;  | ) typeenv, cell, envContainer, HierName.makeHierName(varName),
...
arraySelector
        :                #( arrayExpr:ARRAY_SUB ( Range r; rangeenv, false  if (r.getMin() > r.getMax())
...
assertExclStatement
        :   (                    #( hi:EXCLHI assertExclNodesenv  hiLo = ExclusiveNodeSet.HI; errorAST=hi;  ) 
            |                    #( lo:EXCLLO assertExclNodesenv  hiLo = ExclusiveNodeSet.LO; errorAST=lo;  ) 
            |                    #( cc:EXCLCC assertExclNodesenv  hiLo = ExclusiveNodeSet.CC; errorAST=cc;  ) 
            |                    #( nocc:NOCC assertExclNodesenv  hiLo = ExclusiveNodeSet.NOCC; errorAST=nocc;  ) 
            ) 
...
portParam
        :                #( VAR_DECL id:IDENT ( PLUS if (reverseDirection) in=true; else out=true; | ) ( MINUS if (reverseDirection) out=true; else in=true; | ) typeenv, cell, null, HierName.makeHierName(id.getText()), false,
...
envExtraPortIdent
        ...
        |                #( ARRAY_ACCESS envExtraPortIdentenv, buf  buf.append('[');  subtypesArraySelectorenv, buf  buf.append(']');  ) 
        ;
    
    
subtypesArraySelector
        :                #( ARRAY_SUB subtypesRangeenv, buf (  buf.append(',');  subtypesRangeenv, buf )* ) 
        ;
...
partialExtractExpr
        :                #( FIELD_ACCESS partialExtractExprsb fi:FIELD_IDENT  sb.append('.'); sb.append(fi.getText());  ) 
        |                #( ARRAY_ACCESS partialExtractExprsb  sb.append('[');  partialExtractArraySelectorsb  sb.append(']');  ) 
        |   IDENT 
        ;
    
    
partialExtractArraySelector
        :                #( arrayExpr:ARRAY_SUB partialExtractRangesb (  sb.append(',');  partialExtractRangesb )* ) 
        ;

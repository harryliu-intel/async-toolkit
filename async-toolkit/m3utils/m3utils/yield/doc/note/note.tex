...
\begin{equation}\label{eq:basic}
  P(\text{at least $M$ out of $N$ subsystems OK}) = \sum_{k=M}^N {N \choose k}Y(1)^{N-k}(1-Y(1))^k
\end{equation}
...
binomial yields, assuming independent subsystem yields.  It will be shown that the yield expressions even for
non-Poisson underlying distributions are quite simple in practice.  In
an appendix, we provide a short Scheme program for computing the
necessary formulas.
...
Here we give the detailed implementation that was elided in appendix~\ref{app:hierarchical}.
...
\section{Restrictions on the yield function {\sl Y}}\label{ref:yieldrestrictions}
...
Let's make a small thought experiment.  Three identical submodules $A$, $B$,
...
\begin{equation}\begin{aligned}
 ...
 Y_{X,\epsilon} &=& 0 
\end{aligned}\end{equation}
In other words in this case we seem to have
\begin{equation}
  \xi(2) \le \xi(3) \le {3 \over 2} \xi(2) \quad .
\end{equation}
The same argument generalizes and it seems that
\begin{equation}
  m < n \rightarrow \xi(m) \le \xi(n) \le {{n \choose m}\over {n \choose m } - 1} \xi(m) \quad .
\end{equation}
Generalizing this to the continuous case we have
\begin{equation}
  x < y \rightarrow
  \xi(x) \le
  \xi(y) \le
     { \Gamma(y+1) \over \Gamma(y+1) - \Gamma(y-x+1) \Gamma(x+1) } \xi(x)
\end{equation}
but this seems dubious.  {It's wrong.}

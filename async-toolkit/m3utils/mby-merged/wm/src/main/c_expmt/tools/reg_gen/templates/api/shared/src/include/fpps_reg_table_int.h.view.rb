...
#
class FppsRegTableIntHView < RDL::ViewGen
  ...
  def wmRegTableDefs()
    code = Array.new
    code.push "#define FPPS_REG_TABLE_MAX_ADDR\t\t0x%08x" % @highestRegAddr;
    code.push "#define FPPS_REG_TABLE_NUM_UPPER_ADDR\t%d" %
        @regUpperHash.keys().length();
	code.push "#define FPPS_REG_TABLE_FLAG_VOLATILE\t0x01"
    return code
  end
  ...
  def wmRegTableApi5()
    ...
    @top_map.instances.each_with_index.map do |am_inst,ix|
      ...
      rf_defn.instances.each do |rg_inst|
        ...
        # Calculate register statistics for the rendered header file.
        # - All instances of unique upper 20-bit address ranges
        # - Highest register address in use
        for i0 in 0..max0
            for i1 in 0..max1
                for i2 in 0..max2
                    for w in 0..(width-1) # width is in quad words
                        a = (w * 8) + base + (i0 * stride0) +
                                             (i1 * stride1) +
                                             (i2 * stride2);
                        if (a > @highestRegAddr)
                            @highestRegAddr = a;
                        end
                        regUpper = (a & 0xFFFFF000) >> 12;
                        @regUpperHash[regUpper] = 1;
                    end
                end
            end
        end

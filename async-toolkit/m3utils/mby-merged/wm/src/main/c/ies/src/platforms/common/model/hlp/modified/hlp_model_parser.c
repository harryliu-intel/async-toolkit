...
#include <platforms/common/model/hlp/mby_parser.h>
...
void hlpModelParser( hlp_model *model )
{
    ...
    mbyMacToParser    * const in  = &mac2par;
    mbyParserToMapper       * const out = &par2map;

    in->RX_DATA = model->packetState.RX_DATA;
    in->RX_LENGTH = model->packetState.RX_LENGTH;
    in->RX_PORT = model->packetState.RX_PORT;

    fm_byte hit_idx  [MBY_PA_ANA_STAGES];
    fm_bool hit_idx_v[MBY_PA_ANA_STAGES];
    memset(out->PA_KEYS_VALID,0, 84 * sizeof(fm_bool));
    memset(out->PA_FLAGS,0, 48 * sizeof(fm_bool));
    memset(out->PA_KEYS,0, 84 * sizeof(fm_uint16));
    memset(out->PA_PTRS,0, 8 * sizeof(fm_byte));

    Parser (model->registers , in, out, hit_idx, hit_idx_v);

    model->packetState.RX_FLAGS = out->RX_FLAGS;
    memcpy(model->packetState.PARSER_PKT_META, out->PARSER_PKT_META, 32);
    model->packetState.PA_ADJ_SEG_LEN = out->PA_ADJ_SEG_LEN;
    memcpy(model->packetState.PA_KEYS, out->PA_KEYS, 84);
    memcpy(model->packetState.PA_KEYS_VALID, out->PA_KEYS_VALID, 84);
    memcpy(model->packetState.PA_FLAGS, out->PA_FLAGS, 48); 
    memcpy(model->packetState.PA_PTRS, out->PA_PTRS, 8);
    memcpy(model->packetState.PA_PTRS_VALID, out->PA_PTRS_VALID, 8);
    model->packetState.PA_CSUM_OK = out->PA_CSUM_OK;
    model->packetState.PA_EX_STAGE = out->PA_EX_STAGE;
    model->packetState.PA_EX_DEPTH_EXCEED = out->PA_EX_DEPTH_EXCEED;
    model->packetState.PA_EX_TRUNC_HEADER = out->PA_EX_TRUNC_HEADER;
    model->packetState.PA_EX_PARSING_DONE = out->PA_EX_PARSING_DONE;
    model->packetState.PA_DROP = out->PA_DROP;
    model->packetState.PA_L3LEN_ERR = out->PA_L3LEN_ERR;
...
)
{
    ...
    for (fm_int i = 0; i < MBY_MAX_PA_FLAGS; i++)
        out->PA_FLAGS[i] = 0; 
    ...
    for (fm_int s = 0; s < MBY_PA_ANA_STAGES; s++)
    {
        ...
        for (fm_int r = 0; r < MBY_PA_ANA_RULES; r++)
        {
            ...
            fm_uint16 w1_mask = FM_ARRAY_GET_FIELD(key_w_val, MBY_PARSER_KEY_W, W1_MASK); 
            fm_uint16 w1_val  = FM_ARRAY_GET_FIELD(key_w_val, MBY_PARSER_KEY_W, W1_VALUE);
    ...
    for (fm_int s = 0; s < MBY_PA_ANA_STAGES; s++)
    {
        ...
        if (s_ena && (hit_idx_v[s] == 1))
        {
            ...
            for (fm_int wd = 0; wd < 2; wd++)
            {
                ...
                fm_byte xa_key_offset = FM_ARRAY_GET_FIELD(pa_ext_val, MBY_PARSER_EXT, KEY_OFFSET);
                fm_byte xa_flag_num   = FM_ARRAY_GET_FIELD(pa_ext_val, MBY_PARSER_EXT, FLAG_NUM);
                fm_bool xa_flag_val   = FM_ARRAY_GET_BIT  (pa_ext_val, MBY_PARSER_EXT, FLAG_VALUE);
                fm_byte xa_ptr_num    = FM_ARRAY_GET_FIELD(pa_ext_val, MBY_PARSER_EXT, PTR_NUM);

                // Apply keys to target key array and track keys_valid:
                for (fm_int k = 0; k < xa_key_len; k++)
                {
    ...
    memcpy(hit_idx,firstParam,MBY_PA_ANA_STAGES);
    memcpy(hit_idx_v,secondParam,MBY_PA_ANA_STAGES);

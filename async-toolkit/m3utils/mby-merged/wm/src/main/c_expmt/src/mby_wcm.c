...
)
{
    ...
    {
        mbyClassifierTcamCfg tcam_cfg = mbyClsGetWcmTcamCfg(MBY_CGRP_B_IN_REGS_P, group, slice, profile);
        ...
        {
            mbyClassifierTcamCfg tcam_cfg1 = mbyClsGetWcmTcamCfg(MBY_CGRP_B_IN_REGS_P, group, i, profile);
...
)
{
    ...
    {
        mbyClassifierActionCfg action_cfg = mbyClsGetWcmActionCfg(MBY_CGRP_B_IN_REGS_P, group, profile, ram_num);
...
)
{
    ...
    lookUpWcmTcamCascade(MBY_CGRP_B_IN_REGS_P, keys, profile, group, tcam_hit_info);

    // Get the list of action_entry from action RAMs based on tcam hit index per slice:
    wcmActions(MBY_CGRP_B_IN_REGS_P, profile, group, tcam_hit_info, actions);
}

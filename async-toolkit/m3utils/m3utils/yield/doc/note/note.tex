...
number of subsystems---we can gain some insight by considering the continuous interpretation of equations like (\ref{eq:pi-summary}):
\begin{equation}
  \Pi_{N-k} = \sum_{j=0}^k {k \choose j} (-1)^j Y((N-j)A_u) \quad .
\end{equation}
This equation can be interpreted as the $k$th discrete derivative (finite difference) of the yield function $Y$, with the appropriate sign adjustment:
\begin{equation}\label{eq:finite-diff}
  \Pi_{N-k} = (-1)^k \hat{Y}^{(k)}(N A_u)
\end{equation}
where $\hat{Y}^{(k)}$ is simply the $k$th discrete derivative of $Y$.  We
can then also write
\begin{equation}
  \Pi_{N-k} \approx (-1)^k { Y^{(k)} } (N A_u) \quad .
\end{equation}
...
\begin{equation}\begin{aligned}
    P_{S(18)}(x \geq 16) &= \Pi_{18} + {18 \choose 17} \Pi_{17} + {18 \choose 16} \Pi_{16} \\
    &= Y(18) - {18 \choose 17} \hat{Y}' (18) + {18 \choose 16}\hat{Y}'' (18) \\
    & \approx Y(18) - {18 \choose 17} Y' (17.5) + {18 \choose 16}Y'' (17)
  \end{aligned}
\end{equation}
...
\begin{equation}\begin{aligned}
    Y(18) - {18 \choose 17} \hat{Y}' (18) + {18 \choose 16}\hat{Y}'' (18) &= 0.99932150\ldots = 1 - 0.00067929\ldots\\
     Y(18) - {18 \choose 17} Y' (17.5) + {18 \choose 16}Y'' (17) &= 0.99932071\ldots = 1 - 0.00067850\ldots
\end{aligned}\end{equation}
...
Our approach will be based on polynomial arithmetic.  The yield of a given
system with subsystems incorporated in it, under the assumption of Poisson
statistics, can be represented as a polynomial in $x$, which we define as
\begin{equation}
  x = Y(1) \quad .
\end{equation}

...
sub create_hdl_spec {
	my ( $dir, $model_root, $filename_ral ) = @_;

	my $hdl_file = "./target/GenRTL/regflow/mby/subblock/$filename_ral";
	chdir "$model_root";
	my $cmd    = "find $dir -name \\*.sv ";
	my $vfiles = `$cmd`;
...
my @optionSpec = ( 'dir=s', 'model_root=s', 'filename_ral=s', 'help|h', 'debug|d' );

GetOptions( \%options, @optionSpec );
$debug = $options{debug} if ( defined $options{debug} );

usage() if ( defined $options{help} );
create_hdl_spec( $options{dir}, $options{model_root}, $options{filename_ral} );

...
Finally, the TSU has limited bandwidth: it is able to produce a mark on the
$A$ output at most once in two $A$ cycles.
...
to {\tt o\_vernier\_ready} asserted.  It is composed of time for the
circuit to compute various internal constants plus settling time of
the vernier algorithm.

The initialization time is specified to be
\begin{equation}
  t_{init} \le LCM(p_A,p_B) / p_A + 10 {\tt RAT\_PREC\_BITS}\quad .
\end{equation}

...
(define (read-template-lib path)
  (let* ((rd        (begin
                      (debug "opening for reading : " path " ..." dnl)
                      (FileRd.Open path)))
...
(define *last-template* '())

(define (gen-lib template-path   ;; Liberty template
                 cell-name       ;; name of generated cell
                 named           ;; name of generated library (cell & PVT)
                 ...
                 pow-opts        ;; list of cell power attribs and new values
                 pinpow2-opts    ;; list of pin powers (to be applied fall&rise)
                 comb-read       ;; combinational read?
                 )
  (debug "generating library " named " ..." dnl)
  (debug "CWD is " (run-command "pwd"))
  (debug "reading template lib " template-path " ..." dnl)
  
  (let* ((rd        (begin
                      (debug "opening for reading : " template-path " ..." dnl)
                      (FileRd.Open template-path)))
         (template  (let ((t (LibertyParse.Parse rd))) (Rd.Close rd) t))
         (worklib   (deep-copy template))
         (tech-name (cadr (assoc 'tech-name tech)))
         (wr        (begin
                      (debug "opening for writing : " path " ..." dnl)
                      (FileWr.Open path)))
         )
    (set! *last-template* (deep-copy template))
    
    (dis "/* generated by liberty-scaler.scm */" dnl dnl wr)

    (debug "updating global power attributes..." dnl)
    (map (lambda(x)(update-lib-cell-attr!
                    worklib
                    (car x)
                    (string->number (cadr x))))
         pow-opts)

    (debug "updating pin power attributes..." dnl)
    (map (lambda(x)
           (debug "pin " (car x) " constraint " (cadr x) dnl)
           (update-cell-pin-power-2! (get-lib-cell worklib)
                                     (car x)    ;; pin
                                     (cadr x)   ;; constraint
                                     (caddr x)  ;; value
                                     ))
    ...
    (update-lib-names! worklib named cell-name)
    ...
    (if comb-read
        (begin
          (debug "expanding read timing arcs..." dnl)
          (expand-read-timing-arcs! worklib depth)
          )
        )

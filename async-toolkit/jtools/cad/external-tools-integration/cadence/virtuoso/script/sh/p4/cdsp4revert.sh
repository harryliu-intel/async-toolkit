...
if [ -n "$client_spec_exists" ] ; then
  ...
  p4_revert_command="$p4_command revert"
  
  if [ -n "$change_list" ] ; then
    p4_revert_command="$p4_revert_command -c $change_list"
  fi
  ...
  for cell in $cell_list ; do
    ...
    message "bad_cell_name $bad_cell_name"
    if [ -z "$bad_cell_name" ] ; then
      ...
      if [ -n "$celltargetdir" ] ; then
        ...
        curr_dir_to_revert="$celltargetdir"

        if [ -n "$view_name" ] ; then
          curr_dir_to_revert="$curr_dir_to_revert/$escaped_view"
        fi
        if [ -n "$file_name" ] ; then
          path_to_revert="$path_to_revert/$file_name"
        fi

        message "curr_dir_to_revert $curr_dir_to_revert"
        escaped_dir_to_revert=`echo "$curr_dir_to_revert" | sed -e 's/#/%23/g'`

        curr_p4_revert_command="$p4_revert_command $escaped_dir_to_revert/..."

        $curr_p4_revert_command
      else
